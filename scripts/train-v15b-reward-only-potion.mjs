...
const TRAIN_BASE_SEEDS = [2900000, 2900100, 2900200, 2900300];
const EVAL_BASE_SEEDS = [2905000, 2905100, 2905200];
const REPLICATES = 6;
...
const L2 = 0.001;
const EPSILON = 0.20;
const ORDER_SEED_BASE = 2906000;
const POLICY_SEED = 2908000;

const PREREG_COMMIT = "7d48968b71304cf3190e8c808414455af08b0895";
...
async function collectDataset(connectome, dnSlot, baseSeeds, label) {
  ...
  for (const baseSeed of baseSeeds) {
    ...
    console.log(
      "[v15B-" + label + "] baseSeed=" + baseSeed + " rows=" + rows.length,
    );
...
async function main() {
  const outDir = resolve("results/train-v15b-reward-only-potion");
  ...
  console.log(
    "[v15B] frozen v15A12 representation sha256=" + frozen.digest,
  );
  ...
  const outcome = pass
    ? "V15B_REWARD_ONLY_LEARNING_PASS"
    : "V15B_REWARD_ONLY_LEARNING_FAIL";
  ...
  console.log(
    "[v15B] model sha256=" + modelSha256 +
      " outcome=" + outcome +
      " v15C=" + (pass ? "PREREGISTRATION_AUTHORIZED" : "BLOCKED"),
  );

  const output = {
    schema: "maplefly.v15b.reward-only-potion-learning.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15b.md",
  ...
  await writeFile(
    resolve(outDir, "v15b_training.json"),
