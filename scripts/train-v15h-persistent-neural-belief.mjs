#!/usr/bin/env node
// v15H: persistent recurrent belief from frozen neural margin + own previous action.
...
const V15H_TRAIN_BASE_SEEDS = [3311000, 3321000, 3331000];
const V15H_EVAL_BASE_SEEDS = [3341000, 3351000, 3361000];
const V15H_PREREG_COMMIT =
  "bb2a824016b45dc3243c14b4cd55d174768db5fb";
const V15H_CEM_SEED = 3378000;
const V15H_GENERATIONS = 60;
const V15H_POPULATION = 128;
const V15H_ELITES = 16;
const V15H_OLD_WEIGHT = 0.20;
const V15H_ELITE_WEIGHT = 0.80;
const V15H_MIN_STD = 0.05;
const V15H_PARAM_CLAMP = 6;
  ...
  for (const baseSeed of baseSeeds) {
    for (const episode of makeBlock(baseSeed)) {
      ...
      if (row.liveSteps !== MAX_STEPS || row.potionEvents.length !== 10) {
        throw new Error("v15H incomplete trainer tape");
      }
      rows.push(row);
      console.log(
        "[v15H-" + label + "] seed=" + row.seed +
...
function simulateRecurrentPolicy(
  tape,
  params,
  {
    memoryOff = false,
  ...
  for (const step of steps) {
    ...
    const decision = decisions.get(step);
    if (decision) {
      const previousBelief = memoryOff ? 0 : belief;
      const margin = neuralOff ? 0 : decision.baseMargin;
      const feedback = actionFeedbackOff
        ? 0
        : params[2] * previousAction;
      belief =
        decay * previousBelief +
        params[1] * margin +
        feedback;
      const score = belief + params[3];
...
function evaluateRecurrent(tapes, params, options = {}) {
  const rows = tapes.map((tape) =>
    simulateRecurrentPolicy(tape, params, options),
  );
...
function trainingFitness(tapes, params) {
  const result = evaluateRecurrent(tapes, params);
  return {
    fitness:
...
function trainCem(tapes) {
  const random = mulberry32(V15H_CEM_SEED);
  let distributionMean = new Float64Array([0, 1, -1, 0]);
  let distributionStd = new Float64Array([1, 1, 1, 1]);
  const trace = [];

  for (
    let generation = 1;
    generation <= V15H_GENERATIONS;
    generation += 1
  ) {
    const population = [];
    for (let member = 0; member < V15H_POPULATION; member += 1) {
    ...
    const elites = population.slice(0, V15H_ELITES);
    const eliteMean = new Float64Array(4);
    ...
    const eliteStd = new Float64Array(4);
    for (const elite of elites) {
      for (let i = 0; i < 4; i += 1) {
        eliteStd[i] +=
          (elite.params[i] - eliteMean[i]) ** 2;
      }
    }
    for (let i = 0; i < 4; i += 1) {
      eliteStd[i] = Math.sqrt(eliteStd[i] / elites.length);
      distributionMean[i] = clamp(
        V15H_OLD_WEIGHT * distributionMean[i] +
          V15H_ELITE_WEIGHT * eliteMean[i],
        -V15H_PARAM_CLAMP,
        V15H_PARAM_CLAMP,
      );
      distributionStd[i] = Math.max(
        V15H_MIN_STD,
        V15H_OLD_WEIGHT * distributionStd[i] +
          V15H_ELITE_WEIGHT * eliteStd[i],
      );
    }
    ...
    if (
      ...
      const meanScore = trainingFitness(
        tapes,
        distributionMean,
      );
      trace.push({
  ...
  return {
    params: Float64Array.from(
      distributionMean,
      (value) =>
        clamp(value, -V15H_PARAM_CLAMP, V15H_PARAM_CLAMP),
    ),
...
function oracleMinimumUsesRecurrentTape(tape) {
  let minimum = Infinity;
  let bestActions = null;
  for (let mask = 0; mask < 1024; mask += 1) {
    const actions = Array.from(
    ...
    const row = simulateRecurrentPolicy(
      tape,
      [0, 0, 0, 0],
      { fixedActions: actions },
    );
  ...
  if (!Number.isFinite(minimum)) {
    throw new Error("v15H oracle cannot survive tape " + tape.seed);
  }
  return { minimumUses: minimum, bestActions };
}

function addEconomyMetrics(
  evaluation,
  oracle,
  evalBaseSeeds,
) {
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15H DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(3389000);
  const trainTapes = await collectTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15H_TRAIN_BASE_SEEDS,
    interruptionRandom,
    label: "train",
  });
  const evalTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15H_EVAL_BASE_SEEDS,
    interruptionRandom,
    label: "eval",
  });
  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15H tape count mismatch");
  }
  ...
  const oracle = new Map(
    evalTapes.map((tape) => [
      tape.seed,
      oracleMinimumUsesRecurrentTape(tape),
    ]),
  );

  const full = addEconomyMetrics(
    evaluateRecurrent(evalTapes, params),
    oracle,
    V15H_EVAL_BASE_SEEDS,
  );
  ...
  const memoryContribution = componentContribution(
    full,
    memoryOff,
  );
  const actionFeedbackContribution = componentContribution(
    full,
    actionFeedbackOff,
  );
  ...
  const gateChecks = {
    ...
    minSeedSurvival:
      full.minSeedSurvivalRate >= 0.625,
    meanExcessUses: full.meanExcessUses <= 1.5,
    wastedHealingPerDrink:
      full.wastedHealingPerDrink <= 10,
    memoryContribution:
  ...
  const output = {
    schema: "maplefly.v15h.persistent-neural-belief.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15h.md",
      commit: V15H_PREREG_COMMIT,
    },
    frozenNeuralCore: {
    ...
    cohorts: {
      trainBaseSeeds: V15H_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15H_EVAL_BASE_SEEDS,
      trainTapes: trainTapes.length,
      evalTapes: evalTapes.length,
    },
    optimizer: {
      type: "DETERMINISTIC_CROSS_ENTROPY_METHOD",
      generations: V15H_GENERATIONS,
      population: V15H_POPULATION,
      elites: V15H_ELITES,
      oldWeight: V15H_OLD_WEIGHT,
      eliteWeight: V15H_ELITE_WEIGHT,
      minimumStd: V15H_MIN_STD,
      parameterClamp: [-V15H_PARAM_CLAMP, V15H_PARAM_CLAMP],
      seed: V15H_CEM_SEED,
      trainingFitness:
    ...
    policy: {
      parameters: {
        ...
        marginGain: params[1],
        actionFeedback: params[2],
        bias: params[3],
      },
      paramsSha256,
      runtimeInputs: [
        "frozen_v15e2_baseMargin",
        "previous_potion_action",
        "previous_internal_belief",
      ],
    },
    tapeValidity: {
      metrics: validity,
    ...
    evaluation: {
      ...
      neuralOff,
      memoryContribution,
      actionFeedbackContribution,
      neuralContribution,
      oracle: Object.fromEntries(
        [...oracle.entries()].map(([seed, value]) => [
          seed,
          value,
        ]),
      ),
    ...
    pass: scientificPass,
    next: scientificPass
      ? "FRESH_INTERACTIVE_48S_RECURRENT_VALIDATION_PREREGISTRATION_AUTHORIZED"
      : "FREEZE_AND_DIAGNOSE",
    deployment: "BLOCKED",
  ...
  const outDir = resolve(
    "results/v15h-persistent-neural-belief",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15h_training.json"),
