...
const PRACTICE_BASE_SEEDS = [701000, 711000, 721000];
const PRACTICE_DISTANCES = [145, 185, 225, 265];
const FINAL_BASE_SEEDS = [751000, 761000, 771000];
...
async function main() {
  const outDir = resolve("results/experiment-v11c");
  ...
  const output = {
    meta: {
      schema: "maplefly.experiment-v11c.single-jump-sarsa.1",
      phase: "A",
      ...
      learner: "linear SARSA(0) WAIT/JUMP over frozen 1316-DN current feature; single actual-jump budget",
      windows: {
        settle: SETTLE_STEPS,
      ...
      gate:
        "FULL>=70%; FULL-VISUAL_OFF>=25pp; FULL-DN_SHUFFLED>=20pp; every FULL>=60%; timeout<=25%; NO_OBSTACLE any-jump<=30%; NO_OBSTACLE target reach>=85%",
    },
    practice,
    summary,
    perRun,
  ...
  await writeFile(
    resolve(outDir, "experiment_v11c.json"),
