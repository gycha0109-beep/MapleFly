...
async function main() {
  ...
  const d2ArtifactPath = process.env.V15N_D6_D2_D2_ARTIFACT_FILE;
  if (!d2ArtifactPath) {
    throw new Error("V15N_D6_D2_D2_ARTIFACT_FILE is required");
  }
  const d2Bytes = await readFile(d2ArtifactPath);
  const d2Sha256 = createHash("sha256").update(d2Bytes).digest("hex");
  if (d2Sha256 !== V15N_D6_D2_D3_D2_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D3 authoritative D2-D2 evidence SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2-D3 prerequisite outcome mismatch");
  }
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D3 v15N evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2-D3 DN identity/order mismatch");
  }
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D6-D2-D3 frozen preprocessing mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2-D3 detector weight SHA mismatch");
  }
  d1AssertClose(
    detector.threshold,
  ...
  d2d1VerifyEventMetrics(
    oldD2.train,
    frozenD2.eventizerReproduction.train,
    "oldD2.train",
  );
  d2d1VerifyEventMetrics(
    oldD2.eval,
    frozenD2.eventizerReproduction.eval,
    "oldD2.eval",
  );
  ...
  const scalarFrameMetrics = {
    train: d2d2Metrics(labelledRows.train, scalarModel),
    eval: d2d2Metrics(labelledRows.eval, scalarModel),
  };
  d2d3VerifyScalarMetrics(
    scalarFrameMetrics.train,
    frozenD2.metrics.scalar.train,
    "scalar.train",
  );
  d2d3VerifyScalarMetrics(
    scalarFrameMetrics.eval,
    frozenD2.metrics.scalar.eval,
    "scalar.eval",
  );
  ...
  const support = {
    eval: d2d3Support(metrics.eval),
    holdout: d2d3Support(metrics.holdout),
  };
  const supportPass = support.eval.pass && support.holdout.pass;
  const eventizationPass =
    supportPass &&
    d2d3Pass(metrics.eval) &&
    d2d3Pass(metrics.holdout);

  let outcome;
  if (!supportPass) {
    outcome = "V15N_D6_D2_D3_INSUFFICIENT_EVENT_SUPPORT";
  } else if (eventizationPass) {
    outcome = "V15N_D6_D2_D3_CAUSAL_EVENTIZATION_DEMONSTRATED";
  } else {
    outcome =
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    ...
    console.log(
      "[v15N-D6-D2-D3] " +
        ...
        m.f1.toFixed(4) +
        " countRatio=" +
        m.eventCountRatio.toFixed(4) +
        " countMAE=" +
        m.meanAbsolutePerEpisodeCountError.toFixed(4) +
        " events=" +
        m.neuralEvents +
        " hits=" +
        m.physicalHits,
  ...
  const output = {
    schema: "maplefly.v15n-d6-d2-d3.causal-scalar-onset-eventizer.1",
    preregistration: {
      path: "history/prereg_v15n_d6_d2_d3.md",
      commit: V15N_D6_D2_D3_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD6D2D2Receipt:
    ...
    scalarReadoutReproduction: {
      train: scalarFrameMetrics.train,
      eval: scalarFrameMetrics.eval,
      modelSha256: scalarModelSha256,
      pass: true,
    },
    eventizer: {
      type: "causal binary rising edge",
      previousPositiveInitial: false,
      threshold: V15N_D6_D2_D2_THRESHOLD,
      refractoryFrames: 0,
      matchingWindowSteps: V15N_D6_D2_MATCH_MAX_AGE_STEPS,
    },
    oldD2Comparator: oldD2,
    metrics,
    support,
    gates: {
      supportPass,
      eventizationPass,
      precision: V15N_D6_D2_D3_EVENT_GATE,
      recall: V15N_D6_D2_D3_EVENT_GATE,
      f1: V15N_D6_D2_D3_EVENT_GATE,
      countRatioMin: V15N_D6_D2_D3_COUNT_RATIO_MIN,
      countRatioMax: V15N_D6_D2_D3_COUNT_RATIO_MAX,
      countMaeMax: V15N_D6_D2_D3_COUNT_MAE_MAX,
    },
    provenance: {
    ...
    diagnosticEventizerDeployable: false,
    deployment: "BLOCKED",
    deployedPotion: "v15D",
    v16c: "BLOCKED",
  };
  ...
  const outDir = resolve(
    "results/v15n-d6-d2-d3-causal-scalar-onset-eventizer",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d6_d2_d3.json"),
