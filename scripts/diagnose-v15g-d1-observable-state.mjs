#!/usr/bin/env node
// v15G-D1 diagnostic: oracle labels are evaluator-only and never deployment weights.
...
function buildReachableOracleStates(tape) {
  ...
  for (let mask = 0; mask < 1024; mask += 1) {
    ...
    for (let index = 0; index < row.trajectory.length; index += 1) {
      ...
      const prefix = actions.slice(0, index).join("");
      ...
      if (!state) {
        state = {
          ...
          prefix,
          baseMargin: transition.baseMargin,
        ...
        state.hpBefore !== transition.hpBefore ||
        state.baseMargin !== transition.baseMargin ||
        state.history.some(
          (value, slot) => value !== transition.history[slot],
        )
      ) {
        throw new Error("v15G-D1 prefix state is not deterministic");
      }
  ...
  return [...states.values()].map((state) => {
    if (
      ...
      throw new Error("v15G-D1 incomplete oracle action support");
    }
    const label =
...
function aliasAudit(states) {
  const groups = new Map();
  for (const state of states) {
    let group = groups.get(state.observableGroup);
    if (!group) {
      group = [];
      groups.set(state.observableGroup, group);
    }
    group.push(state);
  }
  ...
  for (const [key, group] of groups) {
    ...
    if (labels.size > 1) {
      ...
      conflicts.push({
        key,
        seed: group[0].seed,
        decisionIndex: group[0].decisionIndex,
        step: group[0].step,
        baseMargin: group[0].baseMargin,
        history: group[0].history,
  ...
  return {
    ...
    conflictStateFraction: strictStates
      ...
      : 0,
    examples: conflicts.slice(0, 12),
  };
}

function probeFeature(row, type) {
  if (type === "MARGIN_ONLY") return [1];
  if (type === "FIXED_V15G_CLASS") {
    return [1, ...row.history];
  }
  if (type === "FREE_MARGIN_LINEAR") {
    return [row.baseMargin, 1, ...row.history];
  }
  throw new Error("unknown probe " + type);
}

function probeScore(row, type, params) {
  if (type === "MARGIN_ONLY") {
    return row.baseMargin + params[0];
  }
  if (type === "FIXED_V15G_CLASS") {
    let value = row.baseMargin + params[0];
    for (let i = 0; i < 4; i += 1) {
      value += params[i + 1] * row.history[i];
    }
    return value;
  }
  if (type === "FREE_MARGIN_LINEAR") {
    let value = params[0] * row.baseMargin + params[1];
    for (let i = 0; i < 4; i += 1) {
      value += params[i + 2] * row.history[i];
    }
    return value;
  }
  throw new Error("unknown probe " + type);
...
function trainProbe(rows, type) {
  const strict = rows.filter((row) => row.label !== null);
  const waitCount = strict.filter((row) => row.label === 0).length;
  const drinkCount = strict.filter((row) => row.label === 1).length;
  if (!waitCount || !drinkCount) {
    throw new Error("v15G-D1 probe class support missing");
  }

  const size =
    type === "MARGIN_ONLY"
      ? 1
      : type === "FIXED_V15G_CLASS"
        ? 5
        : 6;
  const params = new Float64Array(size);

  for (let epoch = 0; epoch < D1_EPOCHS; epoch += 1) {
    const gradient = new Float64Array(size);
    for (const row of strict) {
      const probability = sigmoid(probeScore(row, type, params));
      const classWeight =
        row.label === 1
          ? 0.5 / drinkCount
          : 0.5 / waitCount;
      const residual = row.label - probability;
      const feature = probeFeature(row, type);
      for (let i = 0; i < size; i += 1) {
        gradient[i] += classWeight * residual * feature[i];
      }
    }
    for (let i = 0; i < size; i += 1) {
      params[i] = clamp(
...
function classificationMetrics(rows, type, params) {
  const strict = rows.filter((row) => row.label !== null);
  let waitN = 0;
  let waitCorrect = 0;
  let drinkN = 0;
  let drinkCorrect = 0;
  for (const row of strict) {
    const predicted = probeScore(row, type, params) > 0 ? 1 : 0;
    if (row.label === 1) {
      drinkN += 1;
...
function simulateProbePolicy(tape, type, params) {
  const decisions = new Map(
    tape.potionEvents.map((event, index) => [
      event.step,
      { ...event, index },
  ...
  for (const step of steps) {
    ...
    if (decision) {
      const observable = {
        baseMargin: decision.baseMargin,
        history: [...history],
      };
      const score = probeScore(observable, type, params);
...
function downstreamMetrics(tapes, type, params, oracle) {
  const rows = tapes.map((tape) =>
    simulateProbePolicy(tape, type, params),
  );
  const survivors = rows.filter((row) => row.survived);
  const totalUses = rows.reduce((sum, row) => sum + row.uses, 0);
  const totalWasted = rows.reduce((sum, row) => sum + row.wasted, 0);
  const perSeed = V15G_EVAL_BASE_SEEDS.map((baseSeed) => {
  ...
  const meanExcessUses = survivors.length
    ? mean(
        survivors.map(
...
function probeAudit(type, trainStates, evalStates, evalTapes, oracle) {
  const params = trainProbe(trainStates, type);
  ...
  return {
    ...
    paramsSha256: createHash("sha256")
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15G-D1 DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(3309000);
  const trainTapes = await collectTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15G_TRAIN_BASE_SEEDS,
    interruptionRandom,
    label: "train",
  });
  const evalTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15G_EVAL_BASE_SEEDS,
    interruptionRandom,
    label: "eval",
  });
  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15G-D1 tape count mismatch");
  }
  ...
  if (!Object.values(validityChecks).every(Boolean)) {
    throw new Error("v15G-D1 frozen tape validity mismatch");
  }
  ...
  const trainAlias = aliasAudit(trainStates);
  const evalAlias = aliasAudit(evalStates);
  const oracle = new Map(
    evalTapes.map((tape) => [
      tape.seed,
      oracleMinimumUses(tape),
    ]),
  );

  const marginOnly = probeAudit(
    "MARGIN_ONLY",
    trainStates,
    evalStates,
    evalTapes,
  ...
  let outcome;
  if (fixed.economyCapable) {
    outcome = "V15G_D1_REWARD_CREDIT_ASSIGNMENT_LIMITED";
  } else if (free.economyCapable) {
    outcome = "V15G_D1_FIXED_MARGIN_SCALE_LIMITED";
  } else if (
    evalAlias.conflictStateFraction >= 0.20 ||
    !free.classificationSupport
  ) {
    outcome = "V15G_D1_OBSERVABLE_STATE_LIMITED";
  } else {
    outcome = "V15G_D1_INCONCLUSIVE";
  }

  console.log(
    "[v15G-D1] states train=" + trainStates.length +
      " eval=" + evalStates.length +
      " conflictState=" +
      (evalAlias.conflictStateFraction * 100).toFixed(1) + "%",
  );
  for (const probe of [marginOnly, fixed, free]) {
    console.log(
      "[v15G-D1] " + probe.type +
        " BA=" +
        (probe.classification.balancedAccuracy * 100).toFixed(1) +
        "% wait=" +
        (probe.classification.waitRecall * 100).toFixed(1) +
        "% drink=" +
        (probe.classification.drinkRecall * 100).toFixed(1) +
        "% survival=" +
        (probe.downstream.survivalRate * 100).toFixed(1) +
        "% excess=" +
        Number(probe.downstream.meanExcessUses).toFixed(3) +
        " capable=" +
        probe.economyCapable,
    );
  }
  console.log("[v15G-D1] outcome=" + outcome);

  const output = {
    schema: "maplefly.v15g-d1.observable-state-localization.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15g_d1.md",
      commit: V15G_D1_PREREG_COMMIT,
    },
    sourceV15G: {
      preregistration: V15G_PREREG_COMMIT,
      runId: 35969268691,
      artifactId: 10796405663,
      artifactDigest:
        "sha256:c27d7a42ef2ac9fad1cf9c687b4d55ddf33a019c49af0f185ff138a76693b99f",
      outcome: "V15G_SEQUENTIAL_POTION_ECONOMY_FAIL",
    },
    frozenNeuralCore: {
    ...
    cohorts: {
      trainBaseSeeds: V15G_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15G_EVAL_BASE_SEEDS,
    ...
    probeContract: {
      epochs: D1_EPOCHS,
      learningRate: D1_LEARNING_RATE,
      l2: D1_L2,
      parameterClamp: [-D1_PARAM_CLAMP, D1_PARAM_CLAMP],
      classWeighting: "EQUAL_TOTAL_WAIT_DRINK",
      tieStates: "EXCLUDED_FROM_PROBE_LOSS_AND_CLASSIFICATION",
      deploymentEligible: false,
    },
    probes: {
      marginOnly,
      fixedV15gClass: fixed,
      freeMarginLinear: free,
    },
    outcome,
    deployment: "BLOCKED",
    supervisedProbeDeployment: "FORBIDDEN",
    next: "FREEZE_AND_DESIGN_NEW_PREREGISTRATION_FROM_LOCALIZATION",
  };

  const outDir = resolve(
    "results/v15g-d1-observable-state-localization",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15g_d1.json"),
