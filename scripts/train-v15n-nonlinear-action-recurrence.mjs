...
async function main() {
  ...
  if (
    ...
    throw new Error("v15N DN identity/order mismatch");
  }
  ...
  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15N tape count mismatch");
  }
  ...
  const actionStateOff = addEconomyMetrics(
    evaluatePolicy(evalTapes, params, {
      actionStateOff: true,
    }),
    oracle,
  ...
  const actionStateContribution = contribution(
    full,
    actionStateOff,
    1.0,
  );
  ...
  const gateChecks = {
    ...
    actionStateContribution: actionStateContribution.contributes,
  ...
  console.log(
    "[v15N] ACTION_STATE_OFF survival=" +
      (actionStateOff.survivalRate * 100).toFixed(1) +
      "% excess=" +
      Number(actionStateOff.meanExcessUses).toFixed(3) +
      " contributes=" +
      actionStateContribution.contributes +
      " NEURAL_OFF survival=" +
  ...
  const output = {
    schema: "maplefly.v15n.nonlinear-action-recurrence.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15n.md",
      commit: V15N_PREREG_COMMIT,
    },
    evidenceChain: {
      v15mD1Outcome: "V15M_D1_TRACE_AND_PCA_SIGNAL_STABLE",
      v15mD2Outcome: "V15M_D2_SCALAR_ACTION_STATE_BOTTLENECK",
      v15mD3Receipt:
        "af2564893bf50ebef25e4a95c031588557e3e33b",
      v15mD3Outcome:
        "V15M_D3_TWO_TIMESCALE_STATE_BOTTLENECK",
      supervisedDiagnosticWeightsLoaded: false,
    },
    frozenNeuralCore: {
    ...
    cohorts: {
      ...
      interruptionRngSeed: 4147000,
    },
    representation: {
      neural:
        "CAUSAL_EXPONENTIAL_DN_TRACE_PCA32",
      dnCount: DN_COUNT,
      frameSteps: POTION_FRAME_STEPS,
      frameSeconds: POTION_FRAME_STEPS * STEP_SECONDS,
      traceHalfLifeSeconds: V15N_TRACE_HALF_LIFE_SECONDS,
      traceDecay: V15N_TRACE_DECAY,
      standardization: "TRAIN_ONLY_MEAN_STD_SCALE_FLOOR_1E-6",
      pca: {
        components: V15N_PCA_COMPONENTS,
        iterations: V15N_PCA_ITERATIONS,
        seedBase: V15N_PCA_SEED,
        labelsUsed: false,
      ...
      ownActionState: {
        ...
        update:
          "h=tanh(R*h_previous+q*previousOwnAction)",
        recurrentBias: false,
        zeroActionZeroStateFixedPoint: true,
    ...
    optimizer: {
      ...
      generations: V15N_GENERATIONS,
      population: V15N_POPULATION,
      elites: V15N_ELITES,
      oldWeight: V15N_OLD_WEIGHT,
      eliteWeight: V15N_ELITE_WEIGHT,
      minimumStd: V15N_MIN_STD,
      parameterClamp: [-V15N_PARAM_CLAMP, V15N_PARAM_CLAMP],
      seed: V15N_CEM_SEED,
      initialMean: Array(V15N_POLICY_PARAMS).fill(0),
      initialStd: Array(V15N_POLICY_PARAMS).fill(1),
      trainingFitness:
    ...
    policy: {
      parameterCount: V15N_POLICY_PARAMS,
      parameters: {
        bias: params[0],
        neuralWeights: Array.from(
          params.slice(1, V15N_PCA_COMPONENTS + 1),
        ),
        recurrentMatrix: [
          [params[33], params[34]],
          [params[35], params[36]],
        ],
        actionInput: [params[37], params[38]],
        stateWeights: [params[39], params[40]],
        recurrentBias: [0, 0],
      },
      paramsSha256,
      runtimeInputs: [
        "current_32d_pca_of_causal_malecns_dn_trace",
        "previous_2d_internal_action_state",
    ...
    evaluation: {
      ...
      trainDecisionMean,
      contributions: {
        actionState: actionStateContribution,
    ...
    pass: scientificPass,
    next: scientificPass
      ? "FREEZE_EVIDENCE_THEN_DEPLOYMENT_SPECIFIC_VALIDATION_PREREGISTRATION_AUTHORIZED"
      : "FREEZE_AND_DIAGNOSE",
    deployment: "BLOCKED",
    deployedPotion: "v15D",
  ...
  const outDir = resolve(
    "results/v15n-nonlinear-action-recurrence",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_training.json"),
