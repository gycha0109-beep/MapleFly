...
const FINAL_BASE_SEEDS = [1151000, 1161000, 1171000];
...
async function main() {
  const outDir = resolve("results/verify-v11-cross-skill-lc4");
  ...
  const output = {
    meta: {
      schema:
        "maplefly.verify-v11-cross-skill-lc4-coexistence.1",
      phase: "cross-skill-lc4-coexistence",
  ...
  await writeFile(
    resolve(outDir, "verify_v11_cross_skill_lc4.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  if (!gate) {
    console.log(
      "V11-CROSS-SKILL-LC4=FAIL " +
        "FULL=" + percent(meanFull) +
        " OBSTACLE_CUE_OFF=" + percent(meanVisualOff) +
        " SHUFFLED=" + percent(meanShuffled) +
        " FULL_JUMPS=" + meanFullActualJumps.toFixed(3) +
        " RETRIES=" + meanFullRetryJumps.toFixed(3) +
        " TARGET_ONLY_JUMP=" + percent(noObstacleAnyJump) +
        " TARGET_ONLY_REACH=" + percent(noObstacleReach),
    );
  } else {
    console.log(
      "V11-CROSS-SKILL-LC4=PASS " +
        "FULL=" + percent(meanFull) +
        " OBSTACLE_CUE_OFF=" + percent(meanVisualOff) +
        " SHUFFLED=" + percent(meanShuffled) +
        " FULL_JUMPS=" + meanFullActualJumps.toFixed(3) +
