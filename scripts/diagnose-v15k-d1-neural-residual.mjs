#!/usr/bin/env node
// v15K-D1 diagnostic: episode-specific neural residual audit; supervised probes are never deployment weights.
...
const V15K_D1_PREREG_COMMIT =
  "01d1e90520ddd5891b268f0fd84c2f5aeb9ca04e";
const V15K_D1_TRAIN_BASE_SEEDS = [3601000, 3611000, 3621000];
const V15K_D1_EVAL_BASE_SEEDS = [3631000, 3641000, 3651000];
const V15K_D1_INTERRUPTION_SEED = 3667000;
const V15K_D1_PCA_SEED = 3678000;
const V15K_D1_PCA_COMPONENTS = 32;
const V15K_D1_PCA_ITERATIONS = 80;
const V15K_D1_PROBE_EPOCHS = 250;
const V15K_D1_PROBE_LR = 0.05;
const V15K_D1_PROBE_L2 = 0.001;
const V15K_D1_PROBE_CLAMP = 10;
...
function decisionBoundary({
  ...
  if (state.potionFrameCount === POTION_HISTORY_FRAMES) {
    ...
    state.potionEvents.push({
      ...
      pooledDnFeature: neural.feature,
    });
...
function buildReachableOracleStates(tape) {
  ...
  if (tape.potionEvents.length !== 10) {
    throw new Error("v15K-D1 tape must contain 10 POTION opportunities");
  }
  ...
  for (let mask = 0; mask < 1024; mask += 1) {
    ...
    for (let index = 0; index < row.trajectory.length; index += 1) {
      ...
      if (!state) {
        ...
        throw new Error("v15K-D1 prefix determinism mismatch");
      }
  ...
  return [...states.values()].map((state) => {
    if (
      ...
      throw new Error("v15K-D1 incomplete oracle support");
    }
    return {
      ...state,
      label:
...
function validateRichFeatures(tapes) {
  for (const tape of tapes) {
    if (tape.potionEvents.length !== 10) {
      throw new Error("v15K-D1 decision count mismatch");
    }
    for (const event of tape.potionEvents) {
      if (
        !Array.isArray(event.pooledDnFeature) ||
        event.pooledDnFeature.length !== DN_COUNT ||
        event.pooledDnFeature.some((value) => !Number.isFinite(value))
      ) {
        throw new Error("v15K-D1 pooled DN feature contract mismatch");
      }
    }
  }
}

function computeTrainTemporalMeans(tapes) {
  ...
  for (const tape of tapes) {
    tape.potionEvents.forEach((event, index) => {
      scalar[index] += event.baseMargin;
      const feature = event.pooledDnFeature;
      for (let d = 0; d < DN_COUNT; d += 1) {
        dn[index][d] += feature[d];
      }
      counts[index] += 1;
    });
  }
  ...
  for (let index = 0; index < 10; index += 1) {
    if (counts[index] !== tapes.length) {
      throw new Error("v15K-D1 temporal mean support mismatch");
    }
    scalar[index] /= counts[index];
    for (let d = 0; d < DN_COUNT; d += 1) {
      dn[index][d] /= counts[index];
    }
  }
...
function makeResidualData(tapes, temporalMeans) {
  const bySeed = new Map();
  const residualVectors = [];

  for (const tape of tapes) {
    const rawScalar = [];
    const residualScalar = [];
    const residualDn = [];
    tape.potionEvents.forEach((event, index) => {
      rawScalar.push(event.baseMargin);
      residualScalar.push(
        event.baseMargin - temporalMeans.scalar[index],
      );
      const vector = new Float64Array(DN_COUNT);
      for (let d = 0; d < DN_COUNT; d += 1) {
        vector[d] =
          event.pooledDnFeature[d] - temporalMeans.dn[index][d];
      }
      residualDn.push(vector);
      residualVectors.push(vector);
    });
    bySeed.set(tape.seed, {
      rawScalar,
      residualScalar,
      residualDn,
      residualPc: null,
...
function normalizeVector(vector) {
  ...
  if (!(norm > 1e-12)) {
    throw new Error("v15K-D1 PCA vector collapsed");
  }
  for (let i = 0; i < vector.length; i += 1) {
    vector[i] /= norm;
  }
...
function fitResidualPca(vectors) {
  const n = vectors.length;
  if (n !== 240) {
    throw new Error("v15K-D1 PCA TRAIN vector count mismatch " + n);
  }

  const gram = new Float64Array(n * n);
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j <= i; j += 1) {
      const value = vectorDot(vectors[i], vectors[j]) / n;
      gram[i * n + j] = value;
      gram[j * n + i] = value;
    }
  }
  ...
  for (let component = 0; component < V15K_D1_PCA_COMPONENTS; component += 1) {
    const random = mulberry32(V15K_D1_PCA_SEED + component);
    ...
    for (let iteration = 0; iteration < V15K_D1_PCA_ITERATIONS; iteration += 1) {
...
function projectResidualData(data, pca) {
  for (const row of data.bySeed.values()) {
    row.residualPc = row.residualDn.map((vector) =>
      Float64Array.from(
        pca.components,
        (component) => vectorDot(vector, component),
      ),
    );
  }
}

function actionHistoryFeature(actions) {
  const result = [];
  for (let lag = 1; lag <= 4; lag += 1) {
    const index = actions.length - lag;
    result.push(index >= 0 ? actions[index] : 0);
  }
  return result;
}

function featureForState(
  state,
  variant,
  dataBySeed,
  sequenceSeedOverride = null,
) {
  const sourceSeed =
    sequenceSeedOverride?.get(state.seed) ?? state.seed;
  const data = dataBySeed.get(sourceSeed);
  if (!data) throw new Error("v15K-D1 missing neural sequence");

  const feature = [];
  if (variant === "P0_PHASE_ACTION") {
    for (let position = 0; position < 10; position += 1) {
      feature.push(Number(position === state.decisionIndex));
    }
  } else if (
    variant === "P1_RAW_MARGIN" ||
    variant === "P2_RESIDUAL_MARGIN"
  ) {
    const sequence =
      variant === "P1_RAW_MARGIN"
        ? data.rawScalar
    ...
    for (let lag = 0; lag < 10; lag += 1) {
      const index = state.decisionIndex - lag;
      feature.push(index >= 0 ? sequence[index] : 0);
    }
  } else if (variant === "P3_RESIDUAL_DN32") {
    for (let lag = 0; lag < 10; lag += 1) {
      const index = state.decisionIndex - lag;
      if (index >= 0) {
        for (const value of data.residualPc[index]) feature.push(value);
      } else {
        for (let pc = 0; pc < V15K_D1_PCA_COMPONENTS; pc += 1) {
          feature.push(0);
        }
      }
    }
  } else {
    throw new Error("unknown v15K-D1 probe " + variant);
  }

  feature.push(...actionHistoryFeature(state.actions));
...
function classWeights(examples) {
  ...
  if (!wait || !drink) {
    throw new Error("v15K-D1 class support missing");
  }
  return {
    wait,
    drink,
...
function trainLinearProbe(states, variant, dataBySeed) {
  const examples = states
    .filter((row) => row.label !== null)
    .map((row) => ({
      x: featureForState(row, variant, dataBySeed),
      y: row.label,
    }));
  ...
  for (let epoch = 0; epoch < V15K_D1_PROBE_EPOCHS; epoch += 1) {
    const grad = new Float64Array(dim);
    let gradBias = 0;
    for (const row of examples) {
      let score = bias;
      for (let i = 0; i < dim; i += 1) {
        score += weights[i] * row.x[i];
      }
    ...
    bias = clamp(
      bias + V15K_D1_PROBE_LR * gradBias,
      -V15K_D1_PROBE_CLAMP,
      V15K_D1_PROBE_CLAMP,
    );
    for (let i = 0; i < dim; i += 1) {
      weights[i] = clamp(
        weights[i] +
          V15K_D1_PROBE_LR *
            (grad[i] - V15K_D1_PROBE_L2 * weights[i]),
        -V15K_D1_PROBE_CLAMP,
        V15K_D1_PROBE_CLAMP,
      );
    }
  ...
  return { type: "LINEAR", variant, dim, weights, bias };
}

function probeScore(model, feature) {
  let score = model.bias;
  for (let i = 0; i < model.dim; i += 1) {
    score += model.weights[i] * feature[i];
  }
  return score;
}
...
function classificationMetrics(
  ...
  for (const state of states) {
    ...
    const feature = featureForState(
      state,
      variant,
      dataBySeed,
      sequenceSeedOverride,
    );
    const predicted = probeScore(model, feature) > 0 ? 1 : 0;
    if (state.label === 1) {
      drinkN += 1;
...
function probeActions(
  ...
  for (let decisionIndex = 0; decisionIndex < 10; decisionIndex += 1) {
    ...
    actions.push(probeScore(model, feature) > 0 ? 1 : 0);
  }
  return actions;
}

function downstreamMetrics(
  tapes,
  variant,
  model,
  dataBySeed,
  ...
  const perSeed = V15K_D1_EVAL_BASE_SEEDS.map((baseSeed) => {
  ...
  return {
    ...
    meanExcessUses: survivors.length
      ...
      : Infinity,
    wastedHealingPerDrink: totalUses ? totalWasted / totalUses : 0,
    perSeed,
    rows: rows.map((row) => ({
      seed: row.seed,
      ...
      uses: row.uses,
      wasted: row.wasted,
      policyActions: row.policyActions,
    })),
  };
}

function economyCapable(classification, downstream) {
  return (
...
function makeEpisodeShiftMap(tapes) {
  const map = new Map();
  for (let i = 0; i < tapes.length; i += 1) {
    map.set(tapes[i].seed, tapes[(i + 1) % tapes.length].seed);
  }
  return map;
}

function alignmentContribution(full, shifted) {
  const balancedAccuracyDrop =
    full.classification.balancedAccuracy -
    shifted.classification.balancedAccuracy;
  const survivalDrop =
    full.downstream.survivalRate -
    shifted.downstream.survivalRate;
  const excessIncrease =
    Number.isFinite(shifted.downstream.meanExcessUses)
      ? shifted.downstream.meanExcessUses -
        full.downstream.meanExcessUses
      : Infinity;
  const zeroSurvivorControl =
    full.downstream.survivalRate > 0 &&
    shifted.downstream.survivalRate === 0;
  return {
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15K-D1 DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(V15K_D1_INTERRUPTION_SEED);
  const trainTapes = await collectD1Tapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15K_D1_TRAIN_BASE_SEEDS,
    interruptionRandom,
    label: "train",
  });
  const evalTapes = await collectD1Tapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15K_D1_EVAL_BASE_SEEDS,
    interruptionRandom,
    label: "eval",
  });

  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15K-D1 tape count mismatch");
  }
  validateRichFeatures(trainTapes);
  validateRichFeatures(evalTapes);
  ...
  const temporalMeans = computeTrainTemporalMeans(trainTapes);
  const trainData = makeResidualData(trainTapes, temporalMeans);
  const evalData = makeResidualData(evalTapes, temporalMeans);

  const pca = fitResidualPca(trainData.residualVectors);
  projectResidualData(trainData, pca);
  projectResidualData(evalData, pca);

  const allData = new Map([
    ...trainData.bySeed.entries(),
    ...evalData.bySeed.entries(),
  ]);
  ...
  const episodeShift = makeEpisodeShiftMap(evalTapes);
  ...
  for (const variant of variants) {
    ...
    const result = {
      ...
      modelSha256: hashJson(serializeModel(model)),
      classification,
      downstream,
      economyCapable: economyCapable(
        classification,
        downstream,
      ),
    };
    ...
    if (
      variant === "P2_RESIDUAL_MARGIN" ||
      variant === "P3_RESIDUAL_DN32"
    ) {
      const shiftedClassification = classificationMetrics(
        evalStates,
        variant,
        model,
        allData,
      ...
      const shiftedDownstream = downstreamMetrics(
        evalTapes,
        variant,
        model,
        allData,
      ...
      result.episodeShift = {
        classification: shiftedClassification,
        downstream: shiftedDownstream,
      };
      result.episodeSpecificContribution =
    ...
    console.log(
      "[v15K-D1] " +
        variant +
        " BA=" +
        (classification.balancedAccuracy * 100).toFixed(1) +
        "% WAIT=" +
        (classification.waitRecall * 100).toFixed(1) +
        "% DRINK=" +
        (classification.drinkRecall * 100).toFixed(1) +
        "% survival=" +
        (downstream.survivalRate * 100).toFixed(1) +
        "% excess=" +
        Number(downstream.meanExcessUses).toFixed(3) +
        " capable=" +
        result.economyCapable,
    );
    if (result.episodeSpecificContribution) {
      console.log(
        "[v15K-D1] " +
          variant +
          " shift BA-drop=" +
          (
            result.episodeSpecificContribution.balancedAccuracyDrop *
            100
          ).toFixed(1) +
          "pp survival-drop=" +
          (
            result.episodeSpecificContribution.survivalDrop *
            100
          ).toFixed(1) +
          "pp excess+=" +
          Number(
            result.episodeSpecificContribution.excessIncrease,
          ).toFixed(3) +
          " contributes=" +
          result.episodeSpecificContribution.contributes,
      );
    }
  }
  ...
  let outcome;
  if (!tapeValid) {
    outcome = "V15K_D1_IMPLEMENTATION_OR_ECOLOGY_INVALID";
  } else if (
    probes.P2_RESIDUAL_MARGIN.economyCapable &&
    probes.P2_RESIDUAL_MARGIN.episodeSpecificContribution
      .contributes
  ) {
    outcome = "V15K_D1_SCALAR_RESIDUAL_SIGNAL_PRESENT";
  } else if (
    probes.P3_RESIDUAL_DN32.economyCapable &&
    probes.P3_RESIDUAL_DN32.episodeSpecificContribution
      .contributes
  ) {
    outcome =
  ...
  const scalarMeansArray = Array.from(temporalMeans.scalar);
  const dnMeansArray = temporalMeans.dn.map((row) =>
    Array.from(row),
  );
  const pcaComponentsArray = pca.components.map((row) =>
    Array.from(row),
  );

  const output = {
    schema:
      "maplefly.v15k-d1.episode-specific-neural-residual-audit.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15k_d1.md",
      commit: V15K_D1_PREREG_COMMIT,
    },
    v15kClosure:
    ...
    cohorts: {
      trainBaseSeeds: V15K_D1_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15K_D1_EVAL_BASE_SEEDS,
      interruptionRngSeed: V15K_D1_INTERRUPTION_SEED,
      trainTapes: trainTapes.length,
      evalTapes: evalTapes.length,
    },
    reachableStates: {
      train: trainStates.length,
      eval: evalStates.length,
      strictTrain: trainStates.filter((row) => row.label !== null)
        .length,
      strictEval: evalStates.filter((row) => row.label !== null)
        .length,
    },
    preprocessing: {
      scalarTemporalMeans: scalarMeansArray,
      scalarTemporalMeansSha256: hashJson(scalarMeansArray),
      dnTemporalMeansSha256: hashJson(dnMeansArray),
      pca: {
        components: V15K_D1_PCA_COMPONENTS,
        iterations: V15K_D1_PCA_ITERATIONS,
        seedBase: V15K_D1_PCA_SEED,
        eigenvalues: pca.eigenvalues,
        componentsSha256: hashJson(pcaComponentsArray),
      },
      evalMeansUsed: false,
      labelsUsedForPca: false,
    },
    probeOptimizer: {
      type: "LINEAR_LOGISTIC",
      epochs: V15K_D1_PROBE_EPOCHS,
      learningRate: V15K_D1_PROBE_LR,
      l2: V15K_D1_PROBE_L2,
      clamp: [-V15K_D1_PROBE_CLAMP, V15K_D1_PROBE_CLAMP],
      equalClassWeight: true,
      earlyStopping: false,
    },
    tapeValidity: {
      metrics: validity,
  ...
  console.log("[v15K-D1] outcome=" + outcome);

  const outDir = resolve(
    "results/v15k-d1-episode-specific-neural-residual-audit",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15k_d1.json"),
