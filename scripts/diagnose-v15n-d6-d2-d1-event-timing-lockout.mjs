...
async function main() {
  ...
  const d2ArtifactPath = process.env.V15N_D6_D2_ARTIFACT_FILE;
  if (!d2ArtifactPath) {
    throw new Error("V15N_D6_D2_ARTIFACT_FILE is required");
  }
  const d2Bytes = await readFile(d2ArtifactPath);
  const d2Sha256 = createHash("sha256")
    .update(d2Bytes)
    .digest("hex");
  if (d2Sha256 !== V15N_D6_D2_D1_D2_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D1 authoritative D2 evidence SHA mismatch");
  }
  const frozenD2 = JSON.parse(d2Bytes.toString("utf8"));
  if (
    frozenD2.outcome !==
    "V15N_D6_D2_EVENTIZER_NOT_STABLE"
  ) {
    throw new Error("v15N-D6-D2-D1 prerequisite D2 outcome mismatch");
  }
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D1 v15N evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2-D1 DN identity/order mismatch");
  }
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D6-D2-D1 frozen preprocessing mismatch");
  }
  ...
  verifyAggregate(
    addEconomyMetrics(evaluatePolicy(evalTapes, params), evalOracle),
    artifact.evaluation.full,
    "FULL",
  );
  ...
  const detectorWeightsSha256 = createHash("sha256")
    ...
    .digest("hex");
  if (
    detectorWeightsSha256 !==
    frozenD2.detectorReproduction.weightsSha256
  ) {
    throw new Error("v15N-D6-D2-D1 detector weight SHA mismatch");
  }
  d1AssertClose(
    detector.threshold,
    frozenD2.detectorReproduction.threshold,
    "detector.threshold",
  );
  d1AssertClose(
    detector.separation,
    frozenD2.detectorReproduction.separation,
    "detector.separation",
  );
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    d2d1VerifyEventMetrics(
      eventMetrics[cohort],
      frozenD2.eventMetrics[cohort],
      "eventMetrics." + cohort,
    );
  }
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    d1VerifyMetricObject(
      directMetrics[cohort],
      frozenD2.directMetrics[cohort],
      "direct." + cohort,
    );
  }
  for (const cohort of [
    ...
    d1VerifyMetricObject(
      residualMetrics[cohort],
      frozenD2.residualMetrics[cohort],
      "residual." + cohort,
    );
  }

  const timing = {
    train: d2d1EventTiming(trainTapes),
    eval: d2d1EventTiming(evalTapes),
    holdout: d2d1EventTiming(holdoutTapes),
  };
  const misses = {
    train: d2d1MissedHitAttribution(
      trainTapes,
      phase.means,
      detector,
    ),
    eval: d2d1MissedHitAttribution(
      evalTapes,
      phase.means,
      detector,
    ),
    holdout: d2d1MissedHitAttribution(
      holdoutTapes,
      phase.means,
      detector,
    ),
  };
  const interHitTiming = {
    train: d2d1InterHitTiming(trainTapes),
    eval: d2d1InterHitTiming(evalTapes),
    holdout: d2d1InterHitTiming(holdoutTapes),
  };

  const supportPass =
    timing.eval.falseEvents >= V15N_D6_D2_D1_MIN_FALSE_EVENTS &&
    timing.holdout.falseEvents >=
      V15N_D6_D2_D1_MIN_FALSE_EVENTS &&
    misses.eval.missedHits >= V15N_D6_D2_D1_MIN_MISSED_HITS &&
    misses.holdout.missedHits >=
      V15N_D6_D2_D1_MIN_MISSED_HITS &&
    interHitTiming.eval.count >=
      V15N_D6_D2_D1_MIN_INTER_HIT_INTERVALS &&
    interHitTiming.holdout.count >=
      V15N_D6_D2_D1_MIN_INTER_HIT_INTERVALS;

  const cohortFlags = {
    eval: {
      lingerDominated:
        timing.eval.falseLingerFraction >=
        V15N_D6_D2_D1_ATTRIBUTION_GATE,
      suppressionDominated:
        misses.eval.suppressionFractionAmongMissed >=
        V15N_D6_D2_D1_ATTRIBUTION_GATE,
      backgroundDominated:
        timing.eval.falseBackgroundFraction >=
        V15N_D6_D2_D1_ATTRIBUTION_GATE,
    },
    holdout: {
      lingerDominated:
        timing.holdout.falseLingerFraction >=
        V15N_D6_D2_D1_ATTRIBUTION_GATE,
      suppressionDominated:
        misses.holdout.suppressionFractionAmongMissed >=
        V15N_D6_D2_D1_ATTRIBUTION_GATE,
      backgroundDominated:
        timing.holdout.falseBackgroundFraction >=
        V15N_D6_D2_D1_ATTRIBUTION_GATE,
    },
  };
  const globalFlags = {
    lingerDominated:
      cohortFlags.eval.lingerDominated &&
      cohortFlags.holdout.lingerDominated,
    suppressionDominated:
      cohortFlags.eval.suppressionDominated &&
      cohortFlags.holdout.suppressionDominated,
    backgroundDominated:
      cohortFlags.eval.backgroundDominated &&
      cohortFlags.holdout.backgroundDominated,
  };
  ...
  if (!supportPass) {
    ...
    globalFlags.lingerDominated &&
    globalFlags.suppressionDominated
  ) {
    outcome =
      "V15N_D6_D2_D1_LINGER_RETRIGGER_AND_LOCKOUT";
  } else if (globalFlags.lingerDominated) {
    outcome =
      "V15N_D6_D2_D1_LINGER_RETRIGGER_DOMINANT";
  } else if (globalFlags.suppressionDominated) {
    outcome =
      "V15N_D6_D2_D1_EVENTIZER_LOCKOUT_DOMINANT";
  } else if (globalFlags.backgroundDominated) {
    outcome =
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    console.log(
      "[v15N-D6-D2-D1] " +
        cohort +
        " falseLinger=" +
        timing[cohort].falseLingerFraction.toFixed(4) +
        " falseBackground=" +
        timing[cohort].falseBackgroundFraction.toFixed(4) +
        " rawDetectable=" +
        misses[cohort].rawDetectableHitFraction.toFixed(4) +
        " suppressionAmongMissed=" +
        misses[cohort].suppressionFractionAmongMissed.toFixed(4) +
        " interHitMedian=" +
        interHitTiming[cohort].medianSeconds.toFixed(3) +
        "s",
    );
  }
  console.log(
  ...
  const output = {
    schema: "maplefly.v15n-d6-d2-d1.event-timing-lockout.1",
    preregistration: {
      path: "history/prereg_v15n_d6_d2_d1.md",
      commit: V15N_D6_D2_D1_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD6D2Receipt:
      ...
      v15nD6D2Outcome:
        "V15N_D6_D2_EVENTIZER_NOT_STABLE",
      authoritativeD2EvidenceSha256: d2Sha256,
    },
    ...
    support: {
      ...
      minimumMissedHits:
        V15N_D6_D2_D1_MIN_MISSED_HITS,
      minimumInterHitIntervals:
        V15N_D6_D2_D1_MIN_INTER_HIT_INTERVALS,
      pass: supportPass,
    },
    cohortFlags,
    globalFlags,
    attributionGate: V15N_D6_D2_D1_ATTRIBUTION_GATE,
    provenance: {
    ...
    alternativeEventizerEvaluated: false,
  ...
  const outDir = resolve(
    "results/v15n-d6-d2-d1-event-timing-lockout",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d6_d2_d1.json"),
