...
const PRACTICE_BASE_SEEDS = [451000, 461000, 471000];
const PRACTICE_DISTANCES = [145, 185, 225, 265];
const FINAL_BASE_SEEDS = [501000, 511000, 521000];
const FINAL_DISTANCES = [155, 195, 235, 275];

const EPSILONS = [0.35, 0.20, 0.08];
...
function makePracticeSchedule(baseSeed) {
  ...
  if (obstacle.length !== 72 || noObstacle.length !== 24) {
...
async function runEpisode({
  ...
  while (totalBrainSteps < MAX_STEPS) {
    ...
    let reward = jumpActuated ? -0.02 : 0;
...
async function main() {
  const outDir = resolve("results/experiment-v11");
  ...
  const contract = buildDnContract(connectome.meta);
  const policy = new JumpPolicy(contract.allDn.length);
  const practice = [];

  for (let cohort = 0; cohort < 3; cohort += 1) {
    const result = await runPracticeCohort({
      connectome,
      dnSlot: contract.slot,
      policy,
      cohortIndex: cohort,
    });
    practice.push(result);
    console.log(
      "[v11-practice-summary] cohort=" +
        result.cohort +
        " clear=" +
        percent(result.obstacleClearRate) +
        " timeout=" +
        percent(result.obstacleTimeoutRate) +
        " noObstacleJump=" +
        percent(result.noObstacleAnyJumpRate),
    );
  }

  const perRun = [];

  for (let run = 0; run < FINAL_BASE_SEEDS.length; run += 1) {
    const baseSeed = FINAL_BASE_SEEDS[run];

    const full = await evaluateObstacleCondition({
      connectome,
      dnSlot: contract.slot,
      policy,
      baseSeed,
    ...
    const visualOff = await evaluateObstacleCondition({
      connectome,
      dnSlot: contract.slot,
      policy,
      baseSeed,
    ...
    const shuffled = await evaluateObstacleCondition({
      connectome,
      dnSlot: contract.slot,
      policy,
      baseSeed,
    ...
    const noObstacle = await evaluateNoObstacle({
      connectome,
      dnSlot: contract.slot,
      policy,
      baseSeed,
  ...
  const meanVisualOff = mean(
    perRun.map((run) => run.VISUAL_OFF.clearRate),
  );
  const meanShuffled = mean(
    perRun.map((run) => run.DN_SHUFFLED.clearRate),
  );
  const meanTimeout = mean(
    perRun.map((run) => run.FULL.timeoutRate),
  );
  const meanFullJumps = mean(
    perRun.map((run) => run.FULL.meanActualJumps),
  );
  const noObstacleAnyJump = mean(
    perRun.map((run) => run.NO_OBSTACLE.anyJumpRate),
  );
  const noObstacleReach = mean(
    perRun.map((run) => run.NO_OBSTACLE.targetReachRate),
  );
  ...
  const summary = {
    ...
    visualContribution: meanFull - meanVisualOff,
    meanDnShuffledClearRate: meanShuffled,
    dnIdentityContribution: meanFull - meanShuffled,
    minRunFullClearRate: minRunFull,
    meanFullTimeoutRate: meanTimeout,
    meanFullActualJumps: meanFullJumps,
    noObstacleAnyJumpRate: noObstacleAnyJump,
    noObstacleTargetReachRate: noObstacleReach,
    gate,
  };

  const output = {
    meta: {
      schema: "maplefly.experiment-v11.jump-sarsa.1",
      phase: "A",
      ...
      learner: "linear SARSA(0) WAIT/JUMP over frozen 1316-DN current feature",
      windows: {
        settle: SETTLE_STEPS,
      ...
      physics: {
        ...
        jumpCooldownSteps: JUMP_COOLDOWN_STEPS,
      ...
      antiLeak:
        "policy input is frozen MaleCNS DN activity only; no obstacle distance/coordinates/geometry/correct timing/target coordinate/cohort flag",
      gate:
        "FULL>=70%; FULL-VISUAL_OFF>=25pp; FULL-DN_SHUFFLED>=20pp; every FULL>=60%; timeout<=25%; mean jumps<=2; NO_OBSTACLE any-jump<=30%; NO_OBSTACLE target reach>=85%",
    },
    practice,
    summary,
    perRun,
  ...
  await writeFile(
    resolve(outDir, "experiment_v11.json"),
