...
function trainDeltaRewardModel({
  rows,
  selectedIndices,
  stats,
}) {
...
async function collectPractice({
  ...
  for (
    ...
    let validPairs = 0;
    let maxPairFeatureDifference = 0;
    ...
    for (
      ...
      if (
        ...
        if (
          ...
          throw new Error(
            "paired pre-intervention feature mismatch: " +
              featureDifference,
          );
        }

        maxPairFeatureDifference =
          Math.max(
            maxPairFeatureDifference,
            featureDifference,
          );
...
async function main() {
  ...
  const support = {
    ...
    maxFeatureDifference:
      Math.max(
        ...
        ...practice.rows.map(
          (row) =>
            row.featureDifference,
        ),
      ),
  };

  const supportGate =
    support.validPairs >= 270 &&
    support.positiveDelta >= 24 &&
    support.negativeDelta >= 24 &&
    support.maxFeatureDifference <=
      1e-9;
  ...
  console.log(
    "V11G5-SUPPORT=" +
      ...
      support.validPairs +
      " dR+=" +
      support.positiveDelta +
      " dR-=" +
      support.negativeDelta +
      " dR0=" +
      support.zeroDelta +
      " maxFeatureDiff=" +
      support.maxFeatureDifference,
  ...
  if (!supportGate) {
    const output = {
      meta: {
        schema:
          "maplefly.experiment-v11g5.paired-counterfactual.1",
        phase: "G5",
        brainRepository:
          SOURCE.repository,
        brainCommit:
    ...
    console.log(
      "V11G5-JUMP-GATE=FAIL reason=practice_support",
    );
    return;
  }
  ...
  const output = {
    meta: {
      schema:
        "maplefly.experiment-v11g5.reward-advantage.1",
      phase: "G5",
      brainRepository:
        SOURCE.repository,
      brainCommit:
      ...
      policy: {
        type:
          "paired counterfactual reward-difference ridge linear readout",
        target:
      ...
      finalDistances:
        FINAL_DISTANCES,
      antiLeak:
        "policy input contains only selected slots from 4 consecutive MaleCNS DN windows; paired rewards are training targets only; context/obstacle/target flags, distance, coordinates, collision state, correct timing, side, seed and reward are excluded",
    },
    practice: {
      cohorts:
        practice.cohorts,
