...
const TRAIN_BASE_SEEDS = [2860000, 2860100, 2860200, 2860300];
const EVAL_BASE_SEEDS = [2865000, 2865100, 2865200];
const REPLICATES = 6;
const EPOCHS = 240;
const LEARNING_RATE = 0.03;
const L2 = 0.001;
const THRESHOLD = 0.5;
const ORDER_SEED = 2866000;
const BOOTSTRAP_SEED = 2869000;
const BOOTSTRAP_ITERATIONS = 2000;
const MATERIAL_LOSS_MIN = 0.10;
const PREREG_COMMIT = "d3f8e17430378761df81e7bda27c2ca8c5d009dd";
...
const REPRESENTATIONS = [
  { name: "R0_FULL48", index: 0, startFrame: 0, frameCount: 48 },
  { name: "R1_EARLY24", index: 1, startFrame: 0, frameCount: 24 },
  { name: "R2_LATE24", index: 2, startFrame: 24, frameCount: 24 },
];
...
async function collectDataset(connectome, dnSlot, baseSeeds, label) {
  ...
  for (const baseSeed of baseSeeds) {
    ...
    console.log("[v15A9-" + label + "] baseSeed=" + baseSeed + " rows=" + rows.length);
...
function fitRepresentation(trainAll, representation) {
  ...
  const halfCounts = representation.name === "R0_FULL48"
    ? {
        early24: selectedSlots.filter((slot) => slot.absoluteFrame < HALF_FRAMES).length,
        late24: selectedSlots.filter((slot) => slot.absoluteFrame >= HALF_FRAMES).length,
      }
    : null;
...
function vector(row, fitted, startFrame, dnPermutation = null, maskHalf = null) {
  ...
  for (let i = 0; i < TOP_FEATURES; i += 1) {
    ...
    const masked =
      (maskHalf === "EARLY24" && absoluteFrame < HALF_FRAMES) ||
      (maskHalf === "LATE24" && absoluteFrame >= HALF_FRAMES);
    if (masked) {
      out[i] = 0;
      continue;
    }
...
async function main() {
  const outDir = resolve("results/screen-v15a9-half-history-necessity");
  ...
  for (const representation of REPRESENTATIONS) {
    ...
    console.log(
      "[v15A9] " + representation.name + " sha256=" + fitted.sha256 +
    ...
    for (const task of TASKS) {
      ...
      console.log(
        "[v15A9] " + representation.name + " " + task.name +
  ...
  const r0Fitted = fittedByName.get("R0_FULL48");
  const maskResults = [];
  for (const maskHalf of ["EARLY24", "LATE24"]) {
  ...
  const earlyMask = maskResults.find((row) => row.maskHalf === "EARLY24");
  const lateMask = maskResults.find((row) => row.maskHalf === "LATE24");
  ...
  if (!baselineReplicated) {
    ...
    if (r1TaskA.gatePass) findings.push("EARLY24_SUFFICIENT");
    if (r2TaskA.gatePass) findings.push("LATE24_SUFFICIENT");
    if (!r1TaskA.gatePass && !r2TaskA.gatePass) findings.push("NO_SINGLE_HALF_SUFFICIENT");
    if (earlyMask.materialDecoderContribution) findings.push("EARLY24_DECODER_CONTRIBUTES");
    if (lateMask.materialDecoderContribution) findings.push("LATE24_DECODER_CONTRIBUTES");
    if (earlyMask.materialDecoderContribution && lateMask.materialDecoderContribution) {
      findings.push("BOTH_HALVES_DECODER_CONTRIBUTE");
    }
    if (
      !r1TaskA.gatePass &&
      !r2TaskA.gatePass &&
      earlyMask.materialDecoderContribution &&
      lateMask.materialDecoderContribution
    ) {
      findings.push("CROSS_HALF_DISTRIBUTED_CODE_SUPPORTED");
      outcome = "CROSS_HALF_DISTRIBUTED_CODE_SUPPORTED";
    } else if (r1TaskA.gatePass || r2TaskA.gatePass) {
      outcome = "SINGLE_HALF_SUFFICIENCY_PRESENT";
    } else {
      outcome = "HALF_NECESSITY_INCONCLUSIVE";
    }
  }

  console.log(
    "[v15A9] outcome=" + outcome +
    " findings=" + (findings.join(",") || "none") +
    " v15B=BLOCKED",
  );

  const output = {
    schema: "maplefly.v15a9.half-history-necessity.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15a9.md",
  ...
  await writeFile(
    resolve(outDir, "v15a9_screen.json"),
