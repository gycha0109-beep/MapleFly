...
function decisionBoundary({
  ...
  if (state.potionSteps !== POTION_FRAME_STEPS) {
    throw new Error("v15F POTION frame alignment mismatch");
  }
  ...
  if (state.potionFrameCount === POTION_HISTORY_FRAMES) {
    const potionDecision = remediationPotionDecision(state);
    ...
    state.potionEvents.push({
      ...
      qMargin: potionDecision.qDrink - potionDecision.qWait,
      hpBefore,
      hpAfter: state.hp,
      healed,
      wastedHealing: wasted,
      precedingWindowContacts: state.damageEvents.filter(
        (event) =>
          event.step >= brainStep - 239 &&
          event.step < brainStep,
      ).length,
    });
...
async function main() {
  ...
  console.log(
    "[v15F] v15E2 representation=" +
      remediationPotion.representationSha256 +
      " model=" +
      remediationPotion.modelSha256,
  );
  ...
  if (
    ...
    throw new Error("v15E2 DN identity/order mismatch");
  }
  ...
  console.log(
    "[v15F] outcome=" +
      ...
      " next=" +
      (scientificPass
        ? "FRESH_SEED_48S_REMEDIATION_PREREGISTRATION_AUTHORIZED"
        : "FREEZE_AND_DIAGNOSE"),
  );

  const output = {
    schema: "maplefly.v15f.known-ecology-remediation.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15f.md",
      commit: V15F_PREREG_COMMIT,
    },
    frozenSkills: {
      ...
      potion: {
        deployedBaselineStatus: potionSkill.status,
        deployedBaselineRepresentationSha256:
          potionSkill.representationSha256,
        deployedBaselinePolicySha256: potionSkill.policySha256,
        candidate: "V15E2_PHASE_INVARIANT_POOLED_DN",
  ...
  const outDir = resolve(
    "results/v15f-known-ecology-remediation",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15f_ecology.json"),
