...
async function main() {
  ...
  const output = {
    schema: "maplefly.v13.lower-level-baseline-diagnostic.1",
    purpose:
      "Diagnostic replay of frozen lower-level stack on v13 final seeds; no new gate.",
  ...
  const outDir = resolve("results/diagnose-v13-lower-level-baseline");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "diagnostic_v13_lower_level.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  console.log(
    "V13-LOWER-LEVEL-BASELINE=" +
      (pass ? "PASS" : "FAIL") +
      " complete=" +
