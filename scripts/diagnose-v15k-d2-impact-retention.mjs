#!/usr/bin/env node
// v15K-D2 diagnostic: impact-locked 100 ms DN temporal retention audit.
...
const V15K_D2_PREREG_COMMIT =
  "cf2b221c9002dbf3582f452d3b689415aef048ab";
const V15K_D2_TRAIN_BASE_SEEDS = [3681000, 3691000, 3701000];
const V15K_D2_EVAL_BASE_SEEDS = [3711000, 3721000, 3731000];
const V15K_D2_INTERRUPTION_SEED = 3747000;
const V15K_D2_MIN_SUPPORT = 100;
...
function decisionBoundary({
  ...
  for (let dn = 0; dn < DN_COUNT; dn += 1) {
    ...
    normalizedFrame[dn] = value;
    state.potionPooledSum[dn] += value;
  }
...
async function collectD2FrameTapes({
  connectome,
  dnSlot,
  attackPolicy,
  jumpPolicy,
...
const D2_BANDS = Object.freeze([
  { name: "B0_DIRECT", minSteps: 0, maxSteps: 10, gate: 0.70 },
  { name: "B1_EARLY_POST", minSteps: 10, maxSteps: 25, gate: 0.65 },
  { name: "B2_MID_POST", minSteps: 25, maxSteps: 50, gate: 0.65 },
  { name: "B3_LATE_POST", minSteps: 50, maxSteps: 100, gate: 0.65 },
]);

function validateFrameTapes(tapes) {
  for (const tape of tapes) {
    if (tape.potionFrameEvents.length !== 480) {
      throw new Error(
        "v15K-D2 frame count mismatch seed=" +
          tape.seed +
          " count=" +
          tape.potionFrameEvents.length,
      );
    }
    tape.potionFrameEvents.forEach((event, index) => {
      if (
        event.frameIndex !== index ||
        event.step !== (index + 1) * POTION_FRAME_STEPS ||
        !(event.values instanceof Float32Array) ||
        event.values.length !== DN_COUNT
      ) {
        throw new Error("v15K-D2 frame contract mismatch");
      }
    });
  }
}

function computePhaseMeans(tapes) {
  ...
  for (let phase = 0; phase < POTION_HISTORY_FRAMES; phase += 1) {
    if (counts[phase] !== tapes.length * 10) {
      throw new Error(
        "v15K-D2 phase support mismatch phase=" +
          phase +
          " count=" +
          counts[phase],
      );
...
function residualizeFrames(tapes, phaseMeans) {
  const bySeed = new Map();
  for (const tape of tapes) {
    const frames = tape.potionFrameEvents.map((frame) => {
      const phase = frame.frameIndex % POTION_HISTORY_FRAMES;
      const residual = new Float32Array(DN_COUNT);
      for (let d = 0; d < DN_COUNT; d += 1) {
        residual[d] = frame.values[d] - phaseMeans[phase][d];
      }
      return {
        step: frame.step,
    ...
    bySeed.set(tape.seed, frames);
  }
  return bySeed;
}

function mostRecentContactAgeSteps(tape, step) {
  let latest = null;
  for (const event of tape.damageEvents) {
...
function labelForBand(ageSteps, band) {
  if (
    ageSteps >= band.minSteps &&
    ageSteps < band.maxSteps
  ) {
    return 1;
  }
  if (!Number.isFinite(ageSteps) || ageSteps >= 100) {
    return 0;
  }
  return null;
}

function collectBandExamples(tapes, residualBySeed, band) {
  const examples = [];
  for (const tape of tapes) {
    const frames = residualBySeed.get(tape.seed);
    for (const frame of frames) {
      const ageSteps = mostRecentContactAgeSteps(
        tape,
        frame.step,
      );
      const label = labelForBand(ageSteps, band);
...
function meanVector(examples, label) {
...
function trainMeanDifferenceProbe(examples, band) {
  const positives = examples.filter((row) => row.y === 1).length;
  const negatives = examples.length - positives;
  const supportValid =
    positives >= V15K_D2_MIN_SUPPORT &&
    negatives >= V15K_D2_MIN_SUPPORT;
  if (!supportValid) {
    return {
      band: band.name,
      supportValid: false,
  ...
  const positiveMean = meanVector(examples, 1);
  const negativeMean = meanVector(examples, 0);
  ...
  if (!(norm > 1e-12)) {
    throw new Error("v15K-D2 mean-difference vector collapsed");
  }
  for (let d = 0; d < DN_COUNT; d += 1) {
    weights[d] /= norm;
  }
  const positiveProjected = dotVector(weights, positiveMean);
  const negativeProjected = dotVector(weights, negativeMean);
  const threshold =
    (positiveProjected + negativeProjected) / 2;
  ...
  return {
    ...
    weights,
    threshold,
    trainProjectedPositiveMean: positiveProjected,
    trainProjectedNegativeMean: negativeProjected,
  };
}

function metricsForExamples(examples, probe, residualOverride = null) {
  if (!probe.supportValid) {
    return {
      supportValid: false,
      positives: examples.filter((row) => row.y === 1).length,
      negatives: examples.filter((row) => row.y === 0).length,
    };
  }
  ...
  for (const row of examples) {
    const x = residualOverride
      ? residualOverride(row)
      : row.x;
    const score = dotVector(probe.weights, x);
    const predicted = score > probe.threshold ? 1 : 0;
    if (row.y === 1) {
      positiveN += 1;
      positiveScore += score;
  ...
  const positiveRecall =
    positiveN ? positiveCorrect / positiveN : 0;
  const negativeRecall =
    negativeN ? negativeCorrect / negativeN : 0;
  return {
    supportValid:
      positiveN >= V15K_D2_MIN_SUPPORT &&
      negativeN >= V15K_D2_MIN_SUPPORT,
    positives: positiveN,
    negatives: negativeN,
    positiveRecall,
    negativeRecall,
    ...
    positiveMeanScore:
      positiveN ? positiveScore / positiveN : 0,
    negativeMeanScore:
      negativeN ? negativeScore / negativeN : 0,
    scoreSeparation:
      (positiveN ? positiveScore / positiveN : 0) -
      (negativeN ? negativeScore / negativeN : 0),
  };
...
function shiftedResidualResolver(
  evalResidualBySeed,
  episodeShift,
) {
  return (row) => {
    const donorSeed = episodeShift.get(row.seed);
    const donor = evalResidualBySeed.get(donorSeed);
    return donor[row.frameIndex].residual;
  };
}

function bandPass(metrics, band) {
  return (
    metrics.supportValid &&
    metrics.balancedAccuracy >= band.gate &&
    metrics.positiveRecall >= 0.60 &&
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15K-D2 DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(V15K_D2_INTERRUPTION_SEED);
  const trainTapes = await collectD2FrameTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15K_D2_TRAIN_BASE_SEEDS,
    interruptionRandom,
    label: "train",
  });
  const evalTapes = await collectD2FrameTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15K_D2_EVAL_BASE_SEEDS,
    interruptionRandom,
    label: "eval",
  });
  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15K-D2 tape count mismatch");
  }
  validateFrameTapes(trainTapes);
  validateFrameTapes(evalTapes);
  ...
  const phaseMeans = computePhaseMeans(trainTapes);
  const trainResidual = residualizeFrames(trainTapes, phaseMeans);
  const evalResidual = residualizeFrames(evalTapes, phaseMeans);
  const episodeShift = makeEpisodeShiftMap(evalTapes);
  const shiftedResolver = shiftedResidualResolver(
    evalResidual,
    episodeShift,
  );

  const bands = {};
  for (const band of D2_BANDS) {
    const trainExamples = collectBandExamples(
      trainTapes,
      trainResidual,
      band,
    );
    const evalExamples = collectBandExamples(
      evalTapes,
      evalResidual,
      band,
    );
    const probe = trainMeanDifferenceProbe(
      trainExamples,
      band,
    );
    const full = metricsForExamples(
      evalExamples,
      probe,
    );
    const shifted = metricsForExamples(
      evalExamples,
      probe,
      shiftedResolver,
    );
    const fullPass = bandPass(full, band);
    const alignmentDrop =
      full.supportValid && shifted.supportValid
        ? full.balancedAccuracy - shifted.balancedAccuracy
        : -Infinity;
    const episodeSpecificAlignment =
      fullPass && alignmentDrop >= 0.10;

    const serialProbe = probe.supportValid
      ? {
          band: probe.band,
          positives: probe.positives,
          negatives: probe.negatives,
          threshold: probe.threshold,
          trainProjectedPositiveMean:
            probe.trainProjectedPositiveMean,
          trainProjectedNegativeMean:
            probe.trainProjectedNegativeMean,
          weightsSha256: hashJson(Array.from(probe.weights)),
        }
      : probe;

    bands[band.name] = {
      definition: {
        minSteps: band.minSteps,
        maxSteps: band.maxSteps,
        minMs: band.minSteps * STEP_SECONDS * 1000,
        maxMs: band.maxSteps * STEP_SECONDS * 1000,
        balancedAccuracyGate: band.gate,
      },
      trainSupport: {
        positives: trainExamples.filter((row) => row.y === 1).length,
        negatives: trainExamples.filter((row) => row.y === 0).length,
      },
      probe: serialProbe,
      full,
      shifted,
    ...
    console.log(
      "[v15K-D2] " +
        ...
        full.negatives +
        " BA=" +
        (Number(full.balancedAccuracy || 0) * 100).toFixed(1) +
        "% pos=" +
        (Number(full.positiveRecall || 0) * 100).toFixed(1) +
        "% neg=" +
        (Number(full.negativeRecall || 0) * 100).toFixed(1) +
        "% shiftBA=" +
        (Number(shifted.balancedAccuracy || 0) * 100).toFixed(1) +
        "% drop=" +
        (Number.isFinite(alignmentDrop)
          ? (alignmentDrop * 100).toFixed(1)
          : "NA") +
        "pp aligned=" +
        episodeSpecificAlignment,
    );
  ...
  const b0 = bands.B0_DIRECT;
  const mandatorySupport =
    b0.full.supportValid &&
    b0.trainSupport.positives >= V15K_D2_MIN_SUPPORT &&
    b0.trainSupport.negatives >= V15K_D2_MIN_SUPPORT;

  let outcome;
  if (!tapeValid || !mandatorySupport) {
    outcome = "V15K_D2_IMPLEMENTATION_OR_SUPPORT_INVALID";
  } else if (
    b0.episodeSpecificAlignment &&
    [
      bands.B1_EARLY_POST,
      bands.B2_MID_POST,
      bands.B3_LATE_POST,
    ].some((row) => row.episodeSpecificAlignment)
  ) {
    outcome = "V15K_D2_PERSISTENT_IMPACT_SIGNAL_PRESENT";
  } else if (b0.episodeSpecificAlignment) {
    outcome = "V15K_D2_TRANSIENT_IMPACT_SIGNAL_PRESENT";
  } else {
    outcome = "V15K_D2_IMPACT_SIGNAL_NOT_DEMONSTRATED";
  }

  const phaseMeanHash = hashJson(
    phaseMeans.map((row) => Array.from(row)),
  );
  const output = {
    schema:
      "maplefly.v15k-d2.impact-locked-dn-temporal-retention.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15k_d2.md",
      commit: V15K_D2_PREREG_COMMIT,
    },
    d1Closure:
    ...
    cohorts: {
      trainBaseSeeds: V15K_D2_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15K_D2_EVAL_BASE_SEEDS,
      interruptionRngSeed: V15K_D2_INTERRUPTION_SEED,
    ...
    preprocessing: {
      type: "TRAIN_ONLY_PHASE_48_RESIDUAL",
      phaseMeanSha256: phaseMeanHash,
      evalMeansUsed: false,
    },
    tapeValidity: {
      metrics: validity,
  ...
  console.log("[v15K-D2] outcome=" + outcome);

  const outDir = resolve(
    "results/v15k-d2-impact-locked-dn-retention",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15k_d2.json"),
