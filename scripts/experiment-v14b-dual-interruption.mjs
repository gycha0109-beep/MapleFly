...
const FINAL_SEEDS = [2651000, 2661000, 2671000];
...
function makeGeometry(side, startDistance) {
  ...
  if (Math.abs(matchedDistance - expectedDistance) > 1e-9) {
    throw new Error("v14B matched geometry mismatch");
  }
...
function decisionBoundary({
  ...
  if (
    ...
    throw new Error("v14B decision window alignment mismatch");
  }
...
async function main() {
  ...
  const attackPolicy = interruptionApi.createPolicy();
  const jumpPolicy = interruptionApi.createPolicy();
  ...
  for (let index = 0; index < orderedPractice.length; index += 1) {
    ...
    if ((index + 1) % 16 === 0) {
      const recent = aggregate(practiceRows.slice(-16));
      console.log(
        "[v14B-practice] " +
          (index + 1) +
          "/" +
          ...
          "% jumps=" +
          recent.meanActualJumps.toFixed(3) +
          " postJump=" +
          (recent.postClearJumpEpisodeRate * 100).toFixed(1) +
          "% airAtk=" +
  ...
  const frozenPolicies = {
    attack: {
      bias: attackPolicy.bias,
      weights: Array.from(attackPolicy.weights),
    },
    jump: {
      bias: jumpPolicy.bias,
      weights: Array.from(jumpPolicy.weights),
    },
  };
  ...
  const output = {
    schema: "maplefly.v14b.dual-interruption.1",
    brainCommit: SOURCE.commit,
    preregistration:
      "history/prereg_v14b.md@d208c9ef3ab2c674aad8ed17db38b850f38ee4f3",
    lowerLevelSkills: {
  ...
  const outDir = resolve(
    "results/experiment-v14b-dual-interruption",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "experiment_v14b.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  console.log(
    "V14B-DUAL-INTERRUPTION=" +
      (pass ? "PASS" : "FAIL") +
      " complete=" +
      ...
      " jump=" +
      summary.acceptedJumpProposals +
      "/" +
      summary.jumpProposals,
  );
