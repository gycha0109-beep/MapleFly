...
const V15M_D3_BASE_SEEDS = [4041000, 4051000, 4061000];
const V15M_D3_INTERRUPTION_SEED = 4077000;
const V15M_D3_PREREG_COMMIT =
  "412f08a5994bb6d6a0d7baca54a2096767b25383";
const V15M_D3_SHORT_DECAY = 0.50;
const V15M_D3_LONG_DECAY = 0.90;
const V15M_D3_MIN_FORCED_SUPPORT = 500;
const V15M_D3_MIN_MIXED_GROUPS = 20;
...
function actionPoint(priorActions) {
  return {
    short: ownActionScalar(
      priorActions,
      V15M_D3_SHORT_DECAY,
    ),
    long: ownActionScalar(
      priorActions,
      V15M_D3_LONG_DECAY,
    ),
  };
...
function pointKey(point) {
  return point.short.toPrecision(17) + ":" +
    point.long.toPrecision(17);
}

function cross(o, a, b) {
  return (
    (a.short - o.short) * (b.long - o.long) -
    (a.long - o.long) * (b.short - o.short)
  );
}
...
function convexHull(points) {
  ...
  const sorted = [...unique.values()].sort(
    (a, b) => a.short - b.short || a.long - b.long,
  );
...
function onSegment(a, b, p) {
  return (
    ...
    p.short >= Math.min(a.short, b.short) - 1e-12 &&
    p.short <= Math.max(a.short, b.short) + 1e-12 &&
    p.long >= Math.min(a.long, b.long) - 1e-12 &&
    p.long <= Math.max(a.long, b.long) + 1e-12
  );
}
...
function pointInConvexHull(point, hull) {
  ...
  if (hull.length === 1) {
    return (
      Math.abs(point.short - hull[0].short) <= 1e-12 &&
      Math.abs(point.long - hull[0].long) <= 1e-12
    );
  }
  if (hull.length === 2) {
    return onSegment(hull[0], hull[1], point);
  }
...
function hullsIntersect(a, b) {
  ...
  for (const [a0, a1] of hullEdges(a)) {
    for (const [b0, b1] of hullEdges(b)) {
      if (segmentsIntersect(a0, a1, b0, b1)) return true;
    }
  }

  if (pointInConvexHull(a[0], b)) return true;
  if (pointInConvexHull(b[0], a)) return true;
  return false;
}

function geometryAudit(tapeResults) {
  const groups = new Map();
  const counts = {
    FORCED_WAIT: 0,
    FORCED_DRINK: 0,
  ...
  for (const tape of tapeResults) {
    for (const state of tape.states) {
      ...
      const point = actionPoint(state.priorActions);
      const source = {
        ...point,
        historyMask: state.historyMask,
        priorActions: state.priorActions,
      ...
      if (label === 0) groups.get(key).wait.push(source);
      else groups.get(key).drink.push(source);
    }
  }

  const mixed = [];
  const nonSeparable = [];
  const nonSeparableByDecision = Array(10).fill(0);

  for (const group of groups.values()) {
    if (!group.wait.length || !group.drink.length) continue;
    const waitHull = convexHull(group.wait);
    const drinkHull = convexHull(group.drink);
    const intersects = hullsIntersect(waitHull, drinkHull);
    const row = {
      seed: group.seed,
      baseSeed: group.baseSeed,
      decisionIndex: group.decisionIndex,
      waitStates: group.wait.length,
      drinkStates: group.drink.length,
      waitDistinctPoints: new Set(group.wait.map(pointKey)).size,
      drinkDistinctPoints: new Set(group.drink.map(pointKey)).size,
      waitHull,
      drinkHull,
      nonSeparable: intersects,
    };
    mixed.push(row);
    if (intersects) {
      nonSeparable.push(row);
      nonSeparableByDecision[group.decisionIndex] += 1;
    }
  }
  ...
  return {
    ...
    forcedWait: counts.FORCED_WAIT,
    forcedDrink: counts.FORCED_DRINK,
    forcedTotal: counts.FORCED_WAIT + counts.FORCED_DRINK,
    mixedGroupCount: mixed.length,
    nonSeparableMixedGroupCount: nonSeparable.length,
    nonSeparableMixedGroupFraction:
      mixed.length ? nonSeparable.length / mixed.length : 0,
    nonSeparableByDecision,
    witnesses: nonSeparable
      .sort(
        (a, b) =>
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15M-D3 v14C provenance mismatch");
  }
  const attackPolicy = {
  ...
  if (!actionIndependence.pass) {
    throw new Error(
      "v15M-D3 counterfactual action leaks into neural tape: " +
  ...
  if (
    ...
    throw new Error("v15M-D3 DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(V15M_D3_INTERRUPTION_SEED);
  const tapes = await collectTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15M_D3_BASE_SEEDS,
    interruptionRandom,
    label: "v15m-d3-holdout",
  });
  if (tapes.length !== 24) {
    throw new Error("v15M-D3 tape count mismatch");
  }
  ...
  const audit = geometryAudit(tapeResults);
  const supportPass =
    audit.forcedWait >= V15M_D3_MIN_FORCED_SUPPORT &&
    audit.forcedDrink >= V15M_D3_MIN_FORCED_SUPPORT &&
    audit.mixedGroupCount >= V15M_D3_MIN_MIXED_GROUPS;

  let outcome;
  if (!ecologyPass) {
    outcome = "V15M_D3_IMPLEMENTATION_OR_ECOLOGY_INVALID";
  } else if (!supportPass) {
    outcome = "V15M_D3_INSUFFICIENT_ORACLE_SUPPORT";
  } else if (audit.nonSeparableMixedGroupFraction >= 0.10) {
    outcome = "V15M_D3_TWO_TIMESCALE_STATE_BOTTLENECK";
  } else if (audit.nonSeparableMixedGroupFraction < 0.02) {
    outcome = "V15M_D3_TWO_TIMESCALE_STATE_NOT_RULED_OUT";
  } else {
    outcome = "V15M_D3_TWO_TIMESCALE_STATE_INCONCLUSIVE";
  }

  console.log(
    "[v15M-D3] reachable=" + audit.reachableStates +
      " forcedWait=" + audit.forcedWait +
      " forcedDrink=" + audit.forcedDrink +
      " mixedGroups=" + audit.mixedGroupCount,
  );
  console.log(
    "[v15M-D3] nonseparable=" +
      audit.nonSeparableMixedGroupCount + "/" +
      audit.mixedGroupCount +
      " (" +
      (audit.nonSeparableMixedGroupFraction * 100).toFixed(2) +
      "%) byDecision=" +
      JSON.stringify(audit.nonSeparableByDecision) +
      " outcome=" + outcome,
  );

  const output = {
    schema: "maplefly.v15m-d3.two-timescale-action-state.1",
    preregistration: {
      path: "history/prereg_v15m_d3.md",
      commit: V15M_D3_PREREG_COMMIT,
    },
    evidenceChain: {
      v15mD2Receipt:
    ...
    cohort: {
      baseSeeds: V15M_D3_BASE_SEEDS,
      tapes: tapes.length,
      interruptionRngSeed: V15M_D3_INTERRUPTION_SEED,
    },
    actionIndependence,
    ecology: {
      metrics: validity,
    ...
    state: {
      dimensions: 2,
      shortDecay: V15M_D3_SHORT_DECAY,
      longDecay: V15M_D3_LONG_DECAY,
      runtimeInputs: ["previous_own_potion_action"],
      forbiddenEnvironmentStateLoaded: false,
    },
    audit,
    supportPass,
    outcome,
  ...
  const outDir = resolve(
    "results/v15m-d3-two-timescale-action-state",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15m_d3.json"),
