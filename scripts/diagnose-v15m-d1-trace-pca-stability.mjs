...
const V15M_D1_HOLDOUT_BASE_SEEDS = [3961000, 3971000, 3981000];
const V15M_D1_PREREG_COMMIT =
  "41be09846362c0e4162468b6bbaa183144555f92";
const V15M_D1_HOLDOUT_INTERRUPTION_SEED = 3997000;
const V15M_D1_RECENT_IMPACT_SECONDS = 2.0;
const V15M_D1_MIN_CLASS_SUPPORT = 30;
...
function standardizedTrace(trace, preprocessing) {
  const out = new Float64Array(DN_COUNT);
  for (let d = 0; d < DN_COUNT; d += 1) {
    out[d] =
      (trace[d] - preprocessing.means[d]) /
      preprocessing.scales[d];
  }
  return out;
}

function recentImpactLabel(tape, decisionIndex) {
  const decisionStep = tape.potionEvents[decisionIndex].step;
  const windowSteps = Math.round(
    V15M_D1_RECENT_IMPACT_SECONDS / STEP_SECONDS,
  );
  return Number(
    tape.damageEvents.some(
      (event) =>
        event.step >= decisionStep - windowSteps &&
        event.step < decisionStep,
    ),
  );
}

function representationSequences(tapes, preprocessing, kind) {
  return tapes.map((tape) => {
    if (tape.potionEvents.length !== 10) {
      throw new Error("v15M-D1 decision count mismatch");
    }
    return tape.potionEvents.map((event, decisionIndex) => ({
      seed: tape.seed,
      baseSeed: tape.baseSeed,
      decisionIndex,
      label: recentImpactLabel(tape, decisionIndex),
      vector:
        kind === "raw"
          ? Array.from(standardizedTrace(event.trace, preprocessing))
          : Array.from(tape.neuralFeatures[decisionIndex]),
    }));
  });
}

function flattenSequences(sequences) {
  return sequences.flatMap((sequence) => sequence);
}

function fitCentroidProbe(rows) {
  const positive = rows.filter((row) => row.label === 1);
  const negative = rows.filter((row) => row.label === 0);
  if (
    positive.length < V15M_D1_MIN_CLASS_SUPPORT ||
    negative.length < V15M_D1_MIN_CLASS_SUPPORT
  ) {
    return {
      valid: false,
      positiveSupport: positive.length,
      negativeSupport: negative.length,
    };
  }

  const width = rows[0].vector.length;
  const positiveMean = new Float64Array(width);
  const negativeMean = new Float64Array(width);

  for (const row of positive) {
    for (let i = 0; i < width; i += 1) {
      positiveMean[i] += row.vector[i];
    }
  }
  for (const row of negative) {
    for (let i = 0; i < width; i += 1) {
      negativeMean[i] += row.vector[i];
    }
  }
  for (let i = 0; i < width; i += 1) {
    positiveMean[i] /= positive.length;
    negativeMean[i] /= negative.length;
  }

  const direction = new Float64Array(width);
  for (let i = 0; i < width; i += 1) {
    direction[i] = positiveMean[i] - negativeMean[i];
  }
  const positiveProjection = vectorDot(positiveMean, direction);
  const negativeProjection = vectorDot(negativeMean, direction);
  ...
  return {
    ...
    negativeSupport: negative.length,
    direction,
    threshold: 0.5 * (positiveProjection + negativeProjection),
    positiveProjection,
    negativeProjection,
  };
}

function evaluateProbe(rows, probe) {
  let tp = 0;
  let fn = 0;
  let tn = 0;
  let fp = 0;
  ...
  for (const row of rows) {
    const prediction =
      vectorDot(row.vector, probe.direction) >= probe.threshold ? 1 : 0;
    if (row.label === 1 && prediction === 1) tp += 1;
    if (row.label === 1 && prediction === 0) fn += 1;
    if (row.label === 0 && prediction === 0) tn += 1;
    if (row.label === 0 && prediction === 1) fp += 1;
  }

  const positiveSupport = tp + fn;
  const negativeSupport = tn + fp;
  const positiveRecall = positiveSupport ? tp / positiveSupport : 0;
  const negativeRecall = negativeSupport ? tn / negativeSupport : 0;
...
function shiftedRows(targetSequences, sourceSequences) {
  if (targetSequences.length !== sourceSequences.length) {
    throw new Error("v15M-D1 shift cohort size mismatch");
  }
  const rows = [];
  for (let episode = 0; episode < targetSequences.length; episode += 1) {
    const target = targetSequences[episode];
    const source =
      sourceSequences[(episode + 1) % sourceSequences.length];
    if (target.length !== 10 || source.length !== 10) {
      throw new Error("v15M-D1 shift sequence length mismatch");
    }
    for (let decision = 0; decision < 10; decision += 1) {
      rows.push({
        seed: target[decision].seed,
        baseSeed: target[decision].baseSeed,
        decisionIndex: decision,
        label: target[decision].label,
        vector: source[decision].vector,
      });
    }
  }
  return rows;
...
function cohortSupport(rows) {
  return {
    positive: rows.filter((row) => row.label === 1).length,
    negative: rows.filter((row) => row.label === 0).length,
  };
}
...
function representationAudit(
  trainSequences,
  evalSequences,
  holdoutSequences,
) {
  const trainRows = flattenSequences(trainSequences);
  const evalRows = flattenSequences(evalSequences);
  const holdoutRows = flattenSequences(holdoutSequences);
  const probe = fitCentroidProbe(trainRows);

  if (!probe.valid) {
    return {
      probe: {
        valid: false,
        positiveSupport: probe.positiveSupport,
        negativeSupport: probe.negativeSupport,
      },
      trainSupport: cohortSupport(trainRows),
      evalSupport: cohortSupport(evalRows),
      holdoutSupport: cohortSupport(holdoutRows),
    };
  }

  const train = evaluateProbe(trainRows, probe);
  const evalAligned = evaluateProbe(evalRows, probe);
  const holdoutAligned = evaluateProbe(holdoutRows, probe);
  const evalShifted = evaluateProbe(
    shiftedRows(evalSequences, evalSequences),
    probe,
  );
  const holdoutShifted = evaluateProbe(
    shiftedRows(holdoutSequences, holdoutSequences),
    probe,
  );
  ...
  return {
    probe: {
      ...
      positiveProjection: probe.positiveProjection,
      negativeProjection: probe.negativeProjection,
      directionSha256: createHash("sha256")
        .update(JSON.stringify(Array.from(probe.direction)))
        .digest("hex"),
    },
    train,
    eval: {
      aligned: evalAligned,
      shifted: evalShifted,
      alignedMinusShiftedBa:
        evalAligned.balancedAccuracy - evalShifted.balancedAccuracy,
      trainToAlignedBaDrop:
        train.balancedAccuracy - evalAligned.balancedAccuracy,
    },
    holdout: {
      aligned: holdoutAligned,
      shifted: holdoutShifted,
      alignedMinusShiftedBa:
        holdoutAligned.balancedAccuracy -
        holdoutShifted.balancedAccuracy,
      trainToAlignedBaDrop:
        train.balancedAccuracy - holdoutAligned.balancedAccuracy,
    },
  };
}
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15M-D1 v14C provenance mismatch");
  }
  const attackPolicy = {
  ...
  if (
    ...
    throw new Error("v15M-D1 DN identity/order mismatch");
  }
  ...
  const frozenRandom = mulberry32(3937000);
  const trainTapes = await collectTapes({
    ...
    interruptionRandom: frozenRandom,
    label: "d1-train",
  });
  ...
  const holdoutRandom = mulberry32(
    V15M_D1_HOLDOUT_INTERRUPTION_SEED,
  );
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15M_D1_HOLDOUT_BASE_SEEDS,
    interruptionRandom: holdoutRandom,
    label: "d1-holdout",
  });
  ...
  if (
    ...
    throw new Error("v15M-D1 tape count mismatch");
  }

  const evalValidity = tapeValidity(evalTapes);
  const holdoutValidity = tapeValidity(holdoutTapes);
  const evalChecks = ecologyChecks(evalValidity);
  const holdoutChecks = ecologyChecks(holdoutValidity);
  const ecologyPass =
    Object.values(evalChecks).every(Boolean) &&
    Object.values(holdoutChecks).every(Boolean);
  ...
  const expectedPreprocessingSha256 =
    "83cd6fa437c7cb526fc59bbb19285d7af7c7f2e397a97f7ac3b422409bccda99";
  const preprocessingReproduced =
    preprocessingSha256 === expectedPreprocessingSha256;

  const raw = representationAudit(
    representationSequences(trainTapes, preprocessing, "raw"),
    representationSequences(evalTapes, preprocessing, "raw"),
    representationSequences(holdoutTapes, preprocessing, "raw"),
  );
  const pca = representationAudit(
    representationSequences(trainTapes, preprocessing, "pca"),
    representationSequences(evalTapes, preprocessing, "pca"),
    representationSequences(holdoutTapes, preprocessing, "pca"),
  );
  ...
  const supportPass =
    ...
    supports.every(
      (row) =>
        row.positiveSupport >= V15M_D1_MIN_CLASS_SUPPORT &&
        row.negativeSupport >= V15M_D1_MIN_CLASS_SUPPORT,
    );

  const rawStable =
    supportPass &&
    raw.holdout.aligned.balancedAccuracy >= 0.65 &&
    raw.holdout.alignedMinusShiftedBa >= 0.10;
  const pcaStable =
    supportPass &&
    pca.holdout.aligned.balancedAccuracy >= 0.65 &&
    pca.holdout.alignedMinusShiftedBa >= 0.10;
  const strongPcaDegradation =
    supportPass &&
    (
      pca.holdout.aligned.balancedAccuracy < 0.60 ||
      pca.holdout.trainToAlignedBaDrop >= 0.15
    );

  let outcome;
  if (!ecologyPass || !preprocessingReproduced) {
    outcome = "V15M_D1_IMPLEMENTATION_OR_ECOLOGY_INVALID";
  } else if (!supportPass) {
    outcome = "V15M_D1_INSUFFICIENT_LABEL_SUPPORT";
  } else if (rawStable && !pcaStable && strongPcaDegradation) {
    outcome = "V15M_D1_PCA_BOTTLENECK_SUPPORTED";
  } else if (rawStable && pcaStable) {
    outcome = "V15M_D1_TRACE_AND_PCA_SIGNAL_STABLE";
  } else if (!rawStable) {
    outcome = "V15M_D1_DECISION_TRACE_SIGNAL_UNSTABLE";
  } else {
    outcome = "V15M_D1_INCONCLUSIVE";
  }

  console.log(
    "[v15M-D1] RAW trainBA=" +
      (raw.train?.balancedAccuracy * 100).toFixed(1) +
      "% evalBA=" +
      (raw.eval?.aligned.balancedAccuracy * 100).toFixed(1) +
      "% holdoutBA=" +
      (raw.holdout?.aligned.balancedAccuracy * 100).toFixed(1) +
      "% holdoutShiftDrop=" +
      (raw.holdout?.alignedMinusShiftedBa * 100).toFixed(1) +
      "pp",
  );
  console.log(
    "[v15M-D1] PCA trainBA=" +
      (pca.train?.balancedAccuracy * 100).toFixed(1) +
      "% evalBA=" +
      (pca.eval?.aligned.balancedAccuracy * 100).toFixed(1) +
      "% holdoutBA=" +
      (pca.holdout?.aligned.balancedAccuracy * 100).toFixed(1) +
      "% holdoutShiftDrop=" +
      (pca.holdout?.alignedMinusShiftedBa * 100).toFixed(1) +
      "pp outcome=" +
      outcome,
  );

  const output = {
    schema: "maplefly.v15m-d1.trace-pca-stability.1",
    preregistration: {
      path: "history/prereg_v15m_d1.md",
      commit: V15M_D1_PREREG_COMMIT,
    },
    frozenV15M: {
      receiptCommit:
        "ac0c5c995ec48bfc1ef9c2ef0bd8580f03acf958",
      outcome: "V15M_PERSISTENT_NEURAL_ACTION_BELIEF_FAIL",
      preprocessingSha256: expectedPreprocessingSha256,
      preprocessingReproduced,
    },
    cohorts: {
      trainBaseSeeds: V15M_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15M_EVAL_BASE_SEEDS,
      holdoutBaseSeeds: V15M_D1_HOLDOUT_BASE_SEEDS,
      frozenInterruptionRngSeed: 3937000,
      holdoutInterruptionRngSeed:
        V15M_D1_HOLDOUT_INTERRUPTION_SEED,
    },
    label: {
      name: "RECENT_IMPACT_2S",
      windowSeconds: V15M_D1_RECENT_IMPACT_SECONDS,
      definition:
        "exists damage event with decisionStep-2s <= event.step < decisionStep",
      runtimeFeature: false,
    },
    representation: {
      raw: "TRAIN_STANDARDIZED_CAUSAL_TRACE_1316",
      pca: "TRAIN_STANDARDIZED_CAUSAL_TRACE_PCA32",
      pcaComponents: V15M_PCA_COMPONENTS,
      pcaIterations: V15M_PCA_ITERATIONS,
      pcaSeedBase: V15M_PCA_SEED,
      preprocessingSha256,
    },
    ecology: {
      eval: {
        ...
        checks: evalChecks,
        pass: Object.values(evalChecks).every(Boolean),
      },
      holdout: {
        metrics: holdoutValidity,
        checks: holdoutChecks,
        pass: Object.values(holdoutChecks).every(Boolean),
      },
      pass: ecologyPass,
    },
    diagnostics: {
  ...
  const outDir = resolve("results/v15m-d1-trace-pca-stability");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15m_d1.json"),
