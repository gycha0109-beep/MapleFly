...
async function main() {
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6 evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6 DN identity/order mismatch");
  }
  ...
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D6_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(V15N_D6_HOLDOUT_INTERRUPTION_SEED),
    label: "d6-holdout",
  });
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D6 frozen preprocessing mismatch");
  }
  ...
  const holdoutDetectorSet = collectD6DetectorExamples(
    holdoutTapes,
    phase.means,
  );
  const supports = {
    train: d6Support(trainDetectorSet),
    eval: d6Support(evalDetectorSet),
    holdout: d6Support(holdoutDetectorSet),
  };
  const supportPass = Object.values(supports).every(
    (row) => row.pass,
  );
  ...
  const detectorMetrics = {
    train: d6DetectorMetrics(trainDetectorSet, detector),
    eval: d6DetectorMetrics(evalDetectorSet, detector),
    holdout: d6DetectorMetrics(holdoutDetectorSet, detector),
  };
  const detectorStable =
    detectorMetrics.eval.balancedAccuracy >= 0.75 &&
    detectorMetrics.holdout.balancedAccuracy >= 0.75 &&
    detectorMetrics.eval.positiveRecall >= 0.65 &&
    detectorMetrics.holdout.positiveRecall >= 0.65 &&
    detectorMetrics.eval.negativeRecall >= 0.65 &&
    detectorMetrics.holdout.negativeRecall >= 0.65;

  accumulateD6Evidence(trainTapes, phase.means, detector);
  accumulateD6Evidence(evalTapes, phase.means, detector);
  accumulateD6Evidence(holdoutTapes, phase.means, detector);

  const decisionMeans = trainD6DecisionMeans(trainTapes);
  const trainRows = d6Rows(trainTapes, decisionMeans);
  const evalRows = d6Rows(evalTapes, decisionMeans);
  const holdoutRows = d6Rows(holdoutTapes, decisionMeans);
  const residualSupportPass =
    d6ResidualSd(trainRows) >= V15N_D6_MIN_RESIDUAL_SD &&
    d6ResidualSd(evalRows) >= V15N_D6_MIN_RESIDUAL_SD &&
    d6ResidualSd(holdoutRows) >= V15N_D6_MIN_RESIDUAL_SD;

  const calibration = fitD6Calibration(trainRows);
  const metrics = {
    train: d6Metrics(trainRows, calibration),
    eval: d6Metrics(evalRows, calibration),
    holdout: d6Metrics(holdoutRows, calibration),
    shiftedEval: d6Metrics(
      evalRows,
      calibration,
      d6ShiftedEvidence(evalRows),
    ),
    shiftedHoldout: d6Metrics(
      holdoutRows,
      calibration,
      d6ShiftedEvidence(holdoutRows),
    ),
  };
  const accumulatorStable =
    metrics.eval.r2 >= 0.30 &&
    metrics.holdout.r2 >= 0.30 &&
    metrics.eval.mae <= 10 &&
    metrics.holdout.mae <= 10;
  const episodeSpecific =
    metrics.eval.r2 - metrics.shiftedEval.r2 >= 0.15 &&
    metrics.holdout.r2 - metrics.shiftedHoldout.r2 >= 0.15;

  let outcome;
  if (!ecologyPass || !supportPass || !residualSupportPass) {
    outcome = "V15N_D6_IMPLEMENTATION_OR_SUPPORT_INVALID";
  } else if (
    detectorStable &&
    accumulatorStable &&
    episodeSpecific
    ...
    outcome = "V15N_D6_NON_EPISODE_SPECIFIC_ACCUMULATION";
  } else if (detectorStable && !accumulatorStable) {
    outcome =
      "V15N_D6_IMPACT_DETECTABLE_BUT_ACCUMULATOR_NOT_DEMONSTRATED";
  } else if (!detectorStable) {
    outcome =
      "V15N_D6_IMPACT_DETECTOR_NOT_STABLE_ON_V15N_COHORTS";
  } else {
    outcome = "V15N_D6_INCONCLUSIVE";
  }

  for (const cohort of ["train", "eval", "holdout"]) {
    const d = detectorMetrics[cohort];
    console.log(
      "[v15N-D6] detector " +
        cohort +
        " BA=" +
        d.balancedAccuracy.toFixed(4) +
        " posRecall=" +
        d.positiveRecall.toFixed(4) +
        " negRecall=" +
        d.negativeRecall.toFixed(4) +
        " support=" +
        d.positives +
        "/" +
        d.negatives,
  ...
  console.log(
    "[v15N-D6] accumulator train R2=" +
      metrics.train.r2.toFixed(4) +
      " MAE=" +
      metrics.train.mae.toFixed(2) +
      " eval R2=" +
      metrics.eval.r2.toFixed(4) +
      " MAE=" +
      metrics.eval.mae.toFixed(2) +
      " shifted=" +
      metrics.shiftedEval.r2.toFixed(4) +
      " holdout R2=" +
      metrics.holdout.r2.toFixed(4) +
      " MAE=" +
      metrics.holdout.mae.toFixed(2) +
      " shifted=" +
      metrics.shiftedHoldout.r2.toFixed(4),
  );
  console.log(
    "[v15N-D6] detectorStable=" +
      detectorStable +
      " accumulatorStable=" +
      accumulatorStable +
      " episodeSpecific=" +
  ...
  const output = {
    schema: "maplefly.v15n-d6.mechanistic-impact-accumulator.1",
    preregistration: {
      path: "history/prereg_v15n_d6.md",
      commit: V15N_D6_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD5Receipt:
    ...
    cohorts: {
      ...
      holdoutBaseSeeds: V15N_D6_HOLDOUT_BASE_SEEDS,
      originalInterruptionRngSeed: 4147000,
      holdoutInterruptionRngSeed:
        V15N_D6_HOLDOUT_INTERRUPTION_SEED,
    },
    frameContract: {
      frameSeconds: 0.1,
      framesPerTape: 480,
      phaseFrames: POTION_HISTORY_FRAMES,
      trainPhaseCounts: phase.counts,
      normalizedDnCount: DN_COUNT,
    },
    detector: {
      positiveMaxAgeStepsExclusive:
        V15N_D6_POSITIVE_MAX_AGE_STEPS,
      negativeMinAgeStepsInclusive:
        V15N_D6_NEGATIVE_MIN_AGE_STEPS,
      support: supports,
      threshold: detector.threshold,
      separation: detector.separation,
      positiveProjectedMean:
        detector.positiveProjectedMean,
      negativeProjectedMean:
        detector.negativeProjectedMean,
      weightsSha256: createHash("sha256")
        .update(JSON.stringify(Array.from(detector.weights)))
        .digest("hex"),
      metrics: detectorMetrics,
      stable: detectorStable,
      oracleLabelsUsedForTraining: true,
      deployable: false,
    },
    accumulator: {
      definition:
        "A_t=A_(t-1)+max(0,dot(w,residual_t)-threshold)/separation",
      decisionMeans,
      calibration,
      metrics,
      stable: accumulatorStable,
      episodeSpecific,
    },
    support: {
      detector: supportPass,
      damageResidual: {
        trainSd: d6ResidualSd(trainRows),
        evalSd: d6ResidualSd(evalRows),
        holdoutSd: d6ResidualSd(holdoutRows),
        pass: residualSupportPass,
      },
      pass: supportPass && residualSupportPass,
    },
    ...
    oracleContactAgeRuntimeInput: false,
  ...
  const outDir = resolve(
    "results/v15n-d6-mechanistic-impact-accumulator",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d6.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  if (outcome === "V15N_D6_IMPLEMENTATION_OR_SUPPORT_INVALID") {
    process.exitCode = 1;
  }
}
