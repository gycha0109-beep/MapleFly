...
function parseArgs(argv) {
  ...
  if (
    !values.gains.length ||
    values.gains.some((value) => !Number.isFinite(value) || value <= 0)
  ) {
    throw new Error("--gains must be comma-separated finite values > 0");
  }

  if (!values.gains.includes(1)) {
    throw new Error("--gains must include 1 as the unmodified-connectome reference");
  }
...
function applyAplGain(brain, apl, gain) {
  brain.outGain.fill(1);

  for (const neuron of apl) {
    brain.outGain[neuron] = gain;
  }
}
...
function runCondition({
  ...
  applyAplGain(brain, groups.apl, aplGain);
...
function summarizeByGain(rows, gains) {
  const baselineDnBySeed = new Map(
    rows
      .filter((row) => row.aplGain === 1)
  ...
  const summary = gains.map((level) => {
    const same = rows.filter((row) => row.aplGain === level);
    ...
    // Full-connectome learning also needs a dopamine teacher that is not
    // already firing continuously. 5 Hz is fixed before this run from the
    // upstream small-world MEMORY burst gate; it is a diagnostic threshold,
    // not a claim about exact biological firing rates.
    const teacherQuiet =
      mean(same.map((row) => row.rest.pamHz)) <= TEACHER_QUIET_HZ &&
      mean(same.map((row) => row.rest.ppl1Hz)) <= TEACHER_QUIET_HZ;

    const ready = gate2 && teacherQuiet;
  ...
  for (let i = 0; i + 2 < summary.length; i += 1) {
    ...
    if (window.every((row) => row.ready)) {
      robustWindows.push(
        window.map((row) => row.aplGain),
      );
    }
  }
...
function toMarkdown(meta, summary) {
  const lines = [
    ...
    "",
    "| APL output gain | rest active | vinegar active | cVA active | vinegar/rest | cVA/rest | signature Jaccard | DN drift | vinegar MBON >=2Hz | cVA MBON >=2Hz | Gate1 | Gate2 |",
    "| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | :---: | :---: |",
  ];

  for (const row of summary.gains) {
    lines.push(
      "| " +
        row.aplGain.toFixed(2) +
        " | " +
        ...
        " | " +
        row.restPamHz.toFixed(2) +
        " | " +
        row.restPpl1Hz.toFixed(2) +
        " | " +
        (row.gate1 ? "PASS" : "FAIL") +
        " | " +
        (row.gate2 ? "PASS" : "FAIL") +
        " | " +
        (row.teacherQuiet ? "PASS" : "FAIL") +
        " | " +
        (row.ready ? "PASS" : "FAIL") +
        " |",
    );
  ...
  lines.push(
    ...
    "",
    "> APL output gain는 학습 구현이 아니라 포화 원인 확인용 진단 개입이다. 이 단계에서 PASS가 나와도 정답 행동이나 게임 보상 조건식은 추가하지 않는다.",
    "",
  );
...
async function main() {
  ...
  console.log(
    "MapleFly v6 APL-feedback · " +
      options.seeds +
      " seeds · " +
      options.gains.length +
      " APL-gain levels",
  );
  ...
  for (let seedOffset = 0; seedOffset < options.seeds; seedOffset += 1) {
    ...
    for (let index = 0; index < options.gains.length; index += 1) {
      const aplGain = options.gains[index];
  ...
  const meta = {
    schema:
      "maplefly.experiment-v6.apl-feedback.1",
    startedAt,
    ...
    gates: {
      ...
      robust:
        "three adjacent APL-gain levels all pass Gate2",
    },
    populations: {
      kc: groups.kc.length,
  ...
  await writeFile(
    resolve(outDir, "experiment_v6_apl.json"),
    JSON.stringify(
      {
        meta,
        summary,
