...
async function main() {
  ...
  const d1ArtifactPath = process.env.V15N_D6_D1_ARTIFACT_FILE;
  if (!d1ArtifactPath) {
    throw new Error("V15N_D6_D1_ARTIFACT_FILE is required");
  }
  const d1Bytes = await readFile(d1ArtifactPath);
  const d1Sha256 = createHash("sha256")
    .update(d1Bytes)
    .digest("hex");
  if (d1Sha256 !== V15N_D6_D2_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2 authoritative D1 evidence SHA mismatch");
  }
  const frozenD1 = JSON.parse(d1Bytes.toString("utf8"));
  if (
    frozenD1.outcome !==
    "V15N_D6_D1_TEMPORAL_MULTICOUNT_FAILURE"
  ) {
    throw new Error("v15N-D6-D2 prerequisite D1 outcome mismatch");
  }
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2 v15N evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2 DN identity/order mismatch");
  }
  ...
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D6_D2_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(
      V15N_D6_D2_HOLDOUT_INTERRUPTION_SEED,
    ),
    label: "d6-d2-holdout",
  });
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D6-D2 frozen preprocessing mismatch");
  }
  ...
  const phase = computeD6PhaseMeans(trainTapes);
  const trainDetectorSet = collectD6DetectorExamples(
    trainTapes,
    phase.means,
  );
  const evalDetectorSet = collectD6DetectorExamples(
    evalTapes,
    phase.means,
  );
  const detector = trainD6ImpactDirection(
    trainDetectorSet.examples,
  );
  const detectorWeightsSha256 = createHash("sha256")
    .update(JSON.stringify(Array.from(detector.weights)))
    .digest("hex");
  if (detectorWeightsSha256 !== V15N_D6_D1_WEIGHTS_SHA256) {
    throw new Error("v15N-D6-D2 detector weight SHA mismatch");
  }
  d1AssertClose(
    detector.threshold,
    frozenD1.detectorReproduction.threshold,
    "detector.threshold",
  );
  d1AssertClose(
    detector.separation,
    frozenD1.detectorReproduction.separation,
    "detector.separation",
  );

  const reproducedEvalAttribution = d1Attribution(
    evalTapes,
    phase.means,
    detector,
  );
  for (const [key, actual, expected] of [
    [
      "backgroundMassFraction",
      reproducedEvalAttribution.backgroundMassFraction,
      frozenD1.attribution.eval.backgroundMassFraction,
    ],
    [
      "duplicateMassFraction",
      reproducedEvalAttribution.duplicateMassFraction,
      frozenD1.attribution.eval.duplicateMassFraction,
    ],
    [
      "perHit.cvEvidence",
      reproducedEvalAttribution.perHit.cvEvidence,
      frozenD1.attribution.eval.perHit.cvEvidence,
    ],
    [
      "perHit.medianPositiveFrameCount",
      reproducedEvalAttribution.perHit.medianPositiveFrameCount,
      frozenD1.attribution.eval.perHit.medianPositiveFrameCount,
    ],
  ]) {
    d1AssertClose(actual, expected, "d1.eval." + key);
  }

  d2AttachEventizer(trainTapes, phase.means, detector);
  d2AttachEventizer(evalTapes, phase.means, detector);
  d2AttachEventizer(holdoutTapes, phase.means, detector);

  const eventMetrics = {
    train: d2EventMetrics(trainTapes),
    eval: d2EventMetrics(evalTapes),
    holdout: d2EventMetrics(holdoutTapes),
  };
  const supportPass = Object.values(eventMetrics).every(
    (row) =>
      row.tapes === 24 &&
      row.physicalHits >= V15N_D6_D2_MIN_PHYSICAL_HITS &&
      row.neuralEvents >= V15N_D6_D2_MIN_NEURAL_EVENTS,
  );

  const decisionMeans = d2TrainDecisionMeans(trainTapes);
  const rows = {
    train: d2DecisionRows(trainTapes, decisionMeans),
    eval: d2DecisionRows(evalTapes, decisionMeans),
    holdout: d2DecisionRows(holdoutTapes, decisionMeans),
  };
  const directMetrics = {
    train: d2DirectMetrics(rows.train),
    eval: d2DirectMetrics(rows.eval),
    holdout: d2DirectMetrics(rows.holdout),
  };
  const residualMetrics = {
    train: d2ResidualMetrics(rows.train),
    eval: d2ResidualMetrics(rows.eval),
    holdout: d2ResidualMetrics(rows.holdout),
    shiftedEval: d2ResidualMetrics(
      rows.eval,
      d2ShiftedResidual(rows.eval),
    ),
    shiftedHoldout: d2ResidualMetrics(
      rows.holdout,
      d2ShiftedResidual(rows.holdout),
    ),
  };

  const eventDetectionStable =
    eventMetrics.eval.precision >= 0.75 &&
    eventMetrics.eval.recall >= 0.75 &&
    eventMetrics.holdout.precision >= 0.75 &&
    eventMetrics.holdout.recall >= 0.75;
  const directCountStable =
    directMetrics.eval.mae <= 1.0 &&
    directMetrics.holdout.mae <= 1.0 &&
    Math.abs(directMetrics.eval.meanSignedError) <= 0.75 &&
    Math.abs(directMetrics.holdout.meanSignedError) <= 0.75;
  const residualStable =
    residualMetrics.eval.r2 >= 0.30 &&
    residualMetrics.holdout.r2 >= 0.30 &&
    residualMetrics.eval.mae <= 1.0 &&
    residualMetrics.holdout.mae <= 1.0;
  const episodeSpecific =
    residualMetrics.eval.r2 - residualMetrics.shiftedEval.r2 >=
      0.15 &&
    residualMetrics.holdout.r2 -
        residualMetrics.shiftedHoldout.r2 >=
      0.15;

  let outcome;
  if (!ecologyPass || !supportPass) {
    outcome =
    ...
    eventDetectionStable &&
    !(directCountStable && residualStable && episodeSpecific)
  ) {
    outcome =
      "V15N_D6_D2_EVENT_DETECTION_STABLE_COUNT_NOT_DEMONSTRATED";
  } else if (!eventDetectionStable) {
    outcome = "V15N_D6_D2_EVENTIZER_NOT_STABLE";
  } else {
    outcome = "V15N_D6_D2_INCONCLUSIVE";
  }
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    console.log(
      "[v15N-D6-D2] " +
        cohort +
        " events=" +
        eventMetrics[cohort].neuralEvents +
        " hits=" +
        eventMetrics[cohort].physicalHits +
        " precision=" +
        eventMetrics[cohort].precision.toFixed(4) +
        " recall=" +
        eventMetrics[cohort].recall.toFixed(4) +
        " directMAE=" +
        directMetrics[cohort].mae.toFixed(4) +
        " directBias=" +
        directMetrics[cohort].meanSignedError.toFixed(4) +
        " residualR2=" +
        residualMetrics[cohort].r2.toFixed(4) +
        " residualMAE=" +
        residualMetrics[cohort].mae.toFixed(4),
    );
  }
  console.log(
    "[v15N-D6-D2] shifted eval R2=" +
      residualMetrics.shiftedEval.r2.toFixed(4) +
      " holdout R2=" +
      residualMetrics.shiftedHoldout.r2.toFixed(4),
  );
  console.log(
    "[v15N-D6-D2] eventDetectionStable=" +
      eventDetectionStable +
      " directCountStable=" +
      directCountStable +
      " residualStable=" +
  ...
  const output = {
    schema: "maplefly.v15n-d6-d2.causal-neural-eventizer.1",
    preregistration: {
      path: "history/prereg_v15n_d6_d2.md",
      commit: V15N_D6_D2_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD6D1Receipt:
      ...
      v15nD6D1Outcome:
        "V15N_D6_D1_TEMPORAL_MULTICOUNT_FAILURE",
      authoritativeD1EvidenceSha256: d1Sha256,
    },
    cohorts: {
      trainBaseSeeds: V15N_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15N_EVAL_BASE_SEEDS,
      holdoutBaseSeeds: V15N_D6_D2_HOLDOUT_BASE_SEEDS,
      originalInterruptionRngSeed: 4147000,
      holdoutInterruptionRngSeed:
        V15N_D6_D2_HOLDOUT_INTERRUPTION_SEED,
    },
    detectorReproduction: {
      weightsSha256: detectorWeightsSha256,
      threshold: detector.threshold,
      separation: detector.separation,
      originalEvalAttribution: reproducedEvalAttribution,
      pass: true,
    },
    eventizer: {
      positiveCondition: "margin > 0",
      initialArmed: true,
      rearmConsecutiveNonPositiveFrames:
        V15N_D6_D2_REARM_NEGATIVE_FRAMES,
      runtimeOracleInput: false,
    },
    eventMetrics,
    decisionMeans,
    directMetrics,
    residualMetrics,
    support: {
      minimumPhysicalHits: V15N_D6_D2_MIN_PHYSICAL_HITS,
      minimumNeuralEvents: V15N_D6_D2_MIN_NEURAL_EVENTS,
      pass: supportPass,
    },
    ecology: {
      eval: {
    ...
    provenance: {
      v15nEvidenceSha256: artifactSha256,
      preprocessingSha256,
      pass: true,
    },
    outcome,
    replacementPolicyTrained: false,
    replacementDetectorTrained: false,
    diagnosticImpactDetectorDeployable: false,
    eventizerDeployableWithCurrentDetector: false,
    deployment: "BLOCKED",
    deployedPotion: "v15D",
    v16c: "BLOCKED",
  };
  ...
  const outDir = resolve(
    "results/v15n-d6-d2-causal-neural-eventizer",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d6_d2.json"),
