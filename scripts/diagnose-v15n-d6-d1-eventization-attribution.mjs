...
async function main() {
  ...
  const d6ArtifactPath = process.env.V15N_D6_ARTIFACT_FILE;
  if (!d6ArtifactPath) {
    throw new Error("V15N_D6_ARTIFACT_FILE is required");
  }
  const d6Bytes = await readFile(d6ArtifactPath);
  const d6Sha256 = createHash("sha256")
    .update(d6Bytes)
    .digest("hex");
  if (d6Sha256 !== V15N_D6_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D1 authoritative D6 evidence SHA mismatch");
  }
  const frozenD6 = JSON.parse(d6Bytes.toString("utf8"));
  if (
    frozenD6.outcome !==
    "V15N_D6_IMPACT_DETECTABLE_BUT_ACCUMULATOR_NOT_DEMONSTRATED"
  ) {
    throw new Error("v15N-D6-D1 prerequisite D6 outcome mismatch");
  }
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D1 v15N evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D1 DN identity/order mismatch");
  }
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D6-D1 frozen preprocessing mismatch");
  }
  ...
  if (detectorWeightsSha256 !== V15N_D6_D1_WEIGHTS_SHA256) {
    throw new Error("v15N-D6-D1 detector weight SHA mismatch");
  }
  d1AssertClose(
    detector.threshold,
    frozenD6.detector.threshold,
    "detector.threshold",
  );
  d1AssertClose(
    detector.separation,
    frozenD6.detector.separation,
    "detector.separation",
  );
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    for (const key of [
      ...
      d1AssertClose(
        detectorMetrics[cohort][key],
        frozenD6.detector.metrics[cohort][key],
        "detector." + cohort + "." + key,
      );
    }
  }
  ...
  for (const cohort of [
    ...
    d1VerifyMetricObject(
      reproducedAccumulator[cohort],
      frozenD6.accumulator.metrics[cohort],
      "accumulator." + cohort,
    );
  }

  const attribution = {
    train: d1Attribution(trainTapes, phase.means, detector),
    eval: d1Attribution(evalTapes, phase.means, detector),
    holdout: d1Attribution(holdoutTapes, phase.means, detector),
  };
  const cohortFlags = {
    eval: d1MechanismFlags(attribution.eval),
    holdout: d1MechanismFlags(attribution.holdout),
  };
  const globalFlags = {
    backgroundLeakageHigh:
      cohortFlags.eval.backgroundLeakageHigh &&
      cohortFlags.holdout.backgroundLeakageHigh,
    multiCountHigh:
      cohortFlags.eval.multiCountHigh &&
      cohortFlags.holdout.multiCountHigh,
    amplitudeUnstable:
      cohortFlags.eval.amplitudeUnstable &&
      cohortFlags.holdout.amplitudeUnstable,
    amplitudeShift: false,
  };
  const amplitudeRatio =
    attribution.holdout.perHit.meanEvidence /
    attribution.eval.perHit.meanEvidence;
  globalFlags.amplitudeShift =
    amplitudeRatio < V15N_D6_D1_AMPLITUDE_RATIO_LOW ||
    amplitudeRatio > V15N_D6_D1_AMPLITUDE_RATIO_HIGH;
  ...
  if (!ecologyPass) {
    ...
    globalFlags.backgroundLeakageHigh &&
    globalFlags.multiCountHigh
  ) {
    outcome =
      "V15N_D6_D1_BACKGROUND_AND_MULTICOUNT_FAILURE";
  } else if (globalFlags.backgroundLeakageHigh) {
    outcome = "V15N_D6_D1_BACKGROUND_LEAKAGE_FAILURE";
  } else if (globalFlags.multiCountHigh) {
    outcome = "V15N_D6_D1_TEMPORAL_MULTICOUNT_FAILURE";
  } else if (
    globalFlags.amplitudeUnstable ||
    globalFlags.amplitudeShift
  ) {
    outcome = "V15N_D6_D1_HIT_AMPLITUDE_INSTABILITY";
  } else {
    outcome =
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    const row = attribution[cohort];
    console.log(
      "[v15N-D6-D1] " +
        cohort +
        " backgroundMass=" +
        row.backgroundMassFraction.toFixed(4) +
        " backgroundFPR=" +
        row.backgroundPositiveFrameRate.toFixed(4) +
        " duplicateMass=" +
        row.duplicateMassFraction.toFixed(4) +
        " medianFramesPerHit=" +
        row.perHit.medianPositiveFrameCount.toFixed(2) +
        " hitCV=" +
        row.perHit.cvEvidence.toFixed(4),
    );
  }
  console.log(
    "[v15N-D6-D1] amplitudeRatio=" +
      amplitudeRatio.toFixed(4) +
      " flags=" +
      JSON.stringify(globalFlags) +
      " outcome=" +
      outcome,
  );

  const output = {
    schema: "maplefly.v15n-d6-d1.eventization-attribution.1",
    preregistration: {
      path: "history/prereg_v15n_d6_d1.md",
      commit: V15N_D6_D1_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD6Receipt:
      ...
      v15nD6Outcome:
        "V15N_D6_IMPACT_DETECTABLE_BUT_ACCUMULATOR_NOT_DEMONSTRATED",
      authoritativeD6EvidenceSha256: d6Sha256,
    },
    ...
    accumulatorReproduction: {
      metrics: reproducedAccumulator,
      pass: true,
    },
    attribution,
    cohortFlags,
    amplitudeRatioHoldoutOverEval: amplitudeRatio,
    globalFlags,
    gates: {
      backgroundMassFraction:
        V15N_D6_D1_BACKGROUND_FRACTION_GATE,
      duplicateMassFraction:
        V15N_D6_D1_DUPLICATE_FRACTION_GATE,
      medianPositiveFrames:
        V15N_D6_D1_MEDIAN_POSITIVE_FRAMES_GATE,
      amplitudeCv: V15N_D6_D1_AMPLITUDE_CV_GATE,
      amplitudeRatioLow:
        V15N_D6_D1_AMPLITUDE_RATIO_LOW,
      amplitudeRatioHigh:
        V15N_D6_D1_AMPLITUDE_RATIO_HIGH,
    },
    provenance: {
    ...
    evaluatorAttributionRuntimeInput: false,
    deployment: "BLOCKED",
    deployedPotion: "v15D",
    v16c: "BLOCKED",
  };
  ...
  const outDir = resolve(
    "results/v15n-d6-d1-eventization-attribution",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d6_d1.json"),
