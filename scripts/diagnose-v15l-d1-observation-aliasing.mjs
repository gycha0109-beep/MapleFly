...
async function main() {
  ...
  if (
    ...
    throw new Error("v15L-D1 v14C provenance mismatch");
  }
  const attackPolicy = {
  ...
  if (!actionIndependence.pass) {
    throw new Error(
      "v15L-D1 counterfactual action leaks into neural tape: " +
  ...
  if (
    ...
    throw new Error("v15L-D1 DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(V15L_D1_INTERRUPTION_SEED);
  const tapes = await collectTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15L_D1_BASE_SEEDS,
    interruptionRandom,
    label: "d1-holdout",
  });
  if (tapes.length !== 24) {
    throw new Error("v15L-D1 tape count mismatch");
  }
  ...
  const tapeResults = tapes.map(enumerateTapeAliasing);
  const conflictGroups = tapeResults.flatMap((row) => row.conflictGroups);
  const episodesWithConflict = tapeResults.filter(
    (row) => row.conflictGroups.length > 0,
  );
  const allGroups = tapeResults.flatMap((row) => row.groups);
  const reachableStates = tapeResults.reduce(
    (sum, row) => sum + row.reachableStates,
    0,
  );
  const conflictsByDecision = Array.from(
    { length: 10 },
    (_, decisionIndex) =>
      conflictGroups.filter(
        (row) => row.decisionIndex === decisionIndex,
      ).length,
  );
  const earliestConflictDecision = conflictGroups.length
    ? Math.min(...conflictGroups.map((row) => row.decisionIndex))
    : null;

  const outcome = !ecologyPass
  ...
  console.log(
    "[v15L-D1] tapes=" + tapes.length +
      " reachableStates=" + reachableStates +
      " groups=" + allGroups.length +
      " conflictGroups=" + conflictGroups.length +
      " episodesWithConflict=" + episodesWithConflict.length +
      " earliestDecision=" + earliestConflictDecision +
      " outcome=" + outcome,
  );

  const output = {
    schema: "maplefly.v15l-d1.observation-aliasing.1",
    preregistration: {
      path: "history/prereg_v15l_d1.md",
      commit: V15L_D1_PREREG_COMMIT,
    },
    frozenV15L: {
      receiptCommit: "dee412f6660743c36673415e5f3cd10513b59c63",
      outcome: "V15L_CAUSAL_NEURAL_TRACE_REMEDIATION_FAIL",
      trainSurvivalRate: 0.8333333333333334,
      evalSurvivalRate: 0.125,
      survivalGeneralizationDrop: 0.7083333333333334,
    },
    cohort: {
      baseSeeds: V15L_D1_BASE_SEEDS,
      tapes: tapes.length,
      interruptionRngSeed: V15L_D1_INTERRUPTION_SEED,
    },
    actionIndependence,
    ecology: {
      metrics: validity,
    ...
    aliasing: {
      reachableStates,
      exactObservationGroups: allGroups.length,
      conflictGroups: conflictGroups.length,
      episodesWithConflict: episodesWithConflict.length,
      episodeConflictFraction: episodesWithConflict.length / tapes.length,
      earliestConflictDecision,
      conflictsByDecision,
      classificationCountsInConflictGroups:
        summarizeClassifications(tapeResults),
      conflictDetails: conflictGroups,
    },
    outcome,
  ...
  const outDir = resolve("results/v15l-d1-observation-aliasing");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15l_d1.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  if (!ecologyPass) process.exitCode = 1;
}
