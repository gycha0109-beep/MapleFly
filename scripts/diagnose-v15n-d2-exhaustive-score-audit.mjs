...
function balancedMetrics(rows, actionKey) {
  const wait = rows.filter(
    (row) => row.oracleClass === "FORCED_WAIT",
  );
  const drink = rows.filter(
    (row) => row.oracleClass === "FORCED_DRINK",
  );
  const waitCorrect = wait.filter(
    (row) => row[actionKey] === 0,
  ).length;
  const drinkCorrect = drink.filter(
    (row) => row[actionKey] === 1,
  ).length;
  const waitRecall = wait.length ? waitCorrect / wait.length : null;
  const drinkRecall = drink.length
    ? drinkCorrect / drink.length
    : null;
  return {
    forcedWait: wait.length,
    forcedDrink: drink.length,
    waitRecall,
    drinkRecall,
    balancedAccuracy:
      waitRecall === null || drinkRecall === null
        ? null
        : (waitRecall + drinkRecall) / 2,
    ordinaryAccuracy:
      rows.length
        ? (waitCorrect + drinkCorrect) / rows.length
        : null,
  };
}

function stateWeightedAuc(rows) {
  const drink = rows
    .filter((row) => row.oracleClass === "FORCED_DRINK")
    .map((row) => row.residual)
    .sort((a, b) => a - b);
  ...
  let we = 0;
  for (const d of drink) {
    while (wi < wait.length && wait[wi] < d) wi += 1;
    if (we < wi) we = wi;
    while (we < wait.length && wait[we] === d) we += 1;
    less += wi;
    equal += we - wi;
  }
  return (less + 0.5 * equal) / (drink.length * wait.length);
}

function macroTapeBalancedAccuracy(rows, actionKey) {
  const byTape = new Map();
  for (const row of rows) {
    if (!byTape.has(row.seed)) byTape.set(row.seed, []);
    byTape.get(row.seed).push(row);
  }
  const values = [];
  for (const [seed, tapeRows] of byTape) {
    const metrics = balancedMetrics(tapeRows, actionKey);
    if (
      metrics.forcedWait > 0 &&
      metrics.forcedDrink > 0
    ) {
      values.push({ seed, balancedAccuracy: metrics.balancedAccuracy });
    }
  }
  return {
    mixedTapeCount: values.length,
    meanBalancedAccuracy:
      values.length
        ? mean(values.map((row) => row.balancedAccuracy))
        : null,
    perTape: values,
  };
}

function exhaustiveCohortDiagnostic(
  tapes,
  params,
  meanNeuralScores,
) {
  ...
  let eitherStates = 0;
  let unsurvivableStates = 0;

  for (const tape of tapes) {
    for (let decisionIndex = 0; decisionIndex < 10; decisionIndex += 1) {
      const neural = neuralTerm(
        tape.neuralFeatures[decisionIndex],
        params,
      );
      const residual =
        neural - meanNeuralScores[decisionIndex];

      for (let mask = 0; mask < 2 ** decisionIndex; mask += 1) {
        const history = binaryHistory(mask, decisionIndex);
        const hpBefore = stateBeforeDecision(tape, history);
        if (hpBefore === null) continue;
        reachableStates += 1;

        const oracleClass = oracleClassFromReachedState(
          tape,
          decisionIndex,
          hpBefore,
        );
        if (oracleClass === "EITHER") {
          eitherStates += 1;
          continue;
        }
        if (oracleClass === "UNSURVIVABLE") {
          unsurvivableStates += 1;
          continue;
        }

        const h = recurrentStateForHistory(history, params);
        const stateTerm = params[39] * h[0] + params[40] * h[1];
        const bias = params[0];
        const fullScore = bias + neural + stateTerm;
        const meanScore =
          bias + meanNeuralScores[decisionIndex] + stateTerm;
        const neuralOffScore = bias + stateTerm;
        const stateOffScore = bias + neural;
        ...
        rows.push({
          ...
          neuralTerm: neural,
          residual,
          actionStateTerm: stateTerm,
          h,
          fullScore,
          fullAction: fullScore > 0 ? 1 : 0,
          meanScore,
          meanAction: meanScore > 0 ? 1 : 0,
          neuralOffScore,
          neuralOffAction: neuralOffScore > 0 ? 1 : 0,
          stateOffScore,
          stateOffAction: stateOffScore > 0 ? 1 : 0,
        });
      }
    }
  }
  ...
  const variants = {
    full: balancedMetrics(rows, "fullAction"),
    mean: balancedMetrics(rows, "meanAction"),
    neuralOff: balancedMetrics(rows, "neuralOffAction"),
    actionStateOff: balancedMetrics(rows, "stateOffAction"),
  };

  const macroTape = {
    full: macroTapeBalancedAccuracy(rows, "fullAction"),
    mean: macroTapeBalancedAccuracy(rows, "meanAction"),
    neuralOff: macroTapeBalancedAccuracy(rows, "neuralOffAction"),
    actionStateOff:
      macroTapeBalancedAccuracy(rows, "stateOffAction"),
  };

  const byDecision = Array.from({ length: 10 }, (_, decisionIndex) => {
    const local = rows.filter(
      (row) => row.decisionIndex === decisionIndex,
    );
    const support = balancedMetrics(local, "fullAction");
    return {
      decisionIndex,
      forcedWait: support.forcedWait,
      forcedDrink: support.forcedDrink,
      full:
        support.forcedWait >= 30 && support.forcedDrink >= 30
          ? support
          : null,
      mean:
        support.forcedWait >= 30 && support.forcedDrink >= 30
          ? balancedMetrics(local, "meanAction")
          : null,
      neuralOff:
        support.forcedWait >= 30 && support.forcedDrink >= 30
          ? balancedMetrics(local, "neuralOffAction")
          : null,
      actionStateOff:
        support.forcedWait >= 30 && support.forcedDrink >= 30
          ? balancedMetrics(local, "stateOffAction")
          : null,
    };
  });

  const disagreements = rows
    .filter((row) => row.fullAction !== row.meanAction)
    .slice(0, 20)
    .map((row) => ({
      seed: row.seed,
      decisionIndex: row.decisionIndex,
      history: row.history,
      evaluatorHp: row.hpBefore,
      oracleClass: row.oracleClass,
      neuralTerm: row.neuralTerm,
      meanNeuralTerm: meanNeuralScores[row.decisionIndex],
      residual: row.residual,
      actionStateTerm: row.actionStateTerm,
      fullScore: row.fullScore,
      fullAction: row.fullAction,
      meanScore: row.meanScore,
      meanAction: row.meanAction,
    }));

  return {
    reachableStates,
    forcedStates: rows.length,
    eitherStates,
    unsurvivableStates,
    support: {
      forcedWait: variants.full.forcedWait,
      forcedDrink: variants.full.forcedDrink,
    },
    residualAuc: stateWeightedAuc(rows),
    variants,
    macroTape,
    byDecision,
    magnitude: {
      meanAbsNeuralTerm: meanAbs(rows, "neuralTerm"),
      meanAbsActionStateTerm: meanAbs(rows, "actionStateTerm"),
      meanAbsResidual: meanAbs(rows, "residual"),
      actionStateToNeuralMagnitudeRatio:
        meanAbs(rows, "neuralTerm") > 0
          ? meanAbs(rows, "actionStateTerm") /
            meanAbs(rows, "neuralTerm")
          : null,
    },
    disagreementWitnesses: disagreements,
  };
}
...
async function main() {
  ...
  if (!artifactPath) {
    throw new Error("V15N_ARTIFACT_FILE is required");
  }
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D2 evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D2 v14C provenance mismatch");
  }
  const attackPolicy = {
  ...
  if (
    ...
    throw new Error("v15N-D2 DN identity/order mismatch");
  }
  ...
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D2_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(V15N_D2_HOLDOUT_INTERRUPTION_SEED),
    label: "d2-holdout",
  });
  if (
    trainTapes.length !== 24 ||
    evalTapes.length !== 24 ||
    holdoutTapes.length !== 24
  ) {
    throw new Error("v15N-D2 tape count mismatch");
  }
  ...
  attachTraceFeatures(holdoutTapes, preprocessing);
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D2 preprocessing SHA mismatch");
  }
  ...
  const meanNeuralScores =
    trainDecisionMeanNeuralScores(trainTapes, params);
  const evalDiag = exhaustiveCohortDiagnostic(
    evalTapes,
    params,
    meanNeuralScores,
  );
  const holdoutDiag = exhaustiveCohortDiagnostic(
    holdoutTapes,
    params,
    meanNeuralScores,
  );
  ...
  const supportPass =
    evalDiag.support.forcedWait >= V15N_D2_MIN_FORCED_SUPPORT &&
    evalDiag.support.forcedDrink >= V15N_D2_MIN_FORCED_SUPPORT &&
    holdoutDiag.support.forcedWait >= V15N_D2_MIN_FORCED_SUPPORT &&
    holdoutDiag.support.forcedDrink >= V15N_D2_MIN_FORCED_SUPPORT &&
    evalDiag.macroTape.full.mixedTapeCount >= V15N_D2_MIN_MIXED_TAPES &&
    holdoutDiag.macroTape.full.mixedTapeCount >= V15N_D2_MIN_MIXED_TAPES;

  const evalMeanGain =
    evalDiag.variants.mean.balancedAccuracy -
    evalDiag.variants.full.balancedAccuracy;
  const holdoutMeanGain =
    holdoutDiag.variants.mean.balancedAccuracy -
    holdoutDiag.variants.full.balancedAccuracy;
  const evalNeuralOffGain =
    evalDiag.variants.neuralOff.balancedAccuracy -
    evalDiag.variants.full.balancedAccuracy;
  const holdoutNeuralOffGain =
    holdoutDiag.variants.neuralOff.balancedAccuracy -
    holdoutDiag.variants.full.balancedAccuracy;
  const evalStateOffGain =
    evalDiag.variants.actionStateOff.balancedAccuracy -
    evalDiag.variants.full.balancedAccuracy;
  const holdoutStateOffGain =
    holdoutDiag.variants.actionStateOff.balancedAccuracy -
    holdoutDiag.variants.full.balancedAccuracy;

  let outcome;
  if (!ecologyPass) {
    outcome = "V15N_D2_IMPLEMENTATION_OR_PROVENANCE_INVALID";
  } else if (!supportPass) {
    outcome = "V15N_D2_INSUFFICIENT_COUNTERFACTUAL_SUPPORT";
  } else if (
    evalDiag.residualAuc <= 0.40 &&
    holdoutDiag.residualAuc <= 0.40 &&
    evalMeanGain >= 0.10 &&
    holdoutMeanGain >= 0.10
  ) {
    outcome = "V15N_D2_EPISODE_SPECIFIC_NEURAL_TERM_HARMFUL";
  } else if (
    evalNeuralOffGain >= 0.10 &&
    holdoutNeuralOffGain >= 0.10
  ) {
    outcome = "V15N_D2_NEURAL_READOUT_HARMFUL";
  } else if (
    evalStateOffGain >= 0.10 &&
    holdoutStateOffGain >= 0.10
  ) {
    outcome = "V15N_D2_ACTION_STATE_READOUT_HARMFUL";
  } else if (
    evalDiag.variants.full.balancedAccuracy >= 0.65 &&
    holdoutDiag.variants.full.balancedAccuracy >= 0.65
  ) {
    outcome = "V15N_D2_FROZEN_SCORE_BROADLY_ALIGNED";
  } else {
    outcome = "V15N_D2_INCONCLUSIVE";
  }

  console.log(
    "[v15N-D2] EVAL forced wait=" +
      evalDiag.support.forcedWait +
      " drink=" +
      evalDiag.support.forcedDrink +
      " AUC=" +
      evalDiag.residualAuc.toFixed(4) +
      " FULL_BA=" +
      (evalDiag.variants.full.balancedAccuracy * 100).toFixed(1) +
      "% MEAN_BA=" +
      (evalDiag.variants.mean.balancedAccuracy * 100).toFixed(1) +
      "% gain=" +
      (evalMeanGain * 100).toFixed(1) +
      "pp",
  );
  console.log(
    "[v15N-D2] HOLDOUT forced wait=" +
      holdoutDiag.support.forcedWait +
      " drink=" +
      holdoutDiag.support.forcedDrink +
      " AUC=" +
      holdoutDiag.residualAuc.toFixed(4) +
      " FULL_BA=" +
      (holdoutDiag.variants.full.balancedAccuracy * 100).toFixed(1) +
      "% MEAN_BA=" +
      (holdoutDiag.variants.mean.balancedAccuracy * 100).toFixed(1) +
      "% gain=" +
      (holdoutMeanGain * 100).toFixed(1) +
      "pp",
  );
  console.log(
    "[v15N-D2] EVAL neuralOffGain=" +
      (evalNeuralOffGain * 100).toFixed(1) +
      "pp stateOffGain=" +
      (evalStateOffGain * 100).toFixed(1) +
      "pp HOLDOUT neuralOffGain=" +
      (holdoutNeuralOffGain * 100).toFixed(1) +
      "pp stateOffGain=" +
      (holdoutStateOffGain * 100).toFixed(1) +
      "pp outcome=" +
      outcome,
  );

  const output = {
    schema: "maplefly.v15n-d2.exhaustive-frozen-score-audit.1",
    preregistration: {
      path: "history/prereg_v15n_d2.md",
      commit: V15N_D2_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD1Receipt:
    ...
    frozenCandidate: {
      ...
      paramsSha256: V15N_D1_PARAMS_SHA256,
      preprocessingSha256,
    },
    cohorts: {
      evalBaseSeeds: V15N_EVAL_BASE_SEEDS,
      holdoutBaseSeeds: V15N_D2_HOLDOUT_BASE_SEEDS,
      evalTapes: evalTapes.length,
      holdoutTapes: holdoutTapes.length,
      originalInterruptionRngSeed: 4147000,
      holdoutInterruptionRngSeed:
        V15N_D2_HOLDOUT_INTERRUPTION_SEED,
    },
  ...
  const outDir = resolve("results/v15n-d2-exhaustive-score-audit");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d2.json"),
