...
async function main() {
  ...
  const d3ArtifactPath = process.env.V15N_D6_D2_D3_ARTIFACT_FILE;
  if (!d3ArtifactPath) {
    throw new Error("V15N_D6_D2_D3_ARTIFACT_FILE is required");
  }
  const d3Bytes = await readFile(d3ArtifactPath);
  const d3Sha256 = createHash("sha256").update(d3Bytes).digest("hex");
  if (d3Sha256 !== V15N_D6_D2_D3_D1_D3_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D3-D1 authoritative D2-D3 evidence SHA mismatch");
  }
  const frozenD3 = JSON.parse(d3Bytes.toString("utf8"));
  if (
    frozenD3.outcome !==
    "V15N_D6_D2_D3_CAUSAL_EVENTIZATION_NOT_DEMONSTRATED"
  ) {
    throw new Error("v15N-D6-D2-D3-D1 prerequisite outcome mismatch");
  }
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D3-D1 v15N evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2-D3-D1 DN identity/order mismatch");
  }
  ...
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D6_D2_D3_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(
      V15N_D6_D2_D3_HOLDOUT_INTERRUPTION_SEED,
    ),
    label: "d6-d2-d3-d1-holdout",
  });
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D6-D2-D3-D1 frozen preprocessing mismatch");
  }
  ...
  if (
    ...
    frozenD3.detectorReproduction.weightsSha256
  ) {
    throw new Error("v15N-D6-D2-D3-D1 detector weight SHA mismatch");
  }
  d1AssertClose(
    detector.threshold,
    frozenD3.detectorReproduction.threshold,
    "detector.threshold",
  );
  d1AssertClose(
    detector.separation,
    frozenD3.detectorReproduction.separation,
    "detector.separation",
  );
  ...
  const scalarModel = d2d2FitReadout(labelledRows.train, "scalar");
  const scalarModelSha256 = createHash("sha256")
    .update(JSON.stringify({
      means: scalarModel.means,
      scales: scalarModel.scales,
      weights: scalarModel.weights,
      featureKey: scalarModel.featureKey,
    }))
    .digest("hex");
  if (
    scalarModelSha256 !==
    V15N_D6_D2_D3_D1_SCALAR_MODEL_SHA256
  ) {
    throw new Error("v15N-D6-D2-D3-D1 scalar model SHA mismatch");
  }
  ...
  const reproduced = {
    train: d2d3EventMetrics(trainTapes),
    eval: d2d3EventMetrics(evalTapes),
    holdout: d2d3EventMetrics(holdoutTapes),
  };
  d3d1VerifyEventMetrics(
    reproduced.train,
    frozenD3.metrics.train,
    "train",
  );
  d3d1VerifyEventMetrics(
    reproduced.eval,
    frozenD3.metrics.eval,
    "eval",
  );
  d3d1VerifyEventMetrics(
    reproduced.holdout,
    frozenD3.metrics.holdout,
    "holdout",
  );

  const attribution = {
    train: d3d1CohortAttribution(
      trainTapes,
      phase.means,
      detector,
      scalarModel,
    ),
    eval: d3d1CohortAttribution(
      evalTapes,
      phase.means,
      detector,
      scalarModel,
    ),
    holdout: d3d1CohortAttribution(
      holdoutTapes,
      phase.means,
      detector,
      scalarModel,
  ...
  const support = {
    eval: d3d1Support(attribution.eval),
    holdout: d3d1Support(attribution.holdout),
  };
  const supportPass = support.eval.pass && support.holdout.pass;
  const missFlags = d3d1MissFlags(
    attribution.eval,
    attribution.holdout,
  );
  const falseFlags = d3d1FalseFlags(
    attribution.eval,
    attribution.holdout,
  );

  let falseEventAxis = "MIXED";
  if (falseFlags.recentFalseDominated) {
    falseEventAxis = "RECENT_UNMATCHED_DOMINANT";
  } else if (falseFlags.lingerFalseDominated) {
    falseEventAxis = "LINGER_DOMINANT";
  } else if (falseFlags.backgroundFalseDominated) {
  ...
  } else if (missFlags.classifierMissDominated) {
    outcome =
      "V15N_D6_D2_D3_D1_FRAME_CLASSIFIER_MISS_DOMINANT";
  } else if (missFlags.edgeSuppressionDominated) {
    outcome =
      "V15N_D6_D2_D3_D1_RISING_EDGE_SUPPRESSION_DOMINANT";
  } else if (missFlags.matchingConflictDominated) {
    outcome =
  ...
  for (const cohort of ["train", "eval", "holdout"]) {
    ...
    console.log(
      "[v15N-D6-D2-D3-D1] " +
        ...
        a.missedHits +
        " classifierMiss=" +
        a.missedHitCategories.frameClassifierMissFraction.toFixed(4) +
        " edgeSuppression=" +
        a.missedHitCategories.edgeSuppressionFraction.toFixed(4) +
        " matchingConflict=" +
        a.missedHitCategories.matchingConflictFraction.toFixed(4) +
        " edgeAlreadyPositive=" +
        a.edgeSuppressionDetail.alreadyPositiveFraction.toFixed(4) +
        " falseEvents=" +
        a.falseEvents +
        " falseRecent=" +
        a.falseEventTiming.recentUnmatchedFraction.toFixed(4) +
        " falseLinger=" +
        a.falseEventTiming.lingerFraction.toFixed(4) +
        " falseBackground=" +
        a.falseEventTiming.backgroundFraction.toFixed(4),
    );
  }
  console.log(
    "[v15N-D6-D2-D3-D1] supportPass=" +
      supportPass +
      " missFlags=" +
      JSON.stringify(missFlags) +
      " falseEventAxis=" +
      falseEventAxis +
      " outcome=" +
      outcome,
  ...
  const output = {
    schema: "maplefly.v15n-d6-d2-d3-d1.rising-edge-failure-attribution.1",
    preregistration: {
      path: "history/prereg_v15n_d6_d2_d3_d1.md",
      commit: V15N_D6_D2_D3_D1_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD6D2D3Receipt:
      ...
      v15nD6D2D3Outcome:
        "V15N_D6_D2_D3_CAUSAL_EVENTIZATION_NOT_DEMONSTRATED",
      authoritativeD3EvidenceSha256: d3Sha256,
    },
    cohorts: {
      train: V15N_TRAIN_BASE_SEEDS,
      eval: V15N_EVAL_BASE_SEEDS,
      holdout: V15N_D6_D2_D3_HOLDOUT_BASE_SEEDS,
    ...
    gates: {
      supportPass,
      attributionGate: V15N_D6_D2_D3_D1_ATTRIBUTION_GATE,
      missFlags,
      falseFlags,
      falseEventAxis,
    },
  ...
  const outDir = resolve(
    "results/v15n-d6-d2-d3-d1-rising-edge-failure-attribution",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d6_d2_d3_d1.json"),
