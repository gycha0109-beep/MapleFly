...
const TRAIN_BASE_SEEDS = [2920000, 2920100, 2920200, 2920300];
const EVAL_BASE_SEEDS = [2925000, 2925100, 2925200];
const REPLICATES = 6;
...
const L2 = 0.001;
const EPSILON = 1.00;
const ORDER_SEED_BASE = 2926000;
const POLICY_SEED = 2928000;

const PREREG_COMMIT = "c75371458c35575af16045852e16d3403a0a88aa";
...
async function main() {
  const outDir = resolve("results/train-v15b3-normalized-reward-potion");
  ...
  console.log(
    "[v15B3] frozen v15A12 representation sha256=" + frozen.digest,
  );
  ...
  const outcome = pass
    ? "V15B3_NORMALIZED_VALUE_LEARNING_PASS"
    : "V15B3_NORMALIZED_VALUE_LEARNING_FAIL";
  ...
  console.log(
    "[v15B3] model sha256=" + modelSha256 +
      " outcome=" + outcome +
      " v15C=" + (pass ? "PREREGISTRATION_AUTHORIZED" : "BLOCKED"),
  );

  const output = {
    schema: "maplefly.v15b3.normalized-reward-value-learning.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15b3.md",
  ...
  await writeFile(
    resolve(outDir, "v15b3_training.json"),
