#!/usr/bin/env node
// v15L: reward-only terminal-health shaping on long frozen neural history.
...
function normalizeVector(vector) {
  ...
  if (!(norm > 1e-12)) {
    throw new Error("v15L PCA vector collapsed");
  }
...
function traceSnapshots(tapes) {
  ...
  for (const tape of tapes) {
    if (tape.potionEvents.length !== 10) {
      throw new Error("v15L trace snapshot decision count mismatch");
    }
    for (const event of tape.potionEvents) {
      if (!Array.isArray(event.trace) || event.trace.length !== DN_COUNT) {
        throw new Error("v15L trace snapshot width mismatch");
      }
...
function fitTracePreprocessing(trainTapes) {
  ...
  if (raw.length !== 240) {
    throw new Error("v15L PCA TRAIN snapshot count mismatch " + raw.length);
  }
  ...
  const components = [];
  const eigenvalues = [];
  for (let component = 0; component < V15L_PCA_COMPONENTS; component += 1) {
    const random = mulberry32(V15L_PCA_SEED + component);
    ...
    for (let iteration = 0; iteration < V15L_PCA_ITERATIONS; iteration += 1) {
...
function attachTraceFeatures(tapes, preprocessing) {
  for (const tape of tapes) {
    ...
    if (
      ...
      tape.neuralFeatures.some((row) => row.length !== V15L_PCA_COMPONENTS)
    ) {
      throw new Error("v15L projected feature contract mismatch");
    }
  }
}
...
function policyActions(
  ...
  params,
  {
    neuralTraceOff = false,
    actionMemoryOff = false,
  } = {},
) {
  if (
    neuralSequence.length !== 10 ||
    params.length !== V15L_POLICY_PARAMS
  ) {
    throw new Error("v15L policy contract mismatch");
  }
  ...
  for (const rawNeural of neuralSequence) {
    if (rawNeural.length !== V15L_PCA_COMPONENTS) {
      throw new Error("v15L neural feature width mismatch");
    }
    let score = params[0];
    if (!neuralTraceOff) {
      for (let pc = 0; pc < V15L_PCA_COMPONENTS; pc += 1) {
        score += params[pc + 1] * rawNeural[pc];
      }
    }
    if (!actionMemoryOff) {
      for (let lag = 0; lag < 4; lag += 1) {
        score += params[V15L_PCA_COMPONENTS + 1 + lag] * actionHistory[lag];
      }
    }
...
function tapeContactSchedule(tape) {
  ...
  if (decisions.length !== 10) {
    throw new Error("v15L tape must contain exactly 10 decisions");
  }
...
function tapeNeuralSequence(tape) {
  if (
    ...
    throw new Error("v15L neural feature tape mismatch");
  }
...
function simulatePolicy(
  ...
  const { actions, scores } = policyActions(neuralSequence, params, options);
...
function trainCem(tapes) {
  const random = mulberry32(V15L_CEM_SEED);
  let distributionMean = new Float64Array(V15L_POLICY_PARAMS);
  let distributionStd = new Float64Array(V15L_POLICY_PARAMS).fill(1);
  const trace = [];

  for (
    let generation = 1;
    generation <= V15L_GENERATIONS;
    generation += 1
  ) {
    const population = [];
    for (let member = 0; member < V15L_POPULATION; member += 1) {
      const params = new Float64Array(V15L_POLICY_PARAMS);
      for (let i = 0; i < V15L_POLICY_PARAMS; i += 1) {
        params[i] = clamp(
          distributionMean[i] +
    ...
    const elites = population.slice(0, V15L_ELITES);
    const eliteMean = new Float64Array(V15L_POLICY_PARAMS);
    ...
    const eliteStd = new Float64Array(V15L_POLICY_PARAMS);
    for (const elite of elites) {
      for (let i = 0; i < V15L_POLICY_PARAMS; i += 1) {
    ...
    for (let i = 0; i < V15L_POLICY_PARAMS; i += 1) {
      ...
      distributionMean[i] = clamp(
        V15L_OLD_WEIGHT * distributionMean[i] +
          V15L_ELITE_WEIGHT * eliteMean[i],
        -V15L_PARAM_CLAMP,
        V15L_PARAM_CLAMP,
      );
      distributionStd[i] = Math.max(
        V15L_MIN_STD,
        V15L_OLD_WEIGHT * distributionStd[i] +
          V15L_ELITE_WEIGHT * eliteStd[i],
      );
    }
    if (
      generation === 1 ||
...
function oracleMinimumUses(tape) {
  ...
  if (!Number.isFinite(minimum)) {
    throw new Error("v15L oracle cannot survive tape " + tape.seed);
  }
...
function addEconomyMetrics(evaluation, oracle) {
  ...
  const perSeed = V15L_EVAL_BASE_SEEDS.map((baseSeed) => {
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15L DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(3847000);
  const trainTapes = await collectTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15L_TRAIN_BASE_SEEDS,
    interruptionRandom,
    label: "train",
  });
  const evalTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15L_EVAL_BASE_SEEDS,
    interruptionRandom,
    label: "eval",
  });
  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15L tape count mismatch");
  }
  ...
  const neuralTraceOff = addEconomyMetrics(
    evaluatePolicy(evalTapes, params, {
      neuralTraceOff: true,
    }),
    oracle,
  ...
  const neuralTraceContribution = contribution(
    full,
    neuralTraceOff,
    1.0,
  );
  ...
  const gateChecks = {
    ...
    neuralTraceContribution:
      neuralTraceContribution.contributes,
  ...
  console.log(
    "[v15L] TRACE_OFF survival=" +
      (neuralTraceOff.survivalRate * 100).toFixed(1) +
      "% excess=" +
      Number(neuralTraceOff.meanExcessUses).toFixed(3) +
      " contributes=" +
      neuralTraceContribution.contributes +
      " ACTION_OFF survival=" +
  ...
  const output = {
    ...
    preregistration: {
      path: "history/prereg_v15l.md",
      commit: V15L_PREREG_COMMIT,
    },
    evidenceChain: {
      v15kD2Closure:
    ...
    cohorts: {
      trainBaseSeeds: V15L_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15L_EVAL_BASE_SEEDS,
      trainTapes: trainTapes.length,
      evalTapes: evalTapes.length,
      interruptionRngSeed: 3847000,
    },
    representation: {
      type: "CAUSAL_EXPONENTIAL_DN_TRACE_PCA32",
      dnCount: DN_COUNT,
      frameSteps: POTION_FRAME_STEPS,
      frameSeconds: POTION_FRAME_STEPS * STEP_SECONDS,
      traceHalfLifeSeconds: V15L_TRACE_HALF_LIFE_SECONDS,
      traceDecay: V15L_TRACE_DECAY,
      traceUpdate:
        "trace=lambda*previous+(1-lambda)*baseline_relative_100ms_dn_frame",
      standardization: "TRAIN_ONLY_MEAN_STD_SCALE_FLOOR_1E-6",
      pca: {
        components: V15L_PCA_COMPONENTS,
        iterations: V15L_PCA_ITERATIONS,
        seedBase: V15L_PCA_SEED,
        labelsUsed: false,
    ...
    optimizer: {
      ...
      generations: V15L_GENERATIONS,
      population: V15L_POPULATION,
      elites: V15L_ELITES,
      oldWeight: V15L_OLD_WEIGHT,
      eliteWeight: V15L_ELITE_WEIGHT,
      minimumStd: V15L_MIN_STD,
      parameterClamp: [-V15L_PARAM_CLAMP, V15L_PARAM_CLAMP],
      seed: V15L_CEM_SEED,
      initialMean: Array(V15L_POLICY_PARAMS).fill(0),
      initialStd: Array(V15L_POLICY_PARAMS).fill(1),
      trainingFitness:
    ...
    policy: {
      parameterCount: V15L_POLICY_PARAMS,
      parameters: {
        bias: params[0],
        neuralWeights: Array.from(
          params.slice(1, V15L_PCA_COMPONENTS + 1),
        ),
        actionWeights: Array.from(
          params.slice(V15L_PCA_COMPONENTS + 1),
        ),
      },
      paramsSha256,
      runtimeInputs: [
    ...
    evaluation: {
      ...
      trainDecisionMean,
      contributions: {
        neuralTrace: neuralTraceContribution,
    ...
    pass: scientificPass,
    next: scientificPass
      ? "FREEZE_EVIDENCE_THEN_DEPLOYMENT_SPECIFIC_CLOSURE_PREREGISTRATION_AUTHORIZED"
      : "FREEZE_AND_DIAGNOSE",
    deployment: "BLOCKED",
    deployedPotion: "v15D",
