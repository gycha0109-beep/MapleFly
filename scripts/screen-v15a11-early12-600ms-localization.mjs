...
const TRAIN_BASE_SEEDS = [2880000, 2880100, 2880200, 2880300];
const EVAL_BASE_SEEDS = [2885000, 2885100, 2885200];
const REPLICATES = 6;
const EPOCHS = 240;
const LEARNING_RATE = 0.03;
const L2 = 0.001;
const THRESHOLD = 0.5;
const ORDER_SEED = 2886000;
const BOOTSTRAP_SEED = 2889000;
const BOOTSTRAP_ITERATIONS = 2000;
const MATERIAL_LOSS_MIN = 0.10;
const PREREG_COMMIT = "e60be014bff0604354d4b190dbbc9917a46c6440";
...
const REPRESENTATIONS = [
  { name: "R0_EARLY12_A", index: 0, startFrame: 0, frameCount: 12 },
  { name: "R1_EARLY6_A", index: 1, startFrame: 0, frameCount: 6 },
  { name: "R2_EARLY6_B", index: 2, startFrame: 6, frameCount: 6 },
];
...
async function collectDataset(connectome, dnSlot, baseSeeds, label) {
  ...
  for (const baseSeed of baseSeeds) {
    ...
    console.log("[v15A11-" + label + "] baseSeed=" + baseSeed + " rows=" + rows.length);
...
function fitRepresentation(trainAll, representation) {
  ...
  const halfCounts = representation.name === "R0_EARLY12_A"
    ? {
        early6A: selectedSlots.filter((slot) => slot.absoluteFrame < SUBHALF_FRAMES).length,
        early6B: selectedSlots.filter((slot) => slot.absoluteFrame >= SUBHALF_FRAMES).length,
      }
    : null;
...
function vector(row, fitted, startFrame, dnPermutation = null, maskHalf = null) {
  ...
  for (let i = 0; i < TOP_FEATURES; i += 1) {
    ...
    const masked =
      (maskHalf === "EARLY6_A" && absoluteFrame < SUBHALF_FRAMES) ||
      (maskHalf === "EARLY6_B" && absoluteFrame >= SUBHALF_FRAMES);
    if (masked) {
      out[i] = 0;
      continue;
    }
...
async function main() {
  const outDir = resolve("results/screen-v15a11-early12-600ms-localization");
  ...
  for (const representation of REPRESENTATIONS) {
    ...
    console.log(
      "[v15A11] " + representation.name + " sha256=" + fitted.sha256 +
    ...
    for (const task of TASKS) {
      ...
      console.log(
        "[v15A11] " + representation.name + " " + task.name +
  ...
  const r0Fitted = fittedByName.get("R0_EARLY12_A");
  const maskResults = [];
  for (const maskHalf of ["EARLY6_A", "EARLY6_B"]) {
  ...
  const firstMask = maskResults.find((row) => row.maskHalf === "EARLY6_A");
  const secondMask = maskResults.find((row) => row.maskHalf === "EARLY6_B");
  ...
  if (!baselineReplicated) {
    ...
    if (r1TaskA.gatePass) findings.push("EARLY6_A_SUFFICIENT");
    if (r2TaskA.gatePass) findings.push("EARLY6_B_SUFFICIENT");
    if (!r1TaskA.gatePass && !r2TaskA.gatePass) findings.push("NO_SINGLE_600MS_HALF_SUFFICIENT");
    if (firstMask.materialDecoderContribution) findings.push("EARLY6_A_DECODER_CONTRIBUTES");
    if (secondMask.materialDecoderContribution) findings.push("EARLY6_B_DECODER_CONTRIBUTES");
    if (firstMask.materialDecoderContribution && secondMask.materialDecoderContribution) {
      findings.push("BOTH_600MS_HALVES_DECODER_CONTRIBUTE");
    }
    if (r1TaskA.gatePass || r2TaskA.gatePass) {
      findings.push("PRIOR_V15A8_SINGLE_600MS_WINDOWS_FAILED");
      outcome = "600MS_COHORT_INSTABILITY";
    } else if (firstMask.materialDecoderContribution && secondMask.materialDecoderContribution) {
      findings.push("EARLY12_DISTRIBUTED_ACROSS_600MS_HALVES");
      outcome = "EARLY12_DISTRIBUTED_ACROSS_600MS_HALVES";
    } else {
      findings.push("EARLY12_SUFFICIENT_NO_600MS_SUFFICIENCY");
      outcome = "EARLY12_SUFFICIENT_NO_600MS_SUFFICIENCY";
    }
  }

  console.log(
    "[v15A11] outcome=" + outcome +
    " findings=" + (findings.join(",") || "none") +
    " v15B=BLOCKED",
  );

  const output = {
    schema: "maplefly.v15a11.early12-600ms-localization.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15a11.md",
  ...
  await writeFile(
    resolve(outDir, "v15a11_screen.json"),
