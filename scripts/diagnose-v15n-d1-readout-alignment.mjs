...
function cohortDiagnostic(tapes, params, meanNeuralScores) {
  const fullEval = evaluatePolicy(tapes, params);
  const rows = [];

  for (let tapeIndex = 0; tapeIndex < tapes.length; tapeIndex += 1) {
    const tape = tapes[tapeIndex];
    const policyRow = fullEval.rows[tapeIndex];
    for (
      let decisionIndex = 0;
      decisionIndex < policyRow.trajectory.length;
      decisionIndex += 1
    ) {
      const trajectory = policyRow.trajectory[decisionIndex];
      const h = policyRow.policyStates[decisionIndex];
      const neural = neuralTerm(
        tape.neuralFeatures[decisionIndex],
        params,
      );
      const state =
        params[39] * h[0] + params[40] * h[1];
      const bias = params[0];
      const total = bias + neural + state;
      const action = total > 0 ? 1 : 0;
      if (action !== policyRow.policyActions[decisionIndex]) {
        throw new Error("v15N-D1 score decomposition mismatch");
      }
      ...
      const meanTotal =
        bias + meanNeuralScores[decisionIndex] + state;
      const meanAction = meanTotal > 0 ? 1 : 0;
      const forcedTarget =
        oracleClass === "FORCED_DRINK"
          ? 1
          : oracleClass === "FORCED_WAIT"
            ? 0
            : null;
      rows.push({
        seed: tape.seed,
        ...
        bias,
        neuralTerm: neural,
        actionStateTerm: state,
        totalScore: total,
        action,
        meanNeuralTerm: meanNeuralScores[decisionIndex],
        residual: neural - meanNeuralScores[decisionIndex],
        pairedMeanTotalScore: meanTotal,
        pairedMeanAction: meanAction,
        fullCorrect:
          forcedTarget === null ? null : action === forcedTarget,
        pairedMeanCorrect:
          forcedTarget === null ? null : meanAction === forcedTarget,
        h,
      });
    }
  }
  ...
  const forced = rows.filter(
    (row) =>
      row.oracleClass === "FORCED_WAIT" ||
      row.oracleClass === "FORCED_DRINK",
  );
  const waitSupport = forced.filter(
    (row) => row.oracleClass === "FORCED_WAIT",
  ).length;
  const drinkSupport = forced.filter(
    (row) => row.oracleClass === "FORCED_DRINK",
  ).length;

  const fullCorrect = forced.filter((row) => row.fullCorrect).length;
  const meanCorrect = forced.filter(
    (row) => row.pairedMeanCorrect,
  ).length;
  const flips = forced.filter(
    (row) => row.action !== row.pairedMeanAction,
  );
  const correcting = flips.filter(
    (row) => !row.fullCorrect && row.pairedMeanCorrect,
  ).length;
  const harmful = flips.filter(
    (row) => row.fullCorrect && !row.pairedMeanCorrect,
  ).length;
  ...
  const byDecision = Array.from({ length: 10 }, (_, decisionIndex) => {
    ...
    const forcedRows = all.filter(
      (row) =>
        row.oracleClass === "FORCED_WAIT" ||
        row.oracleClass === "FORCED_DRINK",
    );
    const w = forcedRows.filter(
      (row) => row.oracleClass === "FORCED_WAIT",
    ).length;
    const d = forcedRows.filter(
      (row) => row.oracleClass === "FORCED_DRINK",
    ).length;
    const localAuc = w >= 5 && d >= 5 ? auc(forcedRows) : null;
    const uniqueStates = new Set(
      all.map((row) =>
        row.h.map((value) => value.toFixed(6)).join(","),
      ),
    ).size;
    return {
      decisionIndex,
      ...
      forcedDrink: d,
      auc: localAuc,
      meanAbsNeuralTerm: meanAbs(all, "neuralTerm"),
      meanAbsActionStateTerm: meanAbs(all, "actionStateTerm"),
      meanAbsResidual: meanAbs(all, "residual"),
      actionStateToNeuralMagnitudeRatio:
        meanAbs(all, "neuralTerm") > 0
          ? meanAbs(all, "actionStateTerm") /
            meanAbs(all, "neuralTerm")
          : null,
      uniqueRoundedActionStates: uniqueStates,
      fullAgreement:
        forcedRows.length
          ? forcedRows.filter((row) => row.fullCorrect).length /
            forcedRows.length
          : null,
      pairedMeanAgreement:
        forcedRows.length
          ? forcedRows.filter((row) => row.pairedMeanCorrect).length /
            forcedRows.length
          : null,
    };
  });
  ...
  return {
    ...
    residualAuc: auc(forced),
    fullForcedAgreement:
      forced.length ? fullCorrect / forced.length : null,
    pairedMeanForcedAgreement:
      forced.length ? meanCorrect / forced.length : null,
    pairedMeanAgreementGain:
      forced.length ? (meanCorrect - fullCorrect) / forced.length : null,
    flips: {
      total: flips.length,
      waitToDrink: flips.filter(
        (row) => row.action === 0 && row.pairedMeanAction === 1,
      ).length,
      drinkToWait: flips.filter(
        (row) => row.action === 1 && row.pairedMeanAction === 0,
      ).length,
      correcting,
      harmful,
    },
...
async function main() {
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D1 evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D1 v14C provenance mismatch");
  }
  const attackPolicy = {
  ...
  if (
    ...
    throw new Error("v15N-D1 DN identity/order mismatch");
  }
  ...
  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15N-D1 tape count mismatch");
  }
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D1 preprocessing SHA mismatch");
  }
  ...
  const meanNeuralVectors = Array.from(
    { length: 10 },
    (_, decision) =>
      Array.from(
        { length: V15N_PCA_COMPONENTS },
        (_, pc) =>
          mean(
            trainTapes.map(
              (tape) => tape.neuralFeatures[decision][pc],
            ),
          ),
      ),
  );
  const reproducedMean = addEconomyMetrics(
    evaluatePolicy(
      evalTapes,
      params,
      {},
      evalTapes.map(() => meanNeuralVectors),
    ),
    evalOracle,
  );
  ...
  verifyAggregate(
    reproducedMean,
    artifact.evaluation.decisionMeanNeural,
    "DECISION_MEAN_NEURAL",
  );

  const meanNeuralScores =
    trainDecisionMeanNeuralScores(trainTapes, params);
  const train = cohortDiagnostic(
    trainTapes,
    params,
    meanNeuralScores,
  );
  const evalDiag = cohortDiagnostic(
    evalTapes,
    params,
    meanNeuralScores,
  );
  ...
  const supportPass =
    train.forcedSupport.wait >= V15N_D1_MIN_FORCED_SUPPORT &&
    train.forcedSupport.drink >= V15N_D1_MIN_FORCED_SUPPORT &&
    evalDiag.forcedSupport.wait >= V15N_D1_MIN_FORCED_SUPPORT &&
    evalDiag.forcedSupport.drink >= V15N_D1_MIN_FORCED_SUPPORT;

  let outcome;
  if (!ecologyPass) {
    outcome = "V15N_D1_IMPLEMENTATION_OR_PROVENANCE_INVALID";
  } else if (!supportPass) {
    outcome = "V15N_D1_INSUFFICIENT_FORCED_STATE_SUPPORT";
  } else if (
    train.residualAuc >= 0.60 &&
    evalDiag.residualAuc <= 0.40
  ) {
    outcome = "V15N_D1_NEURAL_READOUT_CROSS_COHORT_REVERSAL";
  } else if (
    evalDiag.residualAuc <= 0.40 &&
    evalDiag.pairedMeanAgreementGain >= 0.10
  ) {
    outcome = "V15N_D1_EVAL_NEURAL_RESIDUAL_HARMFUL";
  } else if (
    evalDiag.residualAuc >= 0.45 &&
    evalDiag.residualAuc <= 0.55 &&
    evalDiag.pairedMeanAgreementGain >= 0.10
  ) {
    outcome = "V15N_D1_EPISODE_SPECIFIC_READOUT_UNINFORMATIVE";
  } else if (
    evalDiag.residualAuc >= 0.60 &&
    evalDiag.pairedMeanAgreementGain >= 0.10
  ) {
    outcome = "V15N_D1_DOWNSTREAM_SCORE_INTERACTION";
  } else {
    outcome = "V15N_D1_INCONCLUSIVE";
  }

  console.log(
    "[v15N-D1] TRAIN forced wait=" +
      train.forcedSupport.wait +
      " drink=" +
      train.forcedSupport.drink +
      " residualAUC=" +
      train.residualAuc.toFixed(4) +
      " fullAgreement=" +
      (train.fullForcedAgreement * 100).toFixed(1) +
      "% meanAgreement=" +
      (train.pairedMeanForcedAgreement * 100).toFixed(1) +
      "%",
  );
  console.log(
    "[v15N-D1] EVAL forced wait=" +
      evalDiag.forcedSupport.wait +
      " drink=" +
      evalDiag.forcedSupport.drink +
      " residualAUC=" +
      evalDiag.residualAuc.toFixed(4) +
      " fullAgreement=" +
      (evalDiag.fullForcedAgreement * 100).toFixed(1) +
      "% meanAgreement=" +
      (evalDiag.pairedMeanForcedAgreement * 100).toFixed(1) +
      "% gain=" +
      (evalDiag.pairedMeanAgreementGain * 100).toFixed(1) +
      "pp",
  );
  console.log(
    "[v15N-D1] EVAL flips=" +
      evalDiag.flips.total +
      " correcting=" +
      evalDiag.flips.correcting +
      " harmful=" +
      evalDiag.flips.harmful +
      " state/neural=" +
      evalDiag.magnitude.actionStateToNeuralMagnitudeRatio.toFixed(4) +
      " outcome=" +
      outcome,
  );

  const output = {
    schema: "maplefly.v15n-d1.frozen-readout-alignment.1",
    preregistration: {
      path: "history/prereg_v15n_d1.md",
      commit: V15N_D1_PREREG_COMMIT,
    },
    frozenCandidate: {
    ...
    provenanceReproduction: {
      ...
      decisionMeanNeural: {
        survivalRate: reproducedMean.survivalRate,
        minSeedSurvivalRate: reproducedMean.minSeedSurvivalRate,
        meanUses: reproducedMean.meanUses,
        meanExcessUses: reproducedMean.meanExcessUses,
        wastedHealingPerDrink:
          reproducedMean.wastedHealingPerDrink,
      },
      pass: true,
    },
    ecology: {
  ...
  const outDir = resolve("results/v15n-d1-readout-alignment");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d1.json"),
