...
const FROZEN_CANDIDATE_PATH =
  "src/brain/fly-skill-v11g3-jump-candidate.json";

function policyFromFrozenCandidate(
  frozen,
) {
  if (
    frozen.provenance.runId !==
      35726010847 ||
    ...
    frozen.candidate
        .persistenceWindows !==
      PERSISTENCE_WINDOWS ||
    frozen.policy.cooldownSteps !==
      JUMP_COOLDOWN_STEPS
  ) {
    throw new Error(
      "frozen G3 candidate contract mismatch",
    );
  }
...
async function main() {
  ...
  const replicationTargetJumps =
    perRun.reduce(
      (sum, run) =>
        sum +
        run.TARGET_ONLY.rows.filter(
          (row) => row.anyJump,
        ).length,
      0,
    );
  ...
  const replicationTargetEpisodes =
    perRun.reduce(
      (sum, run) =>
        sum +
        run.TARGET_ONLY.episodes,
      0,
    );
  const pooledTargetJumpRate =
    (
      originalTargetJumps +
      replicationTargetJumps
    ) /
    (
      originalTargetEpisodes +
      replicationTargetEpisodes
    );
  ...
  const output = {
    meta: {
      schema:
        "maplefly.experiment-v11g3r.one-shot-frozen-replication.1",
      phase: "G3R",
      brainRepository:
        SOURCE.repository,
      brainCommit:
      ...
      finalDistances:
        FINAL_DISTANCES,
      oneShotReplication: true,
      antiOptionalStopping:
        "exactly one independent replication; no further seed reruns if this gate fails",
    },
    summary,
    perRun,
  };
