...
const FINAL_BASE_SEEDS = [951000, 961000, 971000];
...
const CANDIDATE_PATH = resolve(
  "src/brain/fly-skill-v11d-jump-candidate.json",
);
...
class FrozenPersistentOutcomePolicy {
  constructor(candidate) {
    ...
    this.indices = candidate.classifier.featureIndices;
    this.means = candidate.classifier.means;
    this.scales = candidate.classifier.scales;
    this.weights = candidate.classifier.weights;
    this.bias = candidate.classifier.bias;
    this.threshold = candidate.classifier.threshold;
    this.positiveStreak = 0;

    if (
      this.indices.length !== 128 ||
      this.means.length !== 128 ||
      this.scales.length !== 128 ||
      this.weights.length !== 128
    ) {
      throw new Error("v11D candidate sparse shape mismatch");
    }

    if (this.threshold !== 0.5) {
      throw new Error(
        "v11D candidate threshold drift: " +
          this.threshold,
      );
    }
  }
  ...
  probability(feature, permutation = null) {
    ...
    for (
      let slot = 0;
      slot < this.indices.length;
      slot += 1
    ) {
      const selectedDn = this.indices[slot];
      const sourceDn = permutation
        ? permutation[selectedDn]
        : selectedDn;
      const standardized = clamp(
        (feature[sourceDn] - this.means[slot]) /
          this.scales[slot],
        -5,
        5,
      );
  ...
  choose(
    ...
    const positive =
      this.probability(feature, permutation) >=
      this.threshold;
...
function validateCandidate(candidate) {
  if (
    candidate.schema !==
    "maplefly.fly-jump-candidate.v11d.1"
  ) {
    throw new Error(
  ...
  if (
    ...
    "2c9daa3320d597e417217e5ccb2dcc2ffeb56953"
  ) {
  ...
  if (
    ...
    "3709991bed590ecfeea023a0229a27ce228deade0a830ff20db1ee53455ffd9e"
  ) {
    throw new Error(
...
async function main() {
  const outDir = resolve("results/experiment-v11e");
  ...
  const output = {
    meta: {
      schema:
        "maplefly.experiment-v11e.frozen-persistence.1",
      phase: "E",
      ...
      sourceCandidate: {
        path: "src/brain/fly-skill-v11d-jump-candidate.json",
  ...
  await writeFile(
    resolve(outDir, "experiment_v11e.json"),
