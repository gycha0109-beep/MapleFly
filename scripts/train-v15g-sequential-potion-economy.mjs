...
function trainSequentialPolicy(tapes) {
  const theta = new Float64Array(5);
  const random = mulberry32(V15G_POLICY_SEED);
  const trace = [];

  for (let epoch = 0; epoch < V15G_EPOCHS; epoch += 1) {
    const batch = tapes.map((tape) =>
      simulateSequentialPolicy(tape, theta, {
        random,
        stochastic: true,
      }),
    );
    const utilities = batch.map((row) => row.utility);
    const baseline = mean(utilities);
    const variance = mean(
      utilities.map((value) => (value - baseline) ** 2),
    );
    const scale = Math.max(Math.sqrt(variance), 1);
    const gradient = new Float64Array(5);

    for (const row of batch) {
      const advantage = (row.utility - baseline) / scale;
      for (const transition of row.trajectory) {
        const residual =
          transition.action - transition.probability;
        const feature = [1, ...transition.history];
        for (let i = 0; i < 5; i += 1) {
          gradient[i] +=
            advantage * residual * feature[i];
        }
      }
    }

    for (let i = 0; i < 5; i += 1) {
      const averaged = gradient[i] / tapes.length;
      theta[i] = clamp(
        theta[i] +
          V15G_LEARNING_RATE *
            (averaged - V15G_L2 * theta[i]),
        -5,
        5,
      );
    }
    ...
    if (
      ...
      trace.push({
        epoch: epoch + 1,
        meanUtility: baseline,
        survivalRate: mean(
          batch.map((row) => Number(row.survived)),
        ),
        meanUses: mean(batch.map((row) => row.uses)),
...
function evaluateSequential(tapes, theta, options = {}) {
  const rows = tapes.map((tape) =>
    simulateSequentialPolicy(tape, theta, options),
  );
  return {
    rows,
    survivalRate: mean(
  ...
  for (const baseSeed of baseSeeds) {
    for (const episode of makeBlock(baseSeed)) {
      ...
      if (row.liveSteps !== MAX_STEPS || row.potionEvents.length !== 10) {
        throw new Error("v15G incomplete trainer tape");
      }
      rows.push(row);
      console.log(
        "[v15G-" + label + "] seed=" + row.seed +
...
async function main() {
  ...
  if (
    ...
    throw new Error("v15G DN identity/order mismatch");
  }
  ...
  if (trainTapes.length !== 24 || evalTapes.length !== 24) {
    throw new Error("v15G tape count mismatch");
  }
  ...
  const trained = trainSequentialPolicy(trainTapes);
  const theta = trained.theta;
  const thetaSha256 = createHash("sha256")
    .update(JSON.stringify(Array.from(theta)))
    .digest("hex");

  const full = evaluateSequential(evalTapes, theta);
  const historyOff = evaluateSequential(evalTapes, theta, {
    historyOff: true,
  });
  const neuralOff = evaluateSequential(evalTapes, theta, {
    neuralOff: true,
  });
  ...
  const survivorRows = full.rows.filter((row) => row.survived);
  const meanExcessUses = survivorRows.length
    ? mean(
        survivorRows.map(
  ...
  const perSeed = V15G_EVAL_BASE_SEEDS.map((baseSeed) => {
    const rows = full.rows.filter((row) => row.baseSeed === baseSeed);
    return {
      baseSeed,
      survivalRate: mean(
        rows.map((row) => Number(row.survived)),
  ...
  const minSeedSurvivalRate = Math.min(
    ...perSeed.map((row) => row.survivalRate),
  );
  const historyMargin =
    full.meanUtility - historyOff.meanUtility;
  const neuralMargin =
    full.meanUtility - neuralOff.meanUtility;

  const gateChecks = {
    tapeValidity: tapeValid,
    survivalRate: full.survivalRate >= 0.75,
    minSeedSurvival: minSeedSurvivalRate >= 0.625,
    meanExcessUses: meanExcessUses <= V15G_EXCESS_USES_MAX,
    wastedHealingPerDrink:
      full.wastedHealingPerDrink <= 10,
    historyContribution:
      historyMargin >= V15G_HISTORY_MARGIN_MIN,
    neuralContribution:
      neuralMargin >= V15G_NEURAL_MARGIN_MIN,
  };
  ...
  console.log(
    "[v15G] FULL survival=" +
      ...
      "% minSeed=" +
      (minSeedSurvivalRate * 100).toFixed(1) +
      "% uses=" +
      full.meanUses.toFixed(3) +
      " excess=" +
      meanExcessUses.toFixed(3) +
      " U=" +
      full.meanUtility.toFixed(3),
  );
  console.log(
    "[v15G] HISTORY_OFF U=" +
      historyOff.meanUtility.toFixed(3) +
      " margin=" +
      historyMargin.toFixed(3) +
      " NEURAL_OFF U=" +
      neuralOff.meanUtility.toFixed(3) +
      " margin=" +
      neuralMargin.toFixed(3),
  );
  console.log(
    "[v15G] theta=" +
      JSON.stringify(Array.from(theta)) +
      " sha256=" +
      thetaSha256 +
      " outcome=" +
      outcome,
  ...
  const output = {
    schema: "maplefly.v15g.sequential-potion-economy.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15g.md",
      commit: V15G_PREREG_COMMIT,
    },
    frozenNeuralCore: {
    ...
    learner: {
      type: "FROZEN_NEURAL_MARGIN_PLUS_FOUR_ACTION_MEMORY",
      trainableParameters: 5,
      epochs: V15G_EPOCHS,
      learningRate: V15G_LEARNING_RATE,
      l2: V15G_L2,
      parameterClamp: [-5, 5],
      policySeed: V15G_POLICY_SEED,
      survivalBonus: V15G_SURVIVAL_BONUS,
      utility: "165*survived - 15*potionUses",
      theta: Array.from(theta),
      thetaSha256,
      trace: trained.trace,
    },
    ...
    pass: scientificPass,
    next: scientificPass
      ? "FRESH_INTERACTIVE_48S_REMEDIATION_PREREGISTRATION_AUTHORIZED"
      : "FREEZE_AND_DIAGNOSE",
    deployment: "BLOCKED",
    v16c180SecondEndurance: "BLOCKED",
  };

  const outDir = resolve("results/v15g-sequential-potion-economy");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15g_training.json"),
