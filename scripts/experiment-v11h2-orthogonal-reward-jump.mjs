    ...
    if (obstacle && obstacleVisual) {
      ...
      if (!obstaclePassed) {
        ...
        drive["LC16_" + obstacle.side] =
          obstacleLooming;
        drive["LC22_" + obstacle.side] =
          obstacleLooming;
        drive["LPLC4_" + obstacle.side] =
          obstacleLooming;
      }
    }
...
async function main() {
  ...
  const obstaclePopulationCounts = {};
  for (const channel of [
    ...
    obstaclePopulationCounts[channel] = {};
    for (const side of ["L", "R"]) {
      ...
      obstaclePopulationCounts[channel][side] =
        group.length;
    }
  }
  ...
  console.log(
    "V11H2-SUPPORT=" +
      ...
      " WAIT+=" +
      support.WAIT.positive +
      " WAIT-=" +
      support.WAIT.negative +
      " JUMP+=" +
      support.JUMP.positive +
      " JUMP-=" +
      support.JUMP.negative,
  );
  ...
  if (!supportGate) {
    const output = {
      meta: {
        schema:
          "maplefly.experiment-v11h2.orthogonal-sensory-reward-only.1",
        phase: "H2",
        brainRepository:
          SOURCE.repository,
        brainCommit:
        ...
        sensoryArchitecture: {
          target:
            "v10F target sensory unchanged; matched target geometry",
          obstacle:
            "equal LC6/LC16/LC22/LPLC4 looming drive; obstacle LC4 disabled",
          obstaclePopulationCounts,
        },
        supportGate: false,
      },
      practice: {
    ...
    console.log(
      "V11H2-JUMP-GATE=FAIL reason=practice_support",
    );
    return;
  }
  ...
  const output = {
    meta: {
      ...
      policy: {
        type:
          "two class-balanced logistic desirable-outcome models",
        threshold:
          POLICY_THRESHOLD,
        persistenceWindows:
          PERSISTENCE_WINDOWS,
