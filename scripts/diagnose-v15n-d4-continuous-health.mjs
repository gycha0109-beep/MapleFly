...
function allReachableRegressionStates(tapes, params) {
  const rows = [];
  for (const tape of tapes) {
    for (let decisionIndex = 0; decisionIndex < 10; decisionIndex += 1) {
      ...
      for (let mask = 0; mask < 2 ** decisionIndex; mask += 1) {
        ...
        rows.push({
          ...
          hpBefore,
          neural,
          h: recurrentStateForHistory(history, params),
        });
      }
    }
  }
...
function regressionFeatureVector(row, featureSet) {
  ...
  throw new Error("unknown v15N-D4 feature set " + featureSet);
}

function fitRegressionStandardization(rows, featureSet) {
  const first = regressionFeatureVector(rows[0], featureSet);
  const means = first.map((_, index) =>
    mean(rows.map((row) => regressionFeatureVector(row, featureSet)[index])),
  );
  const scales = first.map((_, index) => {
    const variance = mean(
      rows.map((row) => {
        const delta =
          regressionFeatureVector(row, featureSet)[index] - means[index];
        return delta * delta;
      }),
    );
...
function regressionStandardizedFeatures(row, featureSet, standardization) {
  const raw = regressionFeatureVector(row, featureSet);
  return raw.map(
    (value, index) =>
...
function solveRegressionLinearSystem(matrix, vector) {
  ...
  for (let col = 0; col < n; col += 1) {
    ...
    if (Math.abs(a[pivot][col]) < 1e-12) {
      throw new Error("v15N-D4 singular ridge system");
    }
    if (pivot !== col) {
      const temp = a[col];
      a[col] = a[pivot];
...
function fitHpRegression(rows, featureSet) {
  const standardization =
    fitRegressionStandardization(rows, featureSet);
  const dimension =
    regressionFeatureVector(rows[0], featureSet).length + 1;
  ...
  for (const row of rows) {
    const x = [
      ...regressionStandardizedFeatures(
        row,
        featureSet,
        standardization,
      ),
  ...
  for (let i = 0; i < dimension - 1; i += 1) {
    normal[i][i] += V15N_D4_RIDGE_LAMBDA;
  }

  return {
    featureSet,
    lambda: V15N_D4_RIDGE_LAMBDA,
    standardization,
    weights: solveRegressionLinearSystem(normal, rhs),
  };
}

function predictHp(row, model) {
  const x = [
    ...regressionStandardizedFeatures(
      row,
      model.featureSet,
...
function hpStandardDeviation(rows) {
  const center = mean(rows.map((row) => row.hpBefore));
  return Math.sqrt(
    mean(rows.map((row) => (row.hpBefore - center) ** 2)),
  );
}

function regressionMetrics(rows, model) {
  const actual = rows.map((row) => row.hpBefore);
  const predicted = rows.map((row) => predictHp(row, model));
  ...
  for (let i = 0; i < rows.length; i += 1) {
    if (!byTape.has(rows[i].seed)) byTape.set(rows[i].seed, []);
    byTape.get(rows[i].seed).push({
      actual: actual[i],
  ...
  for (const [seed, entries] of byTape) {
    const localMean = mean(entries.map((entry) => entry.actual));
    const localSse = entries.reduce(
      (sum, entry) => sum + (entry.actual - entry.predicted) ** 2,
      0,
    );
    const localSst = entries.reduce(
      (sum, entry) => sum + (entry.actual - localMean) ** 2,
      0,
    );
    perTape.push({
      seed,
      states: entries.length,
      r2: localSst > 0 ? 1 - localSse / localSst : null,
      mae: mean(
        entries.map((entry) =>
  ...
  return {
    states: rows.length,
    hpMean: actualMean,
    hpStandardDeviation: hpStandardDeviation(rows),
...
async function main() {
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D4 evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D4 v14C provenance mismatch");
  }
  const attackPolicy = {
  ...
  if (
    ...
    throw new Error("v15N-D4 DN identity/order mismatch");
  }
  ...
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D4_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(V15N_D4_HOLDOUT_INTERRUPTION_SEED),
    label: "d4-holdout",
  });
  if (
    ...
    throw new Error("v15N-D4 tape count mismatch");
  }
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D4 preprocessing SHA mismatch");
  }
  ...
  const trainStates = allReachableRegressionStates(trainTapes, params);
  const evalStates = allReachableRegressionStates(evalTapes, params);
  const holdoutStates = allReachableRegressionStates(holdoutTapes, params);

  const supportPass =
    trainStates.length >= V15N_D4_MIN_STATES &&
    evalStates.length >= V15N_D4_MIN_STATES &&
    holdoutStates.length >= V15N_D4_MIN_STATES &&
    hpStandardDeviation(trainStates) >= V15N_D4_MIN_HP_SD &&
    hpStandardDeviation(evalStates) >= V15N_D4_MIN_HP_SD &&
    hpStandardDeviation(holdoutStates) >= V15N_D4_MIN_HP_SD;

  const featureSets = ["FULL34", "NEURAL32", "ACTION_STATE2"];
  const models = {};
  const metrics = {};
  for (const featureSet of featureSets) {
    const model = fitHpRegression(trainStates, featureSet);
    models[featureSet] = {
    ...
    metrics[featureSet] = {
      train: regressionMetrics(trainStates, model),
      eval: regressionMetrics(evalStates, model),
      holdout: regressionMetrics(holdoutStates, model),
    };
  }

  const fullStable =
    metrics.FULL34.eval.r2 >= 0.40 &&
    metrics.FULL34.holdout.r2 >= 0.40 &&
    metrics.FULL34.eval.mae <= 15 &&
    metrics.FULL34.holdout.mae <= 15;
  const neuralAddsHealth =
    metrics.FULL34.eval.r2 - metrics.ACTION_STATE2.eval.r2 >= 0.10 &&
    metrics.FULL34.holdout.r2 - metrics.ACTION_STATE2.holdout.r2 >= 0.10;
  const fullClearlyWeak =
    metrics.FULL34.eval.r2 <= 0.15 &&
    metrics.FULL34.holdout.r2 <= 0.15 &&
    metrics.FULL34.eval.mae >= 20 &&
    metrics.FULL34.holdout.mae >= 20;

  let outcome;
  if (!ecologyPass) {
    outcome = "V15N_D4_IMPLEMENTATION_OR_PROVENANCE_INVALID";
  } else if (!supportPass) {
    outcome = "V15N_D4_INSUFFICIENT_CONTINUOUS_HEALTH_SUPPORT";
  } else if (fullStable && neuralAddsHealth) {
    outcome =
      "V15N_D4_CURRENT_HEALTH_RECOVERABLE_WITH_NEURAL_CONTRIBUTION";
  } else if (fullStable) {
    outcome =
      "V15N_D4_CURRENT_HEALTH_RECOVERABLE_ACTION_STATE_DOMINANT";
  } else if (fullClearlyWeak) {
    outcome = "V15N_D4_CURRENT_HEALTH_NOT_LINEARLY_RECOVERABLE";
  } else {
    outcome = "V15N_D4_INCONCLUSIVE";
  }
  ...
  for (const featureSet of featureSets) {
    console.log(
      "[v15N-D4] " +
        featureSet +
        " train R2=" +
        metrics[featureSet].train.r2.toFixed(4) +
        " MAE=" +
        metrics[featureSet].train.mae.toFixed(2) +
        " eval R2=" +
        metrics[featureSet].eval.r2.toFixed(4) +
        " MAE=" +
        metrics[featureSet].eval.mae.toFixed(2) +
        " holdout R2=" +
        metrics[featureSet].holdout.r2.toFixed(4) +
        " MAE=" +
        metrics[featureSet].holdout.mae.toFixed(2),
    );
  }
  console.log(
    "[v15N-D4] states=" +
      ...
      evalStates.length +
      "/" +
      holdoutStates.length +
      " hpSD=" +
      hpStandardDeviation(trainStates).toFixed(2) +
      "/" +
      hpStandardDeviation(evalStates).toFixed(2) +
      "/" +
      hpStandardDeviation(holdoutStates).toFixed(2) +
      " fullStable=" +
      fullStable +
      " neuralAddsHealth=" +
      neuralAddsHealth +
  ...
  const output = {
    schema: "maplefly.v15n-d4.continuous-current-health.1",
    preregistration: {
      path: "history/prereg_v15n_d4.md",
      commit: V15N_D4_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD3Receipt:
    ...
    cohorts: {
      ...
      holdoutBaseSeeds: V15N_D4_HOLDOUT_BASE_SEEDS,
      originalInterruptionRngSeed: 4147000,
      holdoutInterruptionRngSeed:
        V15N_D4_HOLDOUT_INTERRUPTION_SEED,
      trainStates: trainStates.length,
      evalStates: evalStates.length,
      holdoutStates: holdoutStates.length,
      hpStandardDeviation: {
        train: hpStandardDeviation(trainStates),
        eval: hpStandardDeviation(evalStates),
        holdout: hpStandardDeviation(holdoutStates),
      },
    },
    target: {
      name: "current HP before current POTION decision",
      evaluatorOnly: true,
      runtimeInput: false,
    },
    regressionContract: {
      ridgeLambda: V15N_D4_RIDGE_LAMBDA,
      interceptRegularized: false,
      equalStateWeight: true,
      diagnosticWeightsDeployable: false,
    },
    ...
    diagnosticRegressionWeightsDeployable: false,
    deployment: "BLOCKED",
    deployedPotion: "v15D",
    v16c: "BLOCKED",
  };

  const outDir = resolve("results/v15n-d4-continuous-health");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d4.json"),
