...
async function main() {
  ...
  const d1ArtifactPath = process.env.V15N_D6_D2_D1_ARTIFACT_FILE;
  if (!d1ArtifactPath) {
    throw new Error("V15N_D6_D2_D1_ARTIFACT_FILE is required");
  }
  const d1Bytes = await readFile(d1ArtifactPath);
  const d1Sha256 = createHash("sha256").update(d1Bytes).digest("hex");
  if (d1Sha256 !== V15N_D6_D2_D2_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D2 authoritative D2-D1 evidence SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2-D2 prerequisite outcome mismatch");
  }
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D6-D2-D2 v15N evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D6-D2-D2 DN identity/order mismatch");
  }
  ...
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D6_D2_D2_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(
      V15N_D6_D2_D2_HOLDOUT_INTERRUPTION_SEED,
    ),
    label: "d6-d2-d2-holdout",
  });
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D6-D2-D2 frozen preprocessing mismatch");
  }
  ...
  const detector = trainD6ImpactDirection(trainDetectorSet.examples);
  const detectorWeightsSha256 = createHash("sha256")
    .update(JSON.stringify(Array.from(detector.weights)))
    .digest("hex");
  if (
    detectorWeightsSha256 !==
    frozenD1.detectorReproduction.weightsSha256
  ) {
    throw new Error("v15N-D6-D2-D2 detector weight SHA mismatch");
  }
  d1AssertClose(
    detector.threshold,
  ...
  const trainEventMetrics = d2EventMetrics(trainTapes);
  const evalEventMetrics = d2EventMetrics(evalTapes);
  d2d1VerifyEventMetrics(
    trainEventMetrics,
    frozenD1.eventizerReproduction.eventMetrics.train,
    "eventMetrics.train",
  );
  d2d1VerifyEventMetrics(
    evalEventMetrics,
    frozenD1.eventizerReproduction.eventMetrics.eval,
    "eventMetrics.eval",
  );

  const rows = {
    train: d2d2Rows(trainTapes, phase.means, detector),
    eval: d2d2Rows(evalTapes, phase.means, detector),
    holdout: d2d2Rows(holdoutTapes, phase.means, detector),
  };

  const innovationFitRows = [];
  for (const tape of trainTapes) {
  ...
  const innovationPcaSha256 = createHash("sha256")
    .update(JSON.stringify({
      means: Array.from(innovationPca.means),
      scales: Array.from(innovationPca.scales),
      components: innovationPca.components.map(
        (row) => Array.from(row),
      ),
      eigenvalues: innovationPca.eigenvalues,
    }))
    .digest("hex");

  for (const cohort of ["train", "eval", "holdout"]) {
    for (const row of rows[cohort]) {
      row.innovationPca32 = d2d2ProjectPca(
        row.innovation,
        innovationPca,
      );
    }
  ...
  const scalarModel = d2d2FitReadout(rows.train, "scalar");
  const innovationModel = d2d2FitReadout(
    rows.train,
    "innovationPca32",
  );
  const metrics = {
    scalar: {
      train: d2d2Metrics(rows.train, scalarModel),
      eval: d2d2Metrics(rows.eval, scalarModel),
      holdout: d2d2Metrics(rows.holdout, scalarModel),
    },
    innovationPca32: {
      train: d2d2Metrics(rows.train, innovationModel),
      eval: d2d2Metrics(rows.eval, innovationModel),
      holdout: d2d2Metrics(rows.holdout, innovationModel),
    },
  };

  const support = {
    eval: d2d2Support(metrics.scalar.eval, evalTapes.length),
    holdout: d2d2Support(
      metrics.scalar.holdout,
      holdoutTapes.length,
    ),
  };
  const supportPass = support.eval.pass && support.holdout.pass;
  const scalarPass =
    supportPass &&
    d2d2FamilyPass(metrics.scalar.eval, metrics.scalar.holdout);
  const innovationPass =
    supportPass &&
    d2d2FamilyPass(
      metrics.innovationPca32.eval,
      metrics.innovationPca32.holdout,
    );

  let outcome;
  if (!supportPass) {
    outcome = "V15N_D6_D2_D2_INSUFFICIENT_ONSET_SUPPORT";
  } else if (scalarPass) {
    outcome =
      "V15N_D6_D2_D2_REIMPACT_ONSET_OBSERVABLE_SCALAR";
  } else if (innovationPass) {
    outcome =
  ...
  for (const family of ["scalar", "innovationPca32"]) {
    for (const cohort of ["train", "eval", "holdout"]) {
      const m = metrics[family][cohort];
      console.log(
        "[v15N-D6-D2-D2] " +
          family +
          " " +
          cohort +
          " BA=" +
          m.balancedAccuracy.toFixed(4) +
          " posRecall=" +
          m.positiveRecall.toFixed(4) +
          " negRecall=" +
          m.negativeRecall.toFixed(4) +
          " suppressedRecall=" +
          m.suppressedReimpactRecall.toFixed(4) +
          " support=" +
          JSON.stringify({
            positives: m.positives,
            linger: m.lingerNegatives,
            background: m.backgroundNegatives,
            suppressed: m.suppressedReimpacts,
          }),
      );
    }
  }
  console.log(
    "[v15N-D6-D2-D2] supportPass=" +
      supportPass +
      " scalarPass=" +
      scalarPass +
      " innovationPass=" +
  ...
  const output = {
    schema: "maplefly.v15n-d6-d2-d2.reimpact-onset-observability.1",
    preregistration: {
      path: "history/prereg_v15n_d6_d2_d2.md",
      commit: V15N_D6_D2_D2_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD6D2D1Receipt:
    ...
    cohorts: {
      train: V15N_TRAIN_BASE_SEEDS,
      eval: V15N_EVAL_BASE_SEEDS,
      holdout: V15N_D6_D2_D2_HOLDOUT_BASE_SEEDS,
    ...
    eventizerReproduction: {
      train: trainEventMetrics,
      eval: evalEventMetrics,
      pass: true,
    },
    featureFamilies: {
      scalar: {
      ...
      innovationPca32: {
        dimension: 32,
        historyFrames: V15N_D6_D2_D2_HISTORY_FRAMES,
        pcaFitRows: V15N_D6_D2_D2_PCA_FIT_ROWS,
        pcaIterations: V15N_PCA_ITERATIONS,
        pcaSeed: V15N_PCA_SEED,
        preprocessingSha256: innovationPcaSha256,
      },
    },
    readout: {
      type: "class-balanced deterministic ridge least squares",
      lambda: V15N_D6_D2_D2_RIDGE_LAMBDA,
      threshold: V15N_D6_D2_D2_THRESHOLD,
    },
    metrics,
    support,
    gates: {
      ...
      scalarPass,
      innovationPass,
      balancedAccuracy: V15N_D6_D2_D2_GATE,
      positiveRecall: V15N_D6_D2_D2_GATE,
      negativeRecall: V15N_D6_D2_D2_GATE,
      suppressedReimpactRecall: V15N_D6_D2_D2_GATE,
    },
    provenance: {
      v15nEvidenceSha256: artifactSha256,
      v15nPreprocessingSha256: preprocessingSha256,
      pass: true,
    },
    outcome,
    replacementPolicyTrained: false,
    replacementEventizerEvaluated: false,
  ...
  const outDir = resolve(
    "results/v15n-d6-d2-d2-reimpact-onset-observability",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d6_d2_d2.json"),
