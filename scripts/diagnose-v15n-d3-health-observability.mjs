...
function allReachableHealthStates(tapes, params) {
  const rows = [];
  for (const tape of tapes) {
    for (let decisionIndex = 0; decisionIndex < 10; decisionIndex += 1) {
      const neural = Array.from(tape.neuralFeatures[decisionIndex]);
      for (let mask = 0; mask < 2 ** decisionIndex; mask += 1) {
        ...
        rows.push({
          ...
          h,
          labels: {
            critical: hpBefore <= 30 ? 1 : -1,
            fullHealAvailable: hpBefore <= 70 ? 1 : -1,
          },
        });
      }
    }
...
function featureVector(row, featureSet) {
  if (featureSet === "FULL34") return [...row.neural, ...row.h];
  if (featureSet === "NEURAL32") return row.neural;
  if (featureSet === "ACTION_STATE2") return row.h;
  throw new Error("unknown v15N-D3 feature set " + featureSet);
}

function fitStandardization(rows, featureSet) {
  const first = featureVector(rows[0], featureSet);
  const means = first.map((_, index) =>
    mean(rows.map((row) => featureVector(row, featureSet)[index])),
  );
  const scales = first.map((_, index) => {
    const variance = mean(
      rows.map((row) => {
        const delta = featureVector(row, featureSet)[index] - means[index];
        return delta * delta;
      }),
    );
...
function standardizedFeatures(row, featureSet, standardization) {
  const raw = featureVector(row, featureSet);
  return raw.map(
    (value, index) =>
...
function solveLinearSystem(matrix, vector) {
  ...
  for (let col = 0; col < n; col += 1) {
    ...
    if (Math.abs(a[pivot][col]) < 1e-12) {
      throw new Error("v15N-D3 singular ridge system");
    }
    if (pivot !== col) {
      const temp = a[col];
      a[col] = a[pivot];
...
function fitRidgeProbe(rows, featureSet, labelKey) {
  const standardization = fitStandardization(rows, featureSet);
  const positive = rows.filter((row) => row.labels[labelKey] === 1).length;
  const negative = rows.length - positive;
  if (!positive || !negative) {
    throw new Error("v15N-D3 missing TRAIN class");
  }

  const dimension = featureVector(rows[0], featureSet).length + 1;
  ...
  for (const row of rows) {
    const y = row.labels[labelKey];
    const weight =
      y === 1 ? 0.5 / positive : 0.5 / negative;
    const x = [
      ...standardizedFeatures(row, featureSet, standardization),
      1,
    ];
  ...
  for (let i = 0; i < dimension - 1; i += 1) {
    normal[i][i] += V15N_D3_RIDGE_LAMBDA;
  }

  return {
    featureSet,
    labelKey,
    lambda: V15N_D3_RIDGE_LAMBDA,
    standardization,
    weights: solveLinearSystem(normal, rhs),
    trainSupport: { positive, negative },
  };
}

function probeScore(row, probe) {
  const x = [
...
function probeMetrics(rows, probe) {
  const positiveRows = rows.filter(
    (row) => row.labels[probe.labelKey] === 1,
  );
  ...
  for (const row of rows) {
    const prediction = probeScore(row, probe) > 0 ? 1 : -1;
    if (row.labels[probe.labelKey] === 1 && prediction === 1) {
      positiveCorrect += 1;
    }
    if (row.labels[probe.labelKey] === -1 && prediction === -1) {
      negativeCorrect += 1;
    ...
    byTape.get(row.seed).push({ row, prediction });
  }

  const positiveRecall = positiveRows.length
    ? positiveCorrect / positiveRows.length
    : null;
  const negativeRecall = negativeRows.length
    ? negativeCorrect / negativeRows.length
    : null;
  const tapeBas = [];
  for (const [seed, entries] of byTape) {
    const pos = entries.filter(
      (entry) => entry.row.labels[probe.labelKey] === 1,
    );
    ...
    const posRecall =
      pos.filter((entry) => entry.prediction === 1).length / pos.length;
    const negRecall =
      neg.filter((entry) => entry.prediction === -1).length / neg.length;
    tapeBas.push({ seed, balancedAccuracy: (posRecall + negRecall) / 2 });
  }
  ...
  return {
    ...
    ordinaryAccuracy:
      rows.length
        ? (positiveCorrect + negativeCorrect) / rows.length
        : null,
    mixedTapeCount: tapeBas.length,
    macroTapeBalancedAccuracy:
      tapeBas.length
        ? mean(tapeBas.map((row) => row.balancedAccuracy))
        : null,
    perTape: tapeBas,
  };
}
...
async function main() {
  ...
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D3 evidence JSON SHA mismatch");
  }
  ...
  if (
    ...
    throw new Error("v15N-D3 v14C provenance mismatch");
  }
  const attackPolicy = {
  ...
  if (
    ...
    throw new Error("v15N-D3 DN identity/order mismatch");
  }
  ...
  const holdoutTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D3_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(V15N_D3_HOLDOUT_INTERRUPTION_SEED),
    label: "d3-holdout",
  });
  if (
    ...
    throw new Error("v15N-D3 tape count mismatch");
  }
  ...
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D3 preprocessing SHA mismatch");
  }
  ...
  const trainStates = allReachableHealthStates(trainTapes, params);
  const evalStates = allReachableHealthStates(evalTapes, params);
  const holdoutStates = allReachableHealthStates(holdoutTapes, params);

  const targets = ["critical", "fullHealAvailable"];
  const featureSets = ["FULL34", "NEURAL32", "ACTION_STATE2"];
  const probes = {};
  const metrics = {};
  ...
  for (const target of targets) {
    ...
    for (const featureSet of featureSets) {
      const probe = fitRidgeProbe(
        trainStates,
        featureSet,
        target,
      );
      probes[target][featureSet] = {
        featureSet: probe.featureSet,
        labelKey: probe.labelKey,
        lambda: probe.lambda,
        standardization: probe.standardization,
        weights: probe.weights,
        trainSupport: probe.trainSupport,
      };
      metrics[target][featureSet] = {
        train: probeMetrics(trainStates, probe),
        eval: probeMetrics(evalStates, probe),
        holdout: probeMetrics(holdoutStates, probe),
      };
    }
  }

  let supportPass = true;
  for (const target of targets) {
    const full = metrics[target].FULL34;
    for (const cohort of ["train", "eval", "holdout"]) {
      if (
        full[cohort].positive < V15N_D3_MIN_CLASS_SUPPORT ||
        full[cohort].negative < V15N_D3_MIN_CLASS_SUPPORT
      ) {
        supportPass = false;
      }
    }
    if (
      full.eval.mixedTapeCount < V15N_D3_MIN_MIXED_TAPES ||
      full.holdout.mixedTapeCount < V15N_D3_MIN_MIXED_TAPES
    ) {
      supportPass = false;
    }
  }

  const criticalStable =
    metrics.critical.FULL34.eval.balancedAccuracy >= 0.70 &&
    metrics.critical.FULL34.holdout.balancedAccuracy >= 0.70;
  const fullHealStable =
    metrics.fullHealAvailable.FULL34.eval.balancedAccuracy >= 0.70 &&
    metrics.fullHealAvailable.FULL34.holdout.balancedAccuracy >= 0.70;
  const allClearlyWeak = [
    metrics.critical.FULL34.eval.balancedAccuracy,
    metrics.critical.FULL34.holdout.balancedAccuracy,
    metrics.fullHealAvailable.FULL34.eval.balancedAccuracy,
    metrics.fullHealAvailable.FULL34.holdout.balancedAccuracy,
  ].every((value) => value < 0.60);

  let outcome;
  if (!ecologyPass) {
    outcome = "V15N_D3_IMPLEMENTATION_OR_PROVENANCE_INVALID";
  } else if (!supportPass) {
    outcome = "V15N_D3_INSUFFICIENT_HEALTH_STATE_SUPPORT";
  } else if (criticalStable && fullHealStable) {
    outcome = "V15N_D3_CURRENT_HEALTH_LINEARLY_RECOVERABLE";
  } else if (criticalStable !== fullHealStable) {
    outcome = "V15N_D3_PARTIAL_HEALTH_OBSERVABILITY";
  } else if (allClearlyWeak) {
    outcome = "V15N_D3_CURRENT_HEALTH_NOT_LINEARLY_RECOVERABLE";
  } else {
    outcome = "V15N_D3_INCONCLUSIVE";
  }
  ...
  for (const target of targets) {
    ...
    console.log(
      "[v15N-D3] " +
        ...
        full.eval.negative +
        " holdout=" +
        full.holdout.positive +
        "/" +
        full.holdout.negative,
    );
    console.log(
      "[v15N-D3] " +
        target +
        " FULL34 BA train=" +
        (full.train.balancedAccuracy * 100).toFixed(1) +
        "% eval=" +
        (full.eval.balancedAccuracy * 100).toFixed(1) +
        "% holdout=" +
        (full.holdout.balancedAccuracy * 100).toFixed(1) +
        "% NEURAL32 eval=" +
        (
          metrics[target].NEURAL32.eval.balancedAccuracy * 100
        ).toFixed(1) +
        "% holdout=" +
        (
          metrics[target].NEURAL32.holdout.balancedAccuracy * 100
        ).toFixed(1) +
        "% ACTION_STATE2 eval=" +
        (
          metrics[target].ACTION_STATE2.eval.balancedAccuracy * 100
        ).toFixed(1) +
        "% holdout=" +
        (
          metrics[target].ACTION_STATE2.holdout.balancedAccuracy * 100
        ).toFixed(1) +
        "%",
    );
  }
  console.log(
    "[v15N-D3] criticalStable=" +
      criticalStable +
      " fullHealStable=" +
      fullHealStable +
      " supportPass=" +
  ...
  const output = {
    schema: "maplefly.v15n-d3.current-health-observability.1",
    preregistration: {
      path: "history/prereg_v15n_d3.md",
      commit: V15N_D3_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD2Receipt:
        "e39c06ea85ee9a43d38817a41fd3956eb343571d",
      v15nD2Outcome: "V15N_D2_INCONCLUSIVE",
    },
    frozenCandidate: {
    ...
    cohorts: {
      ...
      holdoutBaseSeeds: V15N_D3_HOLDOUT_BASE_SEEDS,
      trainStates: trainStates.length,
      evalStates: evalStates.length,
      holdoutStates: holdoutStates.length,
      originalInterruptionRngSeed: 4147000,
      holdoutInterruptionRngSeed:
        V15N_D3_HOLDOUT_INTERRUPTION_SEED,
    },
    labels: {
      critical: "HP <= 30",
      fullHealAvailable: "HP <= 70",
      evaluatorOnly: true,
    },
    probeContract: {
      classBalanced: true,
      ridgeLambda: V15N_D3_RIDGE_LAMBDA,
      interceptRegularized: false,
      prediction: "positive iff score > 0; tie negative",
      diagnosticWeightsNotDeployable: true,
    },
    ...
    diagnosticProbeWeightsDeployable: false,
    deployment: "BLOCKED",
    deployedPotion: "v15D",
    v16c: "BLOCKED",
  };

  const outDir = resolve("results/v15n-d3-health-observability");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d3.json"),
