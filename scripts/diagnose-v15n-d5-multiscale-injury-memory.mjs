...
function multiscaleSnapshots(tapes) {
  const rows = [];
  for (const tape of tapes) {
    for (const event of tape.potionEvents) {
      if (
        !Array.isArray(event.multiTraces) ||
        event.multiTraces.length !== V15N_D5_HALF_LIVES.length
      ) {
        throw new Error("v15N-D5 multiscale trace count mismatch");
      }
      const flat = new Float64Array(
        DN_COUNT * V15N_D5_HALF_LIVES.length,
      );
      for (
        let scaleIndex = 0;
        scaleIndex < event.multiTraces.length;
        scaleIndex += 1
      ) {
        const trace = event.multiTraces[scaleIndex];
        if (!Array.isArray(trace) || trace.length !== DN_COUNT) {
          throw new Error("v15N-D5 multiscale trace width mismatch");
        }
        flat.set(trace, scaleIndex * DN_COUNT);
      }
      rows.push(flat);
    }
  }
...
function fitGenericPca(rows, width, seedBase) {
  if (rows.length !== 240) {
    throw new Error("v15N-D5 PCA TRAIN snapshot count mismatch");
  }
  const means = new Float64Array(width);
  for (const row of rows) {
    for (let d = 0; d < width; d += 1) means[d] += row[d];
  }
  for (let d = 0; d < width; d += 1) means[d] /= rows.length;

  const scales = new Float64Array(width);
  for (const row of rows) {
  ...
  for (let d = 0; d < width; d += 1) {
    scales[d] = Math.max(
      Math.sqrt(scales[d] / rows.length),
      1e-6,
    );
  }
  ...
  for (
    let component = 0;
    component < V15N_D5_PCA_COMPONENTS;
    component += 1
  ) {
    const random = mulberry32(seedBase + component);
    let u = Float64Array.from(
      { length: n },
      () => random() * 2 - 1,
    );
    ...
    for (
      let iteration = 0;
      iteration < V15N_D5_PCA_ITERATIONS;
      iteration += 1
    ) {
      const next = new Float64Array(n);
      for (let i = 0; i < n; i += 1) {
    ...
    const loading = new Float64Array(width);
    for (let i = 0; i < n; i += 1) {
      const scale = u[i];
      const row = standardized[i];
      for (let d = 0; d < width; d += 1) {
    ...
    if (loading[maxIndex] < 0) {
      for (let d = 0; d < width; d += 1) loading[d] *= -1;
      for (let i = 0; i < n; i += 1) u[i] *= -1;
    }
...
function projectGeneric(row, preprocessing) {
  const z = new Float64Array(row.length);
  for (let d = 0; d < row.length; d += 1) {
    z[d] =
      (row[d] - preprocessing.means[d]) /
...
function attachMultiscaleFeatures(tapes, preprocessing) {
  for (const tape of tapes) {
    tape.multiscaleFeatures = tape.potionEvents.map((event) => {
      const flat = new Float64Array(
        DN_COUNT * V15N_D5_HALF_LIVES.length,
      );
      for (
        ...
        flat.set(
          event.multiTraces[scaleIndex],
          scaleIndex * DN_COUNT,
        );
      }
      return projectGeneric(flat, preprocessing);
    });
  }
}

function maxTwoSecondTraceError(tapes) {
  let maximum = 0;
  for (const tape of tapes) {
    for (const event of tape.potionEvents) {
      const trace2 = event.multiTraces[1];
      for (let d = 0; d < DN_COUNT; d += 1) {
        maximum = Math.max(
          maximum,
          Math.abs(event.trace[d] - trace2[d]),
        );
      }
    }
  }
...
function trainDecisionDamageMeans(tapes) {
...
function injuryRows(tapes, decisionMeans) {
  const rows = [];
  for (const tape of tapes) {
    for (let decisionIndex = 0; decisionIndex < 10; decisionIndex += 1) {
      ...
      rows.push({
        ...
        decisionIndex,
        damage,
        y: damage - decisionMeans[decisionIndex],
        single: Array.from(tape.neuralFeatures[decisionIndex]),
        multi: Array.from(tape.multiscaleFeatures[decisionIndex]),
      });
    }
  }
  return rows;
...
function featureFor(row, representation) {
  if (representation === "MULTISCALE") return row.multi;
  if (representation === "SINGLE2") return row.single;
  throw new Error("v15N-D5 unknown representation");
}

function fitFeatureStandardization(rows, representation) {
  const first = featureFor(rows[0], representation);
  const means = first.map((_, index) =>
    mean(rows.map((row) => featureFor(row, representation)[index])),
  );
  const scales = first.map((_, index) => {
    const variance = mean(
      rows.map((row) => {
        const delta =
          featureFor(row, representation)[index] - means[index];
        return delta * delta;
      }),
    );
...
function standardizedFeature(row, representation, standardization) {
  const raw = featureFor(row, representation);
  return raw.map(
    (value, index) =>
      (value - standardization.means[index]) /
      standardization.scales[index],
  );
}

function solveD5LinearSystem(matrix, vector) {
  ...
  for (let col = 0; col < n; col += 1) {
    ...
    if (Math.abs(a[pivot][col]) < 1e-12) {
      throw new Error("v15N-D5 singular ridge system");
    }
    if (pivot !== col) {
      const temp = a[col];
      a[col] = a[pivot];
...
function fitInjuryProbe(rows, representation) {
  const standardization =
    fitFeatureStandardization(rows, representation);
  const dimension = featureFor(rows[0], representation).length + 1;
  ...
  for (let i = 0; i < dimension - 1; i += 1) {
    normal[i][i] += V15N_D5_RIDGE_LAMBDA;
  }
  return {
    representation,
    standardization,
    weights: solveD5LinearSystem(normal, rhs),
  };
}

function predictResidual(row, probe) {
  const x = [
    ...standardizedFeature(
      row,
  ...
  return x.reduce(
    (sum, value, index) => sum + value * probe.weights[index],
    0,
  );
}

function residualSd(rows) {
...
function injuryMetrics(rows, probe, featureOverride = null) {
  const actual = rows.map((row) => row.y);
  const predicted = rows.map((row, index) => {
    if (!featureOverride) return predictResidual(row, probe);
    const overridden = {
      ...row,
      [probe.representation === "MULTISCALE" ? "multi" : "single"]:
        featureOverride[index],
    };
    return predictResidual(overridden, probe);
  });
  ...
  const pearson =
    actualVariance > 0 && predictedVariance > 0
      ...
      : null;
  const perDecision = Array.from({ length: 10 }, (_, decisionIndex) => {
    const indices = rows
      .map((row, index) => ({ row, index }))
      .filter(({ row }) => row.decisionIndex === decisionIndex);
    return {
      decisionIndex,
      states: indices.length,
      mae: mean(
        indices.map(({ index }) =>
...
function shiftedFeatures(rows, representation) {
...
async function main() {
  ...
  const artifactSha256 = createHash("sha256")
    ...
    .digest("hex");
  if (artifactSha256 !== V15N_D1_EVIDENCE_SHA256) {
    throw new Error("v15N-D5 evidence JSON SHA mismatch");
  }
  const artifact = JSON.parse(artifactBytes.toString("utf8"));
  const params = loadFrozenV15nParams(artifact);
  ...
  if (
    ...
    throw new Error("v15N-D5 DN identity/order mismatch");
  }
  ...
  const interruptionRandom = mulberry32(4147000);
  ...
  const evalTapes = await collectTapes({
    ...
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_EVAL_BASE_SEEDS,
    interruptionRandom,
    label: "d5-eval",
  });
  const holdoutTapes = await collectTapes({
    connectome,
    dnSlot,
    attackPolicy,
    jumpPolicy,
    baseSeeds: V15N_D5_HOLDOUT_BASE_SEEDS,
    interruptionRandom: mulberry32(V15N_D5_HOLDOUT_INTERRUPTION_SEED),
    label: "d5-holdout",
  });

  const evalValidity = tapeValidity(evalTapes);
  const holdoutValidity = tapeValidity(holdoutTapes);
  const evalValidityChecks = ecologyChecks(evalValidity);
  const holdoutValidityChecks = ecologyChecks(holdoutValidity);
  const ecologyPass =
    Object.values(evalValidityChecks).every(Boolean) &&
    Object.values(holdoutValidityChecks).every(Boolean);

  const singlePreprocessing = fitTracePreprocessing(trainTapes);
  attachTraceFeatures(trainTapes, singlePreprocessing);
  attachTraceFeatures(evalTapes, singlePreprocessing);
  attachTraceFeatures(holdoutTapes, singlePreprocessing);
  const preprocessingSha256 = createHash("sha256")
    .update(JSON.stringify({
      means: Array.from(singlePreprocessing.means),
      scales: Array.from(singlePreprocessing.scales),
      components: singlePreprocessing.components.map(
        (row) => Array.from(row),
      ),
    }))
    .digest("hex");
  if (preprocessingSha256 !== V15N_D1_PREPROCESSING_SHA256) {
    throw new Error("v15N-D5 frozen single-trace preprocessing mismatch");
  }

  const traceError = Math.max(
    maxTwoSecondTraceError(trainTapes),
    maxTwoSecondTraceError(evalTapes),
    maxTwoSecondTraceError(holdoutTapes),
  );

  const multiscalePreprocessing = fitGenericPca(
    multiscaleSnapshots(trainTapes),
    DN_COUNT * V15N_D5_HALF_LIVES.length,
    V15N_D5_PCA_SEED,
  );
  attachMultiscaleFeatures(trainTapes, multiscalePreprocessing);
  attachMultiscaleFeatures(evalTapes, multiscalePreprocessing);
  attachMultiscaleFeatures(holdoutTapes, multiscalePreprocessing);

  const evalOracle = new Map(
    evalTapes.map((tape) => [tape.seed, oracleMinimumUses(tape)]),
  );
  const reproducedFull = addEconomyMetrics(
    evaluatePolicy(evalTapes, params),
    evalOracle,
  );
  verifyAggregate(reproducedFull, artifact.evaluation.full, "FULL");

  const decisionMeans = trainDecisionDamageMeans(trainTapes);
  const trainRows = injuryRows(trainTapes, decisionMeans);
  const evalRows = injuryRows(evalTapes, decisionMeans);
  const holdoutRows = injuryRows(holdoutTapes, decisionMeans);

  const supportPass =
    trainRows.length === 240 &&
    evalRows.length === 240 &&
    holdoutRows.length === 240 &&
    residualSd(trainRows) >= V15N_D5_MIN_RESIDUAL_SD &&
    residualSd(evalRows) >= V15N_D5_MIN_RESIDUAL_SD &&
    residualSd(holdoutRows) >= V15N_D5_MIN_RESIDUAL_SD;

  const representations = ["MULTISCALE", "SINGLE2"];
  const probes = {};
  const metrics = {};
  for (const representation of representations) {
    const probe = fitInjuryProbe(trainRows, representation);
    probes[representation] = {
      representation,
      standardization: probe.standardization,
      weights: probe.weights,
      lambda: V15N_D5_RIDGE_LAMBDA,
    };
    metrics[representation] = {
      train: injuryMetrics(trainRows, probe),
      eval: injuryMetrics(evalRows, probe),
      holdout: injuryMetrics(holdoutRows, probe),
      shiftedEval: injuryMetrics(
        evalRows,
        probe,
        shiftedFeatures(evalRows, representation),
      ),
      shiftedHoldout: injuryMetrics(
        holdoutRows,
        probe,
        shiftedFeatures(holdoutRows, representation),
      ),
    };
  }

  const multiStable =
    metrics.MULTISCALE.eval.r2 >= 0.30 &&
    metrics.MULTISCALE.holdout.r2 >= 0.30 &&
    metrics.MULTISCALE.eval.mae <= 10 &&
    metrics.MULTISCALE.holdout.mae <= 10;
  const singleStable =
    metrics.SINGLE2.eval.r2 >= 0.30 &&
    metrics.SINGLE2.holdout.r2 >= 0.30 &&
    metrics.SINGLE2.eval.mae <= 10 &&
    metrics.SINGLE2.holdout.mae <= 10;
  const multiImproves =
    metrics.MULTISCALE.eval.r2 - metrics.SINGLE2.eval.r2 >= 0.15 &&
    metrics.MULTISCALE.holdout.r2 - metrics.SINGLE2.holdout.r2 >= 0.15;
  const episodeSpecific =
    metrics.MULTISCALE.eval.r2 - metrics.MULTISCALE.shiftedEval.r2 >= 0.15 &&
    metrics.MULTISCALE.holdout.r2 -
      metrics.MULTISCALE.shiftedHoldout.r2 >=
      0.15;
  const bothClearlyWeak =
    metrics.MULTISCALE.eval.r2 <= 0.10 &&
    metrics.MULTISCALE.holdout.r2 <= 0.10 &&
    metrics.SINGLE2.eval.r2 <= 0.10 &&
    metrics.SINGLE2.holdout.r2 <= 0.10;

  let outcome;
  if (!ecologyPass || !supportPass || traceError > 1e-12) {
    outcome = "V15N_D5_IMPLEMENTATION_OR_SUPPORT_INVALID";
  } else if (multiStable && multiImproves && episodeSpecific) {
    outcome =
      "V15N_D5_MULTISCALE_CUMULATIVE_INJURY_MEMORY_PRESENT";
  } else if (singleStable && episodeSpecific) {
    outcome = "V15N_D5_SINGLE_TIMESCALE_INJURY_MEMORY_SUFFICIENT";
  } else if (multiStable && !episodeSpecific) {
    outcome = "V15N_D5_NON_EPISODE_SPECIFIC_DECODING";
  } else if (bothClearlyWeak) {
    outcome = "V15N_D5_CUMULATIVE_INJURY_MEMORY_NOT_DEMONSTRATED";
  } else {
    outcome = "V15N_D5_INCONCLUSIVE";
  }

  for (const representation of representations) {
    const row = metrics[representation];
    console.log(
      "[v15N-D5] " +
        representation +
        " train R2=" +
        row.train.r2.toFixed(4) +
        " MAE=" +
        row.train.mae.toFixed(2) +
        " eval R2=" +
        row.eval.r2.toFixed(4) +
        " MAE=" +
        row.eval.mae.toFixed(2) +
        " shifted=" +
        row.shiftedEval.r2.toFixed(4) +
        " holdout R2=" +
        row.holdout.r2.toFixed(4) +
        " MAE=" +
        row.holdout.mae.toFixed(2) +
        " shifted=" +
        row.shiftedHoldout.r2.toFixed(4),
    );
  }
  console.log(
    "[v15N-D5] residualSD=" +
      residualSd(trainRows).toFixed(2) +
      "/" +
      residualSd(evalRows).toFixed(2) +
      "/" +
      residualSd(holdoutRows).toFixed(2) +
      " trace2MaxError=" +
      traceError +
      " multiStable=" +
      multiStable +
  ...
  const output = {
    schema: "maplefly.v15n-d5.multiscale-injury-memory.1",
    preregistration: {
      path: "history/prereg_v15n_d5.md",
      commit: V15N_D5_PREREG_COMMIT,
    },
    evidenceChain: {
      v15nD4Receipt:
        "597d565f461e6efe6e04c4d8ed8d296331417fd5",
      v15nD4Outcome: "V15N_D4_INCONCLUSIVE",
    },
    frozenCandidate: {
      artifactId: V15N_D1_ARTIFACT_ID,
      evidenceJsonSha256: artifactSha256,
      preprocessingSha256,
    },
    cohorts: {
      trainBaseSeeds: V15N_TRAIN_BASE_SEEDS,
      evalBaseSeeds: V15N_EVAL_BASE_SEEDS,
      holdoutBaseSeeds: V15N_D5_HOLDOUT_BASE_SEEDS,
      originalInterruptionRngSeed: 4147000,
      holdoutInterruptionRngSeed:
        V15N_D5_HOLDOUT_INTERRUPTION_SEED,
      snapshots: {
    ...
    representation: {
      halfLivesSeconds: V15N_D5_HALF_LIVES,
      decays: V15N_D5_TRACE_DECAYS,
      multiscaleRawWidth:
        DN_COUNT * V15N_D5_HALF_LIVES.length,
      pcaComponents: V15N_D5_PCA_COMPONENTS,
      pcaSeed: V15N_D5_PCA_SEED,
      pcaIterations: V15N_D5_PCA_ITERATIONS,
      twoSecondTraceMaxAbsoluteError: traceError,
      labelFree: true,
    },
    target: {
      name: "cumulative damage residual before decision",
      trainDecisionMeans: decisionMeans,
      evaluatorOnly: true,
      runtimeInput: false,
    },
    support: {
      residualSd: {
        train: residualSd(trainRows),
        eval: residualSd(evalRows),
        holdout: residualSd(holdoutRows),
      },
      pass: supportPass,
    },
    provenanceReproduction: {
      full: {
        survivalRate: reproducedFull.survivalRate,
        minSeedSurvivalRate: reproducedFull.minSeedSurvivalRate,
        meanUses: reproducedFull.meanUses,
        meanExcessUses: reproducedFull.meanExcessUses,
        wastedHealingPerDrink:
          reproducedFull.wastedHealingPerDrink,
      },
      pass: true,
    },
    ecology: {
      eval: {
        metrics: evalValidity,
        checks: evalValidityChecks,
        pass: Object.values(evalValidityChecks).every(Boolean),
      },
      holdout: {
        metrics: holdoutValidity,
        checks: holdoutValidityChecks,
        pass: Object.values(holdoutValidityChecks).every(Boolean),
      },
      pass: ecologyPass,
    },
    probes,
    ...
    diagnosticDamageProbeWeightsDeployable: false,
    damageTargetRuntimeInput: false,
    deployment: "BLOCKED",
    deployedPotion: "v15D",
    v16c: "BLOCKED",
  };
  ...
  const outDir = resolve(
    "results/v15n-d5-multiscale-injury-memory",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "v15n_d5.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  if (outcome === "V15N_D5_IMPLEMENTATION_OR_SUPPORT_INVALID") {
    process.exitCode = 1;
  }
}
