...
async function main() {
  ...
  const gate =
    current.summary.gate;
  const selectedRepresentation =
    gate ? "CURRENT" : null;
  const historyGain = null;
  ...
  const output = {
    meta: {
      ...
      sensoryArchitecture: {
        target:
          "v10F target sensory unchanged; target geometry matched across classes",
        obstacle:
          "equal fixed looming drive to LC6/LC16/LC22/LPLC4 on obstacle side; obstacle LC4 disabled",
        loomingCounts,
        matchedTargetGeometry: true,
        virtualSourceToTargetCenterOffset:
          PLAYER_WIDTH / 2 +
          OBSTACLE_WIDTH +
          160,
      },
      ...
      selectionRule:
        "CURRENT-only preregistered representation gate; no post-result representation selection",
    },
    results,
