...
const TRAIN_BASE_SEEDS = [2800000, 2800100, 2800200, 2800300];
const EVAL_BASE_SEEDS = [2805000, 2805100, 2805200];
const REPLICATES = 6;
...
const L2 = 0.001;
const ORDER_SEED = 2806000;
const LABEL_SHUFFLE_SEED = 2807000;
const GATE = Object.freeze({
  balancedAccuracyMin: 0.65,
  everyClassRecallMin: 0.50,
  dnShuffleMarginMin: 0.20,
});
const PREREG_COMMIT = "3fca025a106799c9a364cbad7516ce834d9dde6a";
...
async function collectDataset(connectome, dnSlot, baseSeeds, label) {
  ...
  for (const baseSeed of baseSeeds) {
    ...
    console.log("[v15A3-" + label + "] baseSeed=" + baseSeed + " rows=" + rows.length);
...
async function main() {
  const outDir = resolve("results/screen-v15a3-injury-memory");
  ...
  for (const side of ["L", "R"]) {
    ...
    if (impact.length !== expected) {
      throw new Error(
        "corrected LgLG_" + side + " count mismatch: " +
          impact.length + " != " + expected,
      );
    }
  ...
  const output = {
    schema: "maplefly.v15a3.corrected-injury-history-screen.1",
    brainRepository: SOURCE.repository,
    brainCommit: SOURCE.commit,
    preregistration: {
      path: "history/prereg_v15a3.md",
    ...
    featureContract: {
      source: "baseline-relative frozen MaleCNS DN temporal activity only; corrected LgLG prefix family",
      dnCount: DN_COUNT,
  ...
  await writeFile(
    resolve(outDir, "v15a3_screen.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  console.log(
    "V15A3-INJURY-HISTORY-SCREEN=" +
      (pass ? "PASS" : "FAIL") +
      " selected=" +
      (selected ?? "NONE"),
  );
