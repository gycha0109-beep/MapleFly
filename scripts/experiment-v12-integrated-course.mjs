...
const FINAL_SEEDS = [2301000, 2311000, 2321000];
...
async function main() {
  ...
  const output = {
    schema: "maplefly.v12.integrated-course.1",
    purpose:
      "Pre-registered deployment integration acceptance for frozen MOVE v7 + JUMP v11H2 + ATTACK v10F.",
  ...
  const outDir = resolve("results/experiment-v12-integrated-course");
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "experiment_v12.json"),
    JSON.stringify(output, null, 2) + "\n",
  );

  console.log(
    "V12-INTEGRATED-COURSE=" +
      (pass ? "PASS" : "FAIL") +
      " complete=" +
