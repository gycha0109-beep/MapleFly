...
async function main() {
  ...
  const validationReceipt = JSON.parse(
    await readFile(
      new URL(
        "../history/run_receipts/v14c_latest.json",
        import.meta.url,
      ),
      "utf8",
    ),
  ...
  if (
    validationReceipt.authoritativeRun?.runId !== 35799237521 ||
    validationReceipt.deploymentStatus !==
      "VALIDATED_CANDIDATE_NOT_DEPLOYED"
  ) {
    throw new Error("v14C validation receipt provenance mismatch");
  }

  const expected = validationReceipt.final;
  const equivalenceFields = {
    courseCompletionRate: summary.courseCompletionRate,
    minSeedCompletionRate: summary.minSeedCompletionRate,
    obstacleClearRate: summary.obstacleClearRate,
    leftObstacleClearRate: summary.leftObstacleClearRate,
    rightObstacleClearRate: summary.rightObstacleClearRate,
    targetKillRate: summary.targetKillRate,
    timeoutRate: summary.timeoutRate,
    meanActualJumps: summary.meanActualJumps,
    postClearJumpEpisodeRate: summary.postClearJumpEpisodeRate,
    preClearAttackEpisodeRate: summary.preClearAttackEpisodeRate,
    airborneAttackActionFraction:
      summary.airborneAttackActionFraction,
    maxSeedAirborneAttackActionFraction:
      summary.maxSeedAirborneAttackActionFraction,
    attackHitPrecision: summary.attackHitPrecision,
  };
  const deploymentEquivalence = {};
  for (const [key, actual] of Object.entries(equivalenceFields)) {
    const expectedValue = expected[key];
    const error = Math.abs(actual - expectedValue);
    deploymentEquivalence[key] = {
      actual,
    ...
    if (error > 1e-3) {
      throw new Error(
        "v14C deployment equivalence mismatch " +
          key +
          " actual=" +
          actual +
          " expected=" +
  ...
  const output = {
    schema: "maplefly.v14c.deployment-smoke.1",
    brainCommit: SOURCE.commit,
    preregistration:
      "history/prereg_v14c_deployment.md@ee57facc8ddfaeab173598a0d2c7ea98c2d021de",
    candidate: {
  ...
  const outDir = resolve(
    "results/verify-v14c-deployment",
  );
  await mkdir(outDir, { recursive: true });
  await writeFile(
    resolve(outDir, "deployment_smoke.json"),
