...
const L2 = 0.0005;
const THRESHOLD = 0.5;
const ORDER_SEED = 1406000;
const LABEL_SHUFFLE_SEED = 1416000;
const TEMPORAL_ORDER = Object.freeze([2, 0, 3, 1]);
...
function makeTemporalOrderPermutation() {
  const permutation =
    new Int32Array(
      TEMPORAL_FEATURE_COUNT,
    );
...
async function main() {
  ...
  for (
    ...
    const dnPermutation =
      makeDnTemporalPermutation(
        baseSeed + 900000,
      );
    const temporalPermutation =
      makeTemporalOrderPermutation();
    ...
    perRun.push({
      ...
      dnPermutedBalancedAccuracy:
        dnPermuted,
      temporalOrderShuffledBalancedAccuracy:
        temporalOrderShuffled,
    });
  ...
  const temporalShuffledMean = mean(
    perRun.map(
      (row) =>
        row.temporalOrderShuffledBalancedAccuracy,
    ),
  );
  ...
  const gate =
    ...
    fullMean -
        temporalShuffledMean >=
      0.15;

  const output = {
    meta: {
      schema:
        "maplefly.experiment-v11g2.temporal-dn-context-separability.1",
      brainRepository:
        SOURCE.repository,
      brainCommit:
        SOURCE.commit,
      ...
      antiLeak:
        "classifier input is only 4 consecutive 1316-DN activity windows; context identity/distance/coordinates/side/seed/bin are metadata only",
    },
    summary: {
      fullBalancedAccuracy:
        fullMean,
      ...
      temporalOrderShuffledBalancedAccuracy:
        temporalShuffledMean,
      fullMinusTemporalOrderShuffled:
        fullMean -
        temporalShuffledMean,
