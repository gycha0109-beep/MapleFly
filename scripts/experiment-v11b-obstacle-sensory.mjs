...
const TRAIN_BASE_SEEDS = [601000, 601100, 601200, 601300];
const EVAL_BASE_SEEDS = [611000, 611100, 611200];
...
const LABEL_SHUFFLE_SEED = 616000;
...
async function main() {
  const outDir = resolve("results/experiment-v11b");
  ...
  if (trainRows.length !== 256) {
    throw new Error(
      "train sample count mismatch: " + trainRows.length,
    );
  }

  const stats = fitStandardizer(trainRows);
  const trueLabels = trainRows.map((row) => row.label);
  const labelShuffleOrder = shuffledIndices(
    trueLabels.length,
    LABEL_SHUFFLE_SEED,
  );
  const shuffledLabels = labelShuffleOrder.map(
  ...
  const model = trainClassifier({
    ...
    labels: trueLabels,
    stats,
    orderSeed: SHUFFLE_SEED,
  });
  const labelShuffledModel = trainClassifier({
    rows: trainRows,
  ...
  if (evalRows.length !== 192) {
    throw new Error(
      "eval sample count mismatch: " + evalRows.length,
    );
  }

  const full = evaluateModel({
    rows: evalRows,
    model,
    stats,
  });
  const shuffledControl = evaluateModel({
    rows: evalRows,
  ...
  const perRun = [];
  for (let run = 0; run < EVAL_BASE_SEEDS.length; run += 1) {
    ...
    const runLabelShuffled = evaluateModel({
      rows,
      model: labelShuffledModel,
      stats,
    });
    const runDnPermuted = evaluateModel({
      rows,
      model,
      stats,
      permutation,
    ...
    console.log(
      "[v11b-eval] run=" +
        (run + 1) +
        " FULL=" +
        (runFull.balancedAccuracy * 100).toFixed(1) +
        "% LABEL_SHUFFLED=" +
        (runLabelShuffled.balancedAccuracy * 100).toFixed(1) +
        "% DN_PERMUTED=" +
        (runDnPermuted.balancedAccuracy * 100).toFixed(1) +
        "%",
    );
  }

  const fullMean = mean(
    perRun.map((row) => row.fullBalancedAccuracy),
  );
  const shuffledMean = mean(
    perRun.map(
      (row) => row.labelShuffledBalancedAccuracy,
    ),
  );
  const permutedMean = mean(
    perRun.map(
      (row) => row.dnPermutedBalancedAccuracy,
    ),
  );
  ...
  const output = {
    meta: {
      schema:
        "maplefly.experiment-v11b.obstacle-sensory-separability.1",
      ...
      antiLeak:
        "diagnostic classifier input is 1316-DN activity only; distance/side/geometry/seed are metadata only and never model inputs",
    },
    train: {
      episodes: 64,
      samples: trainRows.length,
      rows: compactRows(trainRows),
    },
    evaluation: {
      episodes: 48,
      samples: evalRows.length,
      rows: compactRows(evalRows),
    },
    summary: {
      fullBalancedAccuracy: fullMean,
      labelShuffledBalancedAccuracy: shuffledMean,
      fullMinusLabelShuffled:
        fullMean - shuffledMean,
      dnPermutedBalancedAccuracy: permutedMean,
      fullMinusDnPermuted:
  ...
  await writeFile(
    resolve(outDir, "experiment_v11b.json"),
  ...
  console.log(
    "V11B-SENSORY-GATE=" +
      (gate ? "PASS" : "FAIL") +
      " FULL=" +
      (fullMean * 100).toFixed(1) +
      "% LABEL_SHUFFLED=" +
      (shuffledMean * 100).toFixed(1) +
      "% DN_PERMUTED=" +
      (permutedMean * 100).toFixed(1) +
      "% PAIRED_L2=" +
