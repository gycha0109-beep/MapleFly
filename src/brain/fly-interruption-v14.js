(function installMapleFlyInterruptionV14(global) {
  "use strict";
  ...
  function pushFrame(history, frame) {
    if (
      ...
      throw new Error("v14 interruption history contract mismatch");
    }
    history.shift();
  ...
  function concatHistory(history) {
    if (!Array.isArray(history) || history.length !== HISTORY_FRAMES) {
      throw new Error("v14 interruption history length mismatch");
    }
    ...
    for (let window = 0; window < HISTORY_FRAMES; window += 1) {
      if (history[window].length !== FRAME_SIZE) {
        throw new Error("v14 interruption frame length mismatch");
      }
  ...
  function acceptProbability(policy, feature) {
    if (feature.length !== FEATURE_COUNT) {
      throw new Error("v14 interruption feature length mismatch");
    }
  ...
  global.MapleFlyInterruptionV14 = Object.freeze({
    FRAME_SIZE,
    HISTORY_FRAMES,
    FEATURE_COUNT,
    makeFrame,
